...
static int async_direct_loop(snd_pcm_t *handle,
  ...
  int err, count;
  data.samples =
      NULL; /* we do not require the global sample area for direct write */
...
int main(int argc, char *argv[]) {
    ...
    switch (c) {
      ...
      case 'm':
        for (method = 0; transfer_methods[method].name; method++)
          if (!strcasecmp(transfer_methods[method].name, optarg)) break;
        if (transfer_methods[method].name == NULL) method = 0;
        break;
      case 'o': {
        int formati = 0;
        for (; formati < SND_PCM_FORMAT_LAST; formati++) {
          format = (snd_pcm_format_t)formati;
          const char *format_name = snd_pcm_format_name(format);
          if (format_name)
            if (!strcasecmp(format_name, optarg)) break;
        }
        if (formati == SND_PCM_FORMAT_LAST) format = SND_PCM_FORMAT_S16;
  ...
  samples = (short int *)malloc(
      (period_size * channels * snd_pcm_format_physical_width(format)) / 8);
  if (samples == NULL) {
    printf("No enough memory\n");
    exit(EXIT_FAILURE);
  }
