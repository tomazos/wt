...
struct traits<sparse_solve_retval_base<DecompositionType, Rhs> > {
  ...
  typedef SparseMatrix<typename Rhs::Scalar, Rhs::Options, typename Rhs::Index>
      ReturnType;
};
...
struct sparse_solve_retval_base
  ...
  inline void evalTo(Dest& dst) const {
    static_cast<const sparse_solve_retval<DecompositionType, Rhs>*>(this)
        ->evalTo(dst);
  }

 protected:
  template <typename DestScalar, int DestOptions, typename DestIndex>
  inline void defaultEvalTo(
      SparseMatrix<DestScalar, DestOptions, DestIndex>& dst) const {
...
#define EIGEN_MAKE_SPARSE_SOLVE_HELPERS(DecompositionType, Rhs)             \
  ...
  typedef Eigen::internal::sparse_solve_retval_base<DecompositionType, Rhs> \
      Base;                                                                 \
  using Base::dec;                                                          \
  using Base::rhs;                                                          \
  using Base::rows;                                                         \
...
struct traits<solve_retval_with_guess<DecompositionType, Rhs, Guess> > {
  typedef typename DecompositionType::MatrixType MatrixType;
  typedef Matrix<typename Rhs::Scalar, MatrixType::ColsAtCompileTime,
                 Rhs::ColsAtCompileTime, Rhs::PlainObject::Options,
...
struct solve_retval_with_guess
    : public ReturnByValue<
          solve_retval_with_guess<DecompositionType, Rhs, Guess> > {
  typedef typename DecompositionType::Index Index;

  solve_retval_with_guess(const DecompositionType& dec, const Rhs& rhs,
                          const Guess& guess)
