...
class SparseMatrixBase : public EigenBase<Derived> {
 public:
  ...
  /** This is the "real scalar" type; if the \a Scalar type is already real
    *numbers
    * (e.g. int, float or double) then \a RealScalar is just the same as \a
    *Scalar. If
  ...
  typedef CwiseNullaryOp<internal::scalar_constant_op<Scalar>,
                         Matrix<Scalar, Dynamic, Dynamic> > ConstantReturnType;
...
#undef EIGEN_CURRENT_STORAGE_BASE_CLASS
  ...
  /** \returns the size of the inner dimension according to the storage order,
    * i.e., the number of rows for a columns major matrix, and the number of
   * cols otherwise */
 ...
 protected:
  ...
  inline void assignGeneric(const OtherDerived& other) {
    // const bool transpose = (Flags & RowMajorBit) != (OtherDerived::Flags &
    // RowMajorBit);
    eigen_assert(
        (((internal::traits<Derived>::SupportedAccessPatterns &
  ...
  friend std::ostream& operator<<(std::ostream& s, const SparseMatrixBase& m) {
    ...
    if (Flags & RowMajorBit) {
      ...
      for (Index row = 0; row < nm.outerSize(); ++row) {
        Index col = 0;
        for (typename NestedCleaned::InnerIterator it(nm.derived(), row); it;
             ++it) {
      ...
      if (m.cols() == 1) {
        Index row = 0;
        for (typename NestedCleaned::InnerIterator it(nm.derived(), 0); it;
             ++it) {
        ...
        s << static_cast<
            const SparseMatrixBase<SparseMatrix<Scalar, RowMajorBit, Index> >&>(
            trans);
      }
    }
    return s;
...
#define EIGEN_SPARSE_CWISE_PRODUCT_RETURN_TYPE                              \
  ...
  friend const SparseDiagonalProduct<OtherDerived, Derived> operator*(
    ...
    return SparseDiagonalProduct<OtherDerived, Derived>(lhs.derived(),
                                                        rhs.derived());
  }

  /** dense * sparse (return a dense object unless it is an outer product) */
  template <typename OtherDerived>
  friend const typename DenseSparseProductReturnType<OtherDerived,
                                                     Derived>::Type
  operator*(const MatrixBase<OtherDerived>& lhs, const Derived& rhs) {
    return typename DenseSparseProductReturnType<OtherDerived, Derived>::Type(
        lhs.derived(), rhs);
  }
  ...
  // inner-vector
  typedef Block < Derived, IsRowMajor ? 1 : Dynamic, IsRowMajor ? Dynamic : 1,
      true > InnerVectorReturnType;
  typedef Block < const Derived, IsRowMajor ? 1 : Dynamic,
      IsRowMajor ? Dynamic : 1, true > ConstInnerVectorReturnType;
