...
/** \ingroup SparseCore_Module
  ...
  *
  * This class is an expression of a sefladjoint matrix from a triangular part
  *of a matrix
...
void permute_symm_to_symm(const MatrixType& mat,
                          SparseMatrix<typename MatrixType::Scalar, DestOrder,
                                       typename MatrixType::Index>& _dest,
                          const typename MatrixType::Index* perm = 0);

template <int UpLo, typename MatrixType, int DestOrder>
void permute_symm_to_fullsymm(
    const MatrixType& mat, SparseMatrix<typename MatrixType::Scalar, DestOrder,
                                        typename MatrixType::Index>& _dest,
    const typename MatrixType::Index* perm = 0);
}
...
class SparseSelfAdjointView
 ...
 public:
  ...
  /** \returns an expression of the matrix product between a sparse self-adjoint
    ...
    *compared to a general sparse-sparse matrix product.
    * Indeed, the SparseSelfadjointView operand is first copied into a temporary
    *SparseMatrix before computing the product.
    */
  template <typename OtherDerived>
  SparseSparseProduct<typename OtherDerived::PlainObject, OtherDerived>
  operator*(const SparseMatrixBase<OtherDerived>& rhs) const {
    return SparseSparseProduct<typename OtherDerived::PlainObject,
                               OtherDerived>(*this, rhs.derived());
  }
  ...
  /** \returns an expression of the matrix product between a sparse matrix \a
    ...
    */
  template <typename OtherDerived>
  friend SparseSparseProduct<OtherDerived, typename OtherDerived::PlainObject>
  operator*(const SparseMatrixBase<OtherDerived>& lhs,
            const SparseSelfAdjointView& rhs) {
    return SparseSparseProduct<OtherDerived,
  ...
  SparseSelfAdjointTimeDenseProduct<MatrixType, OtherDerived, UpLo> operator*(
    ...
    return SparseSelfAdjointTimeDenseProduct<MatrixType, OtherDerived, UpLo>(
        m_matrix, rhs.derived());
  }

  /** Efficient dense vector/matrix times sparse self-adjoint matrix product */
  template <typename OtherDerived>
  friend DenseTimeSparseSelfAdjointProduct<OtherDerived, MatrixType, UpLo>
  operator*(const MatrixBase<OtherDerived>& lhs,
            const SparseSelfAdjointView& rhs) {
    return DenseTimeSparseSelfAdjointProduct<OtherDerived, _MatrixTypeNested,
                                             UpLo>(lhs.derived(), rhs.m_matrix);
  }
  ...
  SparseSymmetricPermutationProduct<_MatrixTypeNested, UpLo> twistedBy(
    ...
    return SparseSymmetricPermutationProduct<_MatrixTypeNested, UpLo>(m_matrix,
                                                                      perm);
  }

  template <typename SrcMatrixType, int SrcUpLo>
  SparseSelfAdjointView& operator=(const SparseSymmetricPermutationProduct<
...
const SparseSelfAdjointView<Derived, UpLo>
...
template <typename Derived>
template <unsigned int UpLo>
SparseSelfAdjointView<Derived, UpLo>
...
SparseSelfAdjointView<MatrixType, UpLo>::rankUpdate(
    const SparseMatrixBase<DerivedU>& u, const Scalar& alpha) {
  SparseMatrix < Scalar, MatrixType::Flags& RowMajorBit
                             ? RowMajor
                             : ColMajor > tmp = u * u.adjoint();
  if (alpha == Scalar(0))
    m_matrix.const_cast_derived() = tmp.template triangularView<UpLo>();
  else
    m_matrix.const_cast_derived() +=
        alpha * tmp.template triangularView<UpLo>();
...
struct traits<SparseSelfAdjointTimeDenseProduct<Lhs, Rhs, UpLo> >
    : traits<ProductBase<SparseSelfAdjointTimeDenseProduct<Lhs, Rhs, UpLo>, Lhs,
                         Rhs> > {
...
template <typename Lhs, typename Rhs, int UpLo>
class SparseSelfAdjointTimeDenseProduct
    : public ProductBase<SparseSelfAdjointTimeDenseProduct<Lhs, Rhs, UpLo>, Lhs,
                         Rhs> {
 public:
 ...
 private:
  SparseSelfAdjointTimeDenseProduct& operator=(
...
struct traits<DenseTimeSparseSelfAdjointProduct<Lhs, Rhs, UpLo> >
    : traits<ProductBase<DenseTimeSparseSelfAdjointProduct<Lhs, Rhs, UpLo>, Lhs,
                         Rhs> > {};
}

template <typename Lhs, typename Rhs, int UpLo>
class DenseTimeSparseSelfAdjointProduct
    : public ProductBase<DenseTimeSparseSelfAdjointProduct<Lhs, Rhs, UpLo>, Lhs,
                         Rhs> {
 public:
 ...
 private:
  DenseTimeSparseSelfAdjointProduct& operator=(
  ...
  for (Index j = 0; j < size; ++j) {
    for (typename MatrixType::InnerIterator it(mat, j); it; ++it) {
      ...
      if (UpLo == (Upper | Lower)) {
        ...
        dest.valuePtr()[k] = it.value();
      } else if (((UpLo & Lower) == Lower && r > c) ||
                 ((UpLo & Upper) == Upper && r < c)) {
...
void permute_symm_to_symm(const MatrixType& mat,
  ...
  enum {
    ...
    DstUpLo =
        DstOrder == RowMajor ? (_DstUpLo == Upper ? Lower : Upper) : _DstUpLo,
    SrcUpLo =
        SrcOrder == RowMajor ? (_SrcUpLo == Upper ? Lower : Upper) : _SrcUpLo
  };
  ...
  for (Index j = 0; j < size; ++j) {
    ...
    for (typename MatrixType::InnerIterator it(mat, j); it; ++it) {
      Index i = it.index();
      if ((int(SrcUpLo) == int(Lower) && i < j) ||
          (int(SrcUpLo) == int(Upper) && i > j))
        continue;

      Index ip = perm ? perm[i] : i;
      count[int(DstUpLo) == int(Lower) ? (std::min)(ip, jp)
                                       : (std::max)(ip, jp)]++;
    }
  }
  dest.outerIndexPtr()[0] = 0;
  ...
  for (Index j = 0; j < size; ++j) {
    for (typename MatrixType::InnerIterator it(mat, j); it; ++it) {
      ...
      Index k = count[int(DstUpLo) == int(Lower) ? (std::min)(ip, jp)
                                                 : (std::max)(ip, jp)]++;
