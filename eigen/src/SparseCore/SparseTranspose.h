...
class TransposeImpl<MatrixType, Sparse>
  ...
  typedef typename internal::remove_all<typename MatrixType::Nested>::type
      _MatrixTypeNested;
...
// NOTE: VC10 and VC11 trigger an ICE if don't put typename
// TransposeImpl<MatrixType,Sparse>:: in front of Index,
...
class TransposeImpl<MatrixType, Sparse>::InnerIterator
 ...
 public:
  ...
  typename TransposeImpl<MatrixType, Sparse>::Index row() const {
    return Base::col();
  }
  typename TransposeImpl<MatrixType, Sparse>::Index col() const {
    return Base::row();
  }
};
