...
struct traits<SparseVector<_Scalar, _Options, _Index> > {
  ...
  enum {
    ...
    Flags =
        _Options | NestByRefBit | LvalueBit | (IsColVector ? 0 : RowMajorBit),
...
template <typename Dest, typename Src,
          int AssignmentKind =
              !bool(Src::IsVectorAtCompileTime)
                  ? SVA_RuntimeSwitch
                  : Src::InnerSizeAtCompileTime == 1
                        ? SVA_Outer
                        : SVA_Inner > struct sparse_vector_assign_selector;
}
...
class SparseVector
 ...
 public:
  ...
  /** \returns a reference to the coefficient value at given index \a i
    ...
    *
    * This insertion might be very costly if the number of nonzeros above \a i
    *is large.
    */
 ...
 public:
   ...
   * swaping pointers and attributes only.
...
class SparseVector<Scalar, _Options, _Index>::ReverseInnerIterator {
 public:
  ReverseInnerIterator(const SparseVector& vec, Index outer = 0)
      : m_data(vec.m_data),
        m_id(static_cast<Index>(m_data.size())),
        m_start(0) {
    EIGEN_UNUSED_VARIABLE(outer);
    eigen_assert(outer == 0);
  }
