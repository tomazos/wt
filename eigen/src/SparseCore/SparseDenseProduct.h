...
struct SparseDenseProductReturnType<Lhs, Rhs, 1> {
  typedef typename internal::conditional<
      Lhs::IsRowMajor, SparseDenseOuterProduct<Rhs, Lhs, true>,
...
struct DenseSparseProductReturnType<Lhs, Rhs, 1> {
  typedef typename internal::conditional<
      Rhs::IsRowMajor, SparseDenseOuterProduct<Rhs, Lhs, true>,
      SparseDenseOuterProduct<Lhs, Rhs, false> >::type Type;
};
...
struct traits<SparseDenseOuterProduct<Lhs, Rhs, Tr> > {
  ...
  typedef
      typename scalar_product_traits<typename traits<Lhs>::Scalar,
                                     typename traits<Rhs>::Scalar>::ReturnType
          Scalar;
  ...
  enum {
    ...
    RowsAtCompileTime = Tr ? int(traits<Rhs>::RowsAtCompileTime)
                           : int(traits<Lhs>::RowsAtCompileTime),
    ColsAtCompileTime = Tr ? int(traits<Lhs>::ColsAtCompileTime)
                           : int(traits<Rhs>::ColsAtCompileTime),
    MaxRowsAtCompileTime = Tr ? int(traits<Rhs>::MaxRowsAtCompileTime)
                              : int(traits<Lhs>::MaxRowsAtCompileTime),
    MaxColsAtCompileTime = Tr ? int(traits<Lhs>::MaxColsAtCompileTime)
                              : int(traits<Rhs>::MaxColsAtCompileTime),

    Flags = Tr ? RowMajorBit : 0,

    CoeffReadCost =
        LhsCoeffReadCost + RhsCoeffReadCost + NumTraits<Scalar>::MulCost
  };
};
...
class SparseDenseOuterProduct<Lhs, Rhs, Transpose>::InnerIterator
 ...
 public:
  EIGEN_STRONG_INLINE InnerIterator(const SparseDenseOuterProduct& prod,
                                    Index outer)
      : Base(prod.lhs(), 0),
        m_outer(outer),
        m_factor(get(prod.rhs(), outer,
 ...
 protected:
  ...
  static Scalar get(const _RhsNested& rhs, Index outer, Sparse = Sparse()) {
    ...
    if (it && it.index() == 0) return it.value();
...
template <typename SparseLhsType, typename DenseRhsType, typename DenseResType,
          int LhsStorageOrder = ((SparseLhsType::Flags & RowMajorBit) ==
                                 RowMajorBit)
                                    ? RowMajor
                                    : ColMajor,
          bool ColPerCol = ((DenseRhsType::Flags & RowMajorBit) == 0) ||
                           DenseRhsType::ColsAtCompileTime
                               == 1 > struct sparse_time_dense_product_impl;
...
inline void sparse_time_dense_product(const SparseLhsType& lhs,
  ...
  sparse_time_dense_product_impl<SparseLhsType, DenseRhsType,
                                 DenseResType>::run(lhs, rhs, res, alpha);
}
