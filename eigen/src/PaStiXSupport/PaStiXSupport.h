...
void eigen_pastix(pastix_data_t **pastix_data, int pastix_comm, int n, int *ptr,
                  int *idx, float *vals, int *perm, int *invp, float *x,
...
void eigen_pastix(pastix_data_t **pastix_data, int pastix_comm, int n, int *ptr,
                  int *idx, double *vals, int *perm, int *invp, double *x,
                  int nbrhs, int *iparm, double *dparm) {
  if (n == 0) {
    ptr = NULL;
    idx = NULL;
...
void eigen_pastix(pastix_data_t **pastix_data, int pastix_comm, int n, int *ptr,
                  int *idx, std::complex<float> *vals, int *perm, int *invp,
                  std::complex<float> *x, int nbrhs, int *iparm,
                  double *dparm) {
  if (n == 0) {
    ptr = NULL;
    idx = NULL;
  ...
  c_pastix(pastix_data, pastix_comm, n, ptr, idx,
           reinterpret_cast<COMPLEX *>(vals), perm, invp,
           reinterpret_cast<COMPLEX *>(x), nbrhs, iparm, dparm);
}

void eigen_pastix(pastix_data_t **pastix_data, int pastix_comm, int n, int *ptr,
                  int *idx, std::complex<double> *vals, int *perm, int *invp,
                  std::complex<double> *x, int nbrhs, int *iparm,
                  double *dparm) {
  if (n == 0) {
    ptr = NULL;
    idx = NULL;
  ...
  z_pastix(pastix_data, pastix_comm, n, ptr, idx,
           reinterpret_cast<DCOMPLEX *>(vals), perm, invp,
...
class PastixBase : internal::noncopyable {
 ...
 public:
  ...
  inline const internal::solve_retval<PastixBase, Rhs> solve(
      const MatrixBase<Rhs> &b) const {
    eigen_assert(m_isInitialized && "Pastix solver is not initialized.");
    eigen_assert(rows() == b.rows() &&
                 "PastixBase::solve(): invalid number of rows of the right "
                 "hand side matrix b");
 ...
 protected:
  ...
  mutable Matrix<int, IPARM_SIZE, 1>
      m_iparm;  // integer vector for the input parameters
  mutable Matrix<double, DPARM_SIZE, 1>
...
template <typename Rhs, typename Dest>
bool PastixBase<Base>::_solve(const MatrixBase<Rhs> &b,
...
/** \ingroup PaStiXSupport_Module
  ...
  *default is false
  * NOTE : Note that if the analysis and factorization phase are called
  *separately,
...
class PastixLU : public PastixBase<PastixLU<_MatrixType> > {
 ...
 public:
   ...
   * pattern.
    * Several ordering methods can be used at this step. See the PaStiX user's
   * manual.
    * The result of this operation can be used with successive matrices having
   * the same pattern as \p matrix
    * \sa factorize()
    */
...
/** \ingroup PaStiXSupport_Module
  ...
  * available in the PaStiX library. The matrix A should be symmetric and
  *positive definite
  * WARNING Selfadjoint complex matrices are not supported in the current
  *version of PaStiX
  * The vectors or matrices X and B can be either dense or sparse
  *
...
/** \ingroup PaStiXSupport_Module
  ...
  *based on the PaStiX library
  *
  * This class is used to solve the linear systems A.X = B via a LDL^T
