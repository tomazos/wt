...
/** \internal
  ...
  *matrices
  * and dense matrices. Supernodal and other fancy format are not supported by
  *this wrapper.
  *
...
class SuperLUBase : internal::noncopyable {
 ...
 public:
  ...
  inline const internal::solve_retval<SuperLUBase, Rhs> solve(
      const MatrixBase<Rhs> &b) const {
    eigen_assert(m_isInitialized && "SuperLU is not initialized.");
    eigen_assert(rows() == b.rows() &&
                 "SuperLU::solve(): invalid number of rows of the right hand "
                 "side matrix b");
...
/** \ingroup SuperLUSupport_Module
  ...
  *LU factorization
  * using the SuperLU library. The sparse matrix A must be squared and
  *invertible. The vectors or matrices
...
template <typename MatrixType>
template <typename Rhs, typename Dest>
void SuperLU<MatrixType>::_solve(const MatrixBase<Rhs> &b,
                                 MatrixBase<Dest> &x) const {
  eigen_assert(m_factorizationIsOk &&
               "The decomposition is not in a valid state for solving, you "
               "must first call either compute() or "
...
void SuperLUBase<MatrixType, Derived>::extractData() const {
  eigen_assert(m_factorizationIsOk &&
               "The decomposition is not in a valid state for extracting "
               "factors, you must first call either compute() or "
               "analyzePattern()/factorize()");
...
/** \ingroup SuperLUSupport_Module
  * \class SuperILU
  * \brief A sparse direct \b incomplete LU factorization and solver based on
  *the SuperLU library
  *
  * This class allows to solve for an approximate solution of A.X = B sparse
  *linear problems via an incomplete LU factorization
  ...
  *SparseMatrix<>
  *
  * \sa \ref TutorialSparseDirectSolvers, class ConjugateGradient, class
  *BiCGSTAB
  */
...
template <typename MatrixType>
template <typename Rhs, typename Dest>
void SuperILU<MatrixType>::_solve(const MatrixBase<Rhs> &b,
                                  MatrixBase<Dest> &x) const {
  eigen_assert(m_factorizationIsOk &&
               "The decomposition is not in a valid state for solving, you "
               "must first call either compute() or "
