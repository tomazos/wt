 ...
 *QR decomposition
 * of sparse matrices. The result is then used to solve linear leasts_square
 *systems.
...
class SPQR {
  ...
  */
  ...
  inline const internal::solve_retval<SPQR, Rhs> solve(
      const MatrixBase<Rhs>& B) const {
    eigen_assert(
        m_isInitialized &&
        " The QR factorization should be computed first, call compute()");
    eigen_assert(this->rows() == B.rows() &&
                 "SPQR::solve(): invalid number of rows of the right hand side "
                 "matrix B");
