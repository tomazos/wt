...
/** \ingroup LU_Module
  ...
  *pivoting: the matrix A is
  * decomposed as \f$ A = P^{-1} L U Q^{-1} \f$ where L is
  *unit-lower-triangular, U is
  * upper-triangular, and P and Q are permutation matrices. This is a
  *rank-revealing LU
  * decomposition. The eigenvalues (diagonal coefficients) of U are sorted in
  *such a way that any
  * zeros are at the end.
  *
  * This decomposition provides the generic approach to solving systems of
  *linear equations, computing
  * the rank, invertibility, inverse, kernel, and determinant.
  *
  * This LU decomposition is very stable and well tested with large matrices.
  *However there are use cases where the SVD
  * decomposition is inherently more stable and/or flexible. For example, when
  *computing the kernel of a matrix,
  * working with the SVD allows to select the smallest singular values of the
  *matrix, something that
  * the LU decomposition doesn't see.
  *
  * The data of the LU decomposition can be directly accessed through the
  *methods matrixLU(),
  ...
  *
  * \sa MatrixBase::fullPivLu(), MatrixBase::determinant(),
  *MatrixBase::inverse()
  */
...
class FullPivLU {
 public:
  ...
  typedef typename internal::plain_row_type<MatrixType, Index>::type
      IntRowVectorType;
  typedef typename internal::plain_col_type<MatrixType, Index>::type
      IntColVectorType;
  ...
  /** \returns the image of the matrix, also called its column-space. The
    ...
    *decomposition.
    *                       The reason why it is needed to pass it here, is that
    *this allows
    *                       a large optimization, as otherwise this method would
  ...
  /** \return a solution x to the equation Ax=b, where A is the matrix of which
    ...
    *
    * \param b the right-hand-side of the equation to solve. Can be a vector or
    *a matrix,
    *          the only requirement in order for the equation to make sense is
    *that
    *          b.rows()==A.rows(), where A is the matrix of which *this is the
    *LU decomposition.
    *
    * \returns a solution.
    *
  ...
  inline const internal::solve_retval<FullPivLU, Rhs> solve(
  ...
  /** Allows to prescribe a threshold to be used by certain methods, such as
    *rank(),
    * who need to determine when pivots are to be considered nonzero. This is
    *not used for the
  ...
  RealScalar threshold() const {
    ...
    return m_usePrescribedThreshold
               ...
               // tuning" thread on the list)
               // and turns out to be identical to Higham's formula used already
               // in LDLt.
  ...
  /** \returns the inverse of the matrix of which *this is the LU decomposition.
    ...
    *coefficients.
    *       Use isInvertible() to first determine whether this matrix is
    *invertible.
    *
    * \sa MatrixBase::inverse()
    */
  inline const internal::solve_retval<FullPivLU,
                                      typename MatrixType::IdentityReturnType>
  inverse() const {
    eigen_assert(m_isInitialized && "LU is not initialized.");
    eigen_assert(m_lu.rows() == m_lu.cols() &&
                 "You can't take the inverse of a non-square matrix!");
    return internal::solve_retval<FullPivLU,
                                  typename MatrixType::IdentityReturnType>(
...
template <typename MatrixType>
FullPivLU<MatrixType>& FullPivLU<MatrixType>::compute(
...
struct kernel_retval<FullPivLU<_MatrixType> >
  ...
  void evalTo(Dest& dst) const {
    ...
    if (dimker == 0) {
      ...
      // but let's
      // avoid crashing/asserting as that depends on floating point
      // calculations. Let's
    ...
    // of
    // the main diagonal. We need that to be able to apply our triangular
    // solvers.
    ...
    // the
    // negative of the RHS, but for performance we just put the negative sign
    // elsewhere, see below.
  ...
  void evalTo(Dest& dst) const {
    ...
    if (rows > cols) {
      c.bottomRows(rows - cols) -=
          dec().matrixLU().bottomRows(rows - cols) * c.topRows(cols);
    }
