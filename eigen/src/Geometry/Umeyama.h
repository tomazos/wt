...
struct umeyama_transform_matrix_type {
  enum {
    ...
    // result
    // is likely to fit on the stack. So here, EIGEN_SIZE_MIN_PREFER_DYNAMIC is
    // not what we want.
  ...
  typedef Matrix<typename traits<MatrixType>::Scalar, HomogeneousDimension,
                 ...
                 AutoAlign |
                     (traits<MatrixType>::Flags & RowMajorBit ? RowMajor
                                                              : ColMajor),
                 HomogeneousDimension, HomogeneousDimension> type;
};
}
...
* \param dst Destination points \f$ \mathbf{y} = \left( y_1, \hdots, y_n \right)
*\f$.
