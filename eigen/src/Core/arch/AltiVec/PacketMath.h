...
// constants
// and it doesn't really work to declare them global, so we define macros
// instead
...
static Packet16uc p16uc_REVERSE = {12, 13, 14, 15, 8, 9, 10, 11,
                                   4,  5,  6,  7,  0, 1, 2,  3};
static Packet16uc p16uc_FORWARD = vec_lvsl(0, (float*)0);
static Packet16uc p16uc_DUPLICATE = {0, 1, 2, 3, 0, 1, 2, 3,
                                     4, 5, 6, 7, 4, 5, 6, 7};
 ...
 *
template<> EIGEN_STRONG_INLINE Packet4i pmul<Packet4i>(const Packet4i& a, const
Packet4i& b)
{
  // Detailed in:
http://freevec.org/content/32bit_signed_integer_multiplication_altivec
  ...
  // NOR the product and select only the negative elements according to the sign
mask
...
EIGEN_STRONG_INLINE Packet4f preduxp<Packet4f>(const Packet4f* vecs) {
  ...
  // Check: http://www.freevec.org/function/matrix_4x4_transpose_floats for
  // explanation
...
struct palign_impl<Offset, Packet4f> {
  static EIGEN_STRONG_INLINE void run(Packet4f& first, const Packet4f& second) {
    if (Offset != 0) first = vec_sld(first, second, Offset * 4);
  }
};
