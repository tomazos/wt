...
plog<Packet4f>(const Packet4f& _x) {
  ...
  */
  ...
  Packet4f invalid_mask = _mm_cmpnge_ps(
      x, _mm_setzero_ps());  // not greater equal is true if x is NaN
...
// It lacks 1 (or 2 bits in some rare cases) of precision, and does not handle
// negative, +inf, or denormalized numbers correctly.
template <>
EIGEN_DEFINE_FUNCTION_ALLOWING_MULTIPLE_DEFINITIONS EIGEN_UNUSED Packet4f
psqrt<Packet4f>(const Packet4f& _x) {
  ...
  Packet4f non_zero_mask =
      _mm_cmpge_ps(_x, pset1<Packet4f>((std::numeric_limits<float>::min)()));
