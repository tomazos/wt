...
struct traits<ProductBase<Derived, _Lhs, _Rhs> > {
  ...
  typedef
      typename scalar_product_traits<typename Lhs::Scalar,
                                     typename Rhs::Scalar>::ReturnType Scalar;
...
class ProductBase : public MatrixBase<Derived> {
 ...
 public:
  ...
  typedef Matrix < Scalar, RowsAtCompileTime == 1 ? 1 : Dynamic,
      ColsAtCompileTime == 1 ? 1 : Dynamic,
      BasePlainObject::Options > DynPlainObject;
  typedef typename internal::conditional<
      (BasePlainObject::SizeAtCompileTime == Dynamic) ||
  ...
  ProductBase(const Lhs& a_lhs, const Rhs& a_rhs) : m_lhs(a_lhs), m_rhs(a_rhs) {
    eigen_assert(a_lhs.cols() == a_rhs.rows() && "invalid matrix product" &&
  ...
  const Diagonal<FullyLazyCoeffBaseProductType, Dynamic> diagonal(
      Index index) const {
    return FullyLazyCoeffBaseProductType(m_lhs, m_rhs).diagonal(index);
  }

  // restrict coeff accessors to 1x1 expressions. No need to care about mutators
  // here since this isnt a Lvalue expression
...
template <typename Derived, typename Lhs, typename Rhs>
const ScaledProduct<Derived> operator*(
