...
template <typename LhsScalar, typename RhsScalar, typename Index, int mr,
          int nr, bool ConjugateLhs = false, bool ConjugateRhs = false>
struct gebp_kernel;
...
struct conj_helper<Scalar, Scalar, false, false> {
  ...
  pmadd(const Scalar& x, const Scalar& y, const Scalar& c) const {
...
struct conj_helper<std::complex<RealScalar>, std::complex<RealScalar>, false,
                   true> {
  ...
  EIGEN_STRONG_INLINE Scalar pmul(const Scalar& x, const Scalar& y) const {
    return Scalar(
        numext::real(x) * numext::real(y) + numext::imag(x) * numext::imag(y),
        numext::imag(x) * numext::real(y) - numext::real(x) * numext::imag(y));
  }
};

template <typename RealScalar>
struct conj_helper<std::complex<RealScalar>, std::complex<RealScalar>, true,
                   false> {
  typedef std::complex<RealScalar> Scalar;
  EIGEN_STRONG_INLINE Scalar
  pmadd(const Scalar& x, const Scalar& y, const Scalar& c) const {
...
struct conj_helper<std::complex<RealScalar>, RealScalar, Conj, false> {
  typedef std::complex<RealScalar> Scalar;
  EIGEN_STRONG_INLINE Scalar
  pmadd(const Scalar& x, const RealScalar& y, const Scalar& c) const {
...
struct conj_helper<RealScalar, std::complex<RealScalar>, false, Conj> {
  typedef std::complex<RealScalar> Scalar;
  EIGEN_STRONG_INLINE Scalar
  pmadd(const RealScalar& x, const Scalar& y, const Scalar& c) const {
    return padd(c, pmul(x, y));
  }
...
// Yes, this is somehow redundant with Map<>, but this version is much much
// lighter,
...
struct blas_traits<CwiseUnaryOp<scalar_multiple_op<Scalar>, NestedXpr> >
  ...
  static inline Scalar extractScalarFactor(const XprType& x) {
    return x.functor().m_other *
           Base::extractScalarFactor(x.nestedExpression());
  }
};
...
struct blas_traits<Transpose<NestedXpr> > : blas_traits<NestedXpr> {
  ...
  typedef Transpose<const typename Base::_ExtractType> ExtractType;  // const to
                                                                     // get rid
                                                                     // of a
                                                                     // compile
                                                                     // error;
