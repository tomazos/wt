...
struct scalar_product_op {
  enum {
    ...
    Vectorizable = is_same<LhsScalar, RhsScalar>::value &&
...
/** \internal
  ...
  *
  * This is a short cut for conj(x) * y which is needed for optimization
  *purpose; in Eigen2 support mode, this becomes x * conj(y)
  */
template <typename LhsScalar, typename RhsScalar>
...
struct functor_traits<scalar_quotient_op<LhsScalar, RhsScalar> > {
  enum {
    Cost = (NumTraits<LhsScalar>::MulCost +
            NumTraits<RhsScalar>::MulCost),  // rough estimate!
 ...
 *  - GCC does not like m_other as a Packet and generate a load every time it
 * needs it
 *  - on the other hand GCC is able to moves the pset1() outside the loop :)
 *  - simpler code ;)
 * (ICC and gcc 4.4 seems to perform well in both cases, the issue is visible
 * with y = a*x + b*y)
 */
template <typename Scalar>
struct scalar_multiple_op {
  ...
  typename add_const_on_value_type<typename NumTraits<Scalar>::Nested>::type
      m_other;
};
template <typename Scalar>
...
struct scalar_multiple2_op {
  typedef
      typename scalar_product_traits<Scalar1, Scalar2>::ReturnType result_type;
  ...
  typename add_const_on_value_type<typename NumTraits<Scalar2>::Nested>::type
      m_other;
};
template <typename Scalar1, typename Scalar2>
...
// 2) each step (where size is 1 for coeff access or PacketSize for packet
// access)
//   base += [size*step, ..., size*step]
//
// TODO: Perhaps it's better to initialize lazily (so not in the constructor but
// in packetOp)
  ...
  linspaced_op_impl(const Scalar& low, const Scalar& step)
      : m_low(low),
        ...
        m_base(padd(pset1<Packet>(low),
                    pmul(pset1<Packet>(step),
                         plset<Scalar>(-packet_traits<Scalar>::size)))) {}
...
// all functors allow linear access, except scalar_identity_op. So we fix here a
// quick meta
// to indicate whether a functor allows linear access, just always answering
// 'yes' except for
// scalar_identity_op.
...
// In Eigen, any binary op (Product, CwiseBinaryOp) require the Lhs and Rhs to
// have the same scalar type, except for multiplication
...
/** \internal
  ...
  */
/* If you wonder why doing the pset1() in packetOp() is an optimization check
 * scalar_multiple_op */
template <typename Scalar>
...
struct scalar_pow_op {
  // FIXME default copy constructors seems bugged with std::complex<>
  inline scalar_pow_op(const scalar_pow_op& other)
...
/** \internal
  * \brief Template functor to compute the quotient between a scalar and array
 * entries.
...
// Eigen.
// this macro is really needed because functor_traits must be specialized after
// it is declared but before it is used...
