...
/** \internal \returns a packet with elements of \a *from duplicated.
  * For instance, for a packet of 8 elements, 4 scalar will be read from \a
 * *from and
  * duplicated to form:
 * {from[0],from[0],from[1],from[1],,from[2],from[2],,from[3],from[3]}
...
/** \internal \returns a packet where the element i contains the sum of the
 * packet of \a vec[i] */
template <typename Packet>
...
inline Packet pcplxflip(const Packet& a) {
  // FIXME: uncomment the following in case we drop the internal imag and real
  // functions.
  //   using std::imag;
  //   using std::real;
...
/** \internal copy a packet with constant coeficient \a a (e.g., [a,a,a,a]) to
...
/** \internal update \a first using the concatenation of the packet_size minus
  *\a Offset last elements
  * of \a first and \a Offset first elements of \a second.
  *
  * This function is currently only used to optimize matrix-vector products on
  *unligned matrices.
  * It takes 2 packets that represent a contiguous memory array, and returns a
  *packet starting
