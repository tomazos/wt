...
/** \class DenseBase
  ...
  *vector, arrays,
  * and related expression types). The common Eigen API for dense objects is
...
class DenseBase
    ...
    : public DenseCoeffsBase<Derived>
      ...
      {
 public:
  using internal::special_scalar_op_base<
      Derived, typename internal::traits<Derived>::Scalar,
      typename NumTraits<typename internal::traits<Derived>::Scalar>::Real>::
  operator*;
  ...
  enum {
    ...
    /**< This value is equal to the maximum possible number of rows that this
      *expression
      * might have. If this expression might have an arbitrarily high number of
      *rows,
    ...
    MaxSizeAtCompileTime = (internal::size_at_compile_time<
    ...
    /**< This is set to true if either the number of rows or the number of
      ...
      * we are dealing with a column-vector (if there is only one column) or
     * with
      * a row-vector (if there is only one row). */

    Flags = internal::traits<Derived>::Flags,
    /**< This stores expression \ref flags flags which may or may not be
     * inherited by new expressions
    ...
    /**< This is a rough measure of how expensive it is to read one coefficient
     * from
      * this expression.
      */
  ...
  /** \returns the outer size.
    *
    * \note For a vector, this returns just 1. For a matrix (non-vector), this
    *is the major dimension
    * with respect to the \ref TopicStorageOrders "storage order", i.e., the
    *number of columns for a
    * column-major matrix, and the number of rows for a row-major matrix. */
  Index outerSize() const {
    return IsVectorAtCompileTime ? 1 : int(IsRowMajor) ? this->rows()
                                                       : this->cols();
  }
  ...
  Index innerSize() const {
    return IsVectorAtCompileTime ? this->size() : int(IsRowMajor)
                                                      ? this->cols()
                                                      : this->rows();
  }

  /** Only plain matrices/arrays, not expressions, may be resized; therefore the
   * only useful resize methods are
    * Matrix::resize() and Array::resize(). The present method only asserts that
   * the new size equals the old size, and does
    * nothing else.
    */
  ...
  typedef CwiseNullaryOp<internal::scalar_constant_op<Scalar>, Derived>
      ConstantReturnType;
  ...
  typedef Matrix<
      typename NumTraits<typename internal::traits<Derived>::Scalar>::Real,
      internal::traits<Derived>::ColsAtCompileTime, 1> EigenvaluesReturnType;
  ...
  /** \internal Copies \a other into *this without evaluating other. \returns a
   * reference to *this. */
  template <typename OtherDerived>
 ...
 public:
  ...
  static const ConstantReturnType Constant(Index rows, Index cols,
                                           const Scalar& value);
  static const ConstantReturnType Constant(Index size, const Scalar& value);
  static const ConstantReturnType Constant(const Scalar& value);

  static const SequentialLinSpacedReturnType LinSpaced(Sequential_t, Index size,
                                                       const Scalar& low,
                                                       const Scalar& high);
  ...
  static const CwiseNullaryOp<CustomNullaryOp, Derived> NullaryExpr(
  ...
  inline const typename internal::conditional<
  ...
  typename internal::traits<Derived>::Scalar minCoeff(IndexType* row,
                                                      IndexType* col) const;
  template <typename IndexType>
  typename internal::traits<Derived>::Scalar maxCoeff(IndexType* row,
                                                      IndexType* col) const;
  template <typename IndexType>
  ...
  typename internal::result_of<
      BinaryOp(typename internal::traits<Derived>::Scalar)>::type
  ...
  static const CwiseNullaryOp<internal::scalar_random_op<Scalar>, Derived>
  ...
  inline const Select<Derived, ThenDerived,
                      typename ThenDerived::ConstantReturnType>
  select(const DenseBase<ThenDerived>& thenMatrix,
         const typename ThenDerived::Scalar& elseScalar) const;

  template <typename ElseDerived>
  inline const Select<Derived, typename ElseDerived::ConstantReturnType,
                      ElseDerived>
  ...
  inline const ReplicateReturnType replicate(Index rowFacor,
 ...
 * Only do it when debugging Eigen, as this borders on paranoiac and could slow
 * compilation down
 */
