...
template <typename Index>
Index cs_tdfs(Index j, Index k, Index* head, const Index* next, Index* post,
              Index* stack) {
  int i, p, top = 0;
 ...
 * SparseMatrix object. Both the upper and lower parts have to be stored, but
 * the diagonal entries are optional.
...
void minimum_degree_ordering(SparseMatrix<Scalar, ColMajor, Index>& C,
  ...
  int d, dk, dext, lemax = 0, e, elenk, eln, i, j, k, k1, k2, k3, jlast, ln,
                   dense, nzmax, mindeg = 0, nvi, nvj, nvk, mark, wnvi, ok,
                   nel = 0, p, p1, p2, p3, p4, pj, pk, pk1, pk2, pn, q, t;
  unsigned int h;

  Index n = C.cols();
  dense = std::max<Index>(
      16, Index(10 * sqrt(double(n)))); /* find dense threshold */
  ...
  while (nel < n) /* while (selecting pivots) do */
  {
    ...
    mark = internal::cs_wclear<Index>(mark, lemax, w,
