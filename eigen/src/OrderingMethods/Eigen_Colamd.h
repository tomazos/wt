...
template <typename Index>
static Index init_rows_cols(Index n_row, Index n_col, Colamd_Row<Index> Row[],
                            colamd_col<Index> col[], Index A[], Index p[],
                            Index stats[COLAMD_STATS]);

template <typename Index>
static void init_scoring(Index n_row, Index n_col, Colamd_Row<Index> Row[],
                         colamd_col<Index> Col[], Index A[], Index head[],
                         double knobs[COLAMD_KNOBS], Index *p_n_row2,
                         Index *p_n_col2, Index *p_max_deg);

template <typename Index>
static Index find_ordering(Index n_row, Index n_col, Index Alen,
                           Colamd_Row<Index> Row[], colamd_col<Index> Col[],
                           Index A[], Index head[], Index n_col2, Index max_deg,
                           Index pfree);
...
template <typename Index>
static void detect_super_cols(colamd_col<Index> Col[], Index A[], Index head[],
                              Index row_start, Index row_length);

template <typename Index>
static Index garbage_collection(Index n_row, Index n_col,
                                Colamd_Row<Index> Row[],
                                colamd_col<Index> Col[], Index A[],
                                Index *pfree);
 ...
 */
template <typename Index>
static bool colamd(Index n_row, Index n_col, Index Alen, Index *A, Index *p,
  ...
  Eigen::internal::init_scoring(n_row, n_col, Row, Col, A, p, knobs, &n_row2,
                                &n_col2, &max_deg);
...
static void init_scoring(
  ...
  dense_row_count =
      COLAMD_MAX(0, COLAMD_MIN(knobs[COLAMD_DENSE_ROW] * n_col, n_col));
  dense_col_count =
      COLAMD_MAX(0, COLAMD_MIN(knobs[COLAMD_DENSE_COL] * n_row, n_row));
  COLAMD_DEBUG1(
...
static Index find_ordering /* return the number of garbage collections */
  ...
  for (k = 0; k < n_col2; /* 'k' is incremented below */) {
    ...
    if (pfree + needed_memory >= Alen) {
      pfree = Eigen::internal::garbage_collection(n_row, n_col, Row, Col, A,
                                                  &A[pfree]);
      ngarbage++;
    ...
    Eigen::internal::detect_super_cols(Col, A, head, pivot_row_start,
                                       pivot_row_length);
