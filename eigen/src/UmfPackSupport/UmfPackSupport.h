...
struct umfpack_helper_is_sparse_plain<
    SparseMatrix<Scalar, Options, StorageIndex> > : true_type {};
template <typename Scalar, int Options, typename StorageIndex>
struct umfpack_helper_is_sparse_plain<
    MappedSparseMatrix<Scalar, Options, StorageIndex> > : true_type {};
}
...
/** \ingroup UmfPackSupport_Module
  ...
  *factorization
  * using the UmfPack library. The sparse matrix A must be squared and full
  *rank.
...
class UmfPackLU : internal::noncopyable {
 ...
 public:
  ...
  inline const internal::solve_retval<UmfPackLU, Rhs> solve(
      const MatrixBase<Rhs> &b) const {
    eigen_assert(m_isInitialized && "UmfPackLU is not initialized.");
    eigen_assert(rows() == b.rows() &&
                 "UmfPackLU::solve(): invalid number of rows of the right hand "
                 "side matrix b");
 ...
 protected:
  ...
  void grapInput_impl(const InputMatrixType &mat, internal::true_type) {
    ...
    if (((MatrixType::Flags & RowMajorBit) == RowMajorBit) ||
        sizeof(typename MatrixType::Index) != sizeof(int) ||
        !mat.isCompressed()) {
  ...
  void grapInput(const InputMatrixType &mat) {
    grapInput_impl(mat,
                   internal::umfpack_helper_is_sparse_plain<InputMatrixType>());
  }

  void analyzePattern_impl() {
    int errorCode = 0;
    errorCode = umfpack_symbolic(m_copyMatrix.rows(), m_copyMatrix.cols(),
...
template <typename MatrixType>
template <typename BDerived, typename XDerived>
bool UmfPackLU<MatrixType>::_solve(const MatrixBase<BDerived> &b,
  ...
  for (int j = 0; j < rhsCols; ++j) {
    errorCode = umfpack_solve(UMFPACK_A, m_outerIndexPtr, m_innerIndexPtr,
                              m_valuePtr, &x.col(j).coeffRef(0),
