...
class ColPivHouseholderQR {
 public:
  ...
  typedef Matrix<Scalar, RowsAtCompileTime, RowsAtCompileTime, Options,
                 MaxRowsAtCompileTime, MaxRowsAtCompileTime> MatrixQType;
  ...
  */
  ...
  inverse() const {
    eigen_assert(m_isInitialized && "ColPivHouseholderQR is not initialized.");
    return internal::solve_retval<ColPivHouseholderQR,
                                  typename MatrixType::IdentityReturnType>(
...
ColPivHouseholderQR<MatrixType>& ColPivHouseholderQR<MatrixType>::compute(
  ...
  RealScalar threshold_helper = m_colSqNorms.maxCoeff() *
                                numext::abs2(NumTraits<Scalar>::epsilon()) /
                                RealScalar(rows);
  ...
  for (Index k = 0; k < size; ++k) {
    // first, we look up in our table m_colSqNorms which column has the biggest
    // squared norm
    Index biggest_col_index;
    RealScalar biggest_col_sq_norm =
        m_colSqNorms.tail(cols - k).maxCoeff(&biggest_col_index);
    biggest_col_index += k;

    // since our table m_colSqNorms accumulates imprecision at every step, we
    // must now recompute
    // the actual squared norm of the selected column.
    // Note that not doing so does result in solve() sometimes returning inf/nan
    // values
    ...
    if (k != biggest_col_index) {
      ...
      std::swap(m_colSqNorms.coeffRef(k),
                m_colSqNorms.coeffRef(biggest_col_index));
    ...
    m_colSqNorms.tail(cols - k - 1) -=
        m_qr.row(k).tail(cols - k - 1).cwiseAbs2();
  }
