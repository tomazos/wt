...
/** \ingroup IterativeLinearSolvers_Module
  ...
  *    (in addition to the diagonal element ). Note that @p fill is computed
  *from
  ...
  *
  * References : Yousef Saad, ILUT: A dual threshold incomplete LU
  *factorization,
  *              Numerical Linear Algebra with Applications, 1(4), pp 387-402,
  *1994.
  *
  * NOTE : The following implementation is derived from the ILUT implementation
  * in the SPARSKIT package, Copyright (C) 2005, the Regents of the University
  *of Minnesota
...
class IncompleteLUT : internal::noncopyable {
 ...
 public:
  ...
  IncompleteLUT(const MatrixType& mat, const RealScalar& droptol =
  ...
  inline const internal::solve_retval<IncompleteLUT, Rhs> solve(
      const MatrixBase<Rhs>& b) const {
    eigen_assert(m_isInitialized && "IncompleteLUT is not initialized.");
    eigen_assert(cols() == b.rows() &&
                 "IncompleteLUT::solve(): invalid number of rows of the right "
                 "hand side matrix b");
...
void IncompleteLUT<Scalar>::analyzePattern(const _MatrixType& amat) {
  ...
  AtA.prune(keep_diag());
  internal::minimum_degree_ordering<Scalar, Index>(
      AtA, m_P);  // Then compute the AMD ordering...
...
void IncompleteLUT<Scalar>::factorize(const _MatrixType& amat) {
  ...
  VectorXi jr(n);  // Indicate the position of the nonzero elements in the
                   // vector u -- A zero location is indicated by -1
