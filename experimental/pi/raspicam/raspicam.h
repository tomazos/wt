...
class RaspiCam {
  ...
  */
   ...
   * You can decide how image is given by setting type. The input buffer
   * provided must have the appropriate
   * size accordingly. You can use getImageTypeSize() to determine the size
      * NOTE: Change in version 0.0.5. Format is stablished in setFormat
   * function
  ...
  /**Alternative to retrieve. Returns a pointer to the original image data
    *buffer (which is in getFormat() format).
    *
  ...
  */
  ...
  // et specific values for whitebalance. Requires to set seAWB in OFF mode
  // first
