 ...
 * The newly created component will expose ports to the client. All the exposed
 *ports are
 * disabled by default.
 * Note that components are reference counted and creating a component
 *automatically
 * acquires a reference to it (released when \ref mmal_component_destroy is
 *called).
 *
 ...
 * Acquiring a reference on a component will prevent a component from being
 *destroyed until
 * the acquired reference is released (by a call to \ref
 *mmal_component_destroy).
 * References are internally counted so all acquired references need a matching
 *call to
 * release them.
 *
 ...
 * Release an acquired reference on a component. Triggers the destruction of the
 *component when
 * the last reference is being released.
 * \note This is in fact an alias of \ref mmal_component_destroy which is added
 *to make client
 * code clearer.
 *
 ...
 * Release an acquired reference on a component. Only actually destroys the
 *component when
 * the last reference is being released.
 *
 ...
 *
 * Disabling a port will stop all processing on this port and return all
 *(non-processed)
 * buffer headers to the client.
 *
 * If this is a connected output port, the input port to which it is connected
 *shall
 ...
 *
 * Flushing a port will ask the port to send all the buffer headers it currently
 *has
 ...
 * It is also important to note that flushing will also reset the state of the
 *port
 ...
 * This allows a client to allocate memory for a payload buffer based on the
 *preferences
 * of a port. This for instance will allow the port to allocate memory which can
 *be shared
 ...
 * This allows a client to free memory allocated by a previous call to \ref
 ...
 * @param buffer The address of a buffer header pointer, which will be set on
 *return.
 * @param event The specific event FourCC required. See the \ref MmalEvents
 *"pre-defined events".
 * @return MMAL_SUCCESS on success
 */
 ...
 * It is worth noting that the extradata buffer will not be copied in the new
 *format.
 *
 ...
 * The connection shall include a pool of buffer headers suitable for the
 *current format of
 * the output port. The format of the input port shall have been set to the same
 *as that of
 * the input port.
 * Note that connections are reference counted and creating a connection
 *automatically
 * acquires a reference to it (released when \ref mmal_connection_destroy is
 *called).
 *
 * @param connection The address of a connection pointer that will be set to
 *point to the created
 * connection.
 * @param out        The output port to use for the connection.
 * @param in         The input port to use for the connection.
 * @param flags      The flags specifying which type of connection should be
 *created.
 ...
 * Acquiring a reference on a connection will prevent a connection from being
 *destroyed until
 * the acquired reference is released (by a call to \ref
 ...
 * Release an acquired reference on a connection. Triggers the destruction of
 *the connection when
 * the last reference is being released.
 * \note This is in fact an alias of \ref mmal_connection_destroy which is added
 *to make client
 * code clearer.
 *
 ...
 * Release an acquired reference on a connection. Only actually destroys the
 *connection when
 * the last reference is being released.
 * The actual destruction of the connection will start by disabling it, if
 *necessary.
...
/** Enable a connection.
 * The format of the two ports must have been committed before calling this
 *function,
 * although note that on creation, the connection automatically copies and
 *commits the
 * output port's format to the input port.
 *
 * The MMAL_CONNECTION_T::callback field must have been set if the \ref
 *MMAL_CONNECTION_FLAG_TUNNELLING
 * flag was not specified on creation. The client may also set the
 *MMAL_CONNECTION_T::user_data
 ...
 * This function can be used when the client is processing buffer headers and
 *receives
 * a format changed event (\ref MMAL_EVENT_FORMAT_CHANGED). The connection is
 * reconfigured, changing the format of the ports, the number of buffer headers
 *and
 ...
 * @return MMAL_SUCCESS on success.
 */
MMAL_STATUS_T mmal_connection_event_format_changed(
 ...
 */
MMAL_STATUS_T mmal_port_parameter_set_rational(MMAL_PORT_T *port, uint32_t id,
 ...
 * @return MMAL_SUCCESS or error
 */
MMAL_STATUS_T mmal_port_parameter_set_int32(MMAL_PORT_T *port, uint32_t id,
                                            int32_t value) {}
 ...
 * @return MMAL_SUCCESS or error
 */
MMAL_STATUS_T mmal_port_parameter_set_uint32(MMAL_PORT_T *port, uint32_t id,
                                             uint32_t value) {}
 ...
 * @return MMAL_SUCCESS or error
 */
MMAL_STATUS_T mmal_port_parameter_set_boolean(MMAL_PORT_T *port, uint32_t id,
                                              MMAL_BOOL_T value) {}

/** Create a pool of MMAL_BUFFER_HEADER_T associated with a specific port.
 * This allows a client to allocate memory for the payload buffers based on the
 *preferences
 * of a port. This for instance will allow the port to allocate memory which can
 *be shared
 * between the host processor and videocore.
 * After allocation, all allocated buffer headers will have been added to the
 *queue.
 *
 * It is valid to create a pool with no buffer headers, or with zero size
 *payload buffers.
 * The mmal_pool_resize() function can be used to increase or decrease the
 *number of buffer
 ...
 * Releasing a buffer header will decrease its reference counter and when no
 *more references
 * are left, the buffer header will be recycled by calling its 'release'
 *callback function.
 *
 * If a pre-release callback is set (\ref MMAL_BH_PRE_RELEASE_CB_T), this will
 *be invoked
 * before calling the buffer's release callback and potentially postpone buffer
 *recycling.
