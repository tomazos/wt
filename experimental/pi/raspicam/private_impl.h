...
#include "raspicam/private_types.h"
...
class Private_Impl {
  ...
  */
  ...
  /**Grabs the next frame and keeps it in internal buffer. Blocks until next
   * frame arrives
  */
  bool grab();
  ...
  * You can use getFormat to know the current format
  ...
  /**Alternative to retrieve. Returns a pointer to the original image data
   * buffer.
    * Be careful, if you call grab(), this will be rewritten with the new data
   */
  unsigned char *getImageBufferData() const;
  /**
   * Returns the size of the buffer returned in getImagePtr. If is like calling
   * getImageTypeSize(getFormat()). Just for dummies :P
   */
  ...
  */
  ...
  /**
    ...
    *There is currently an upper limit of approximately 330000us (330ms, 0.33s)
    *past which operation is undefined.
    */
  ...
  int getISO() const { return State.ISO; }
  ...
  int getShutterSpeed() const { return State.shutterSpeed; }
  RASPICAM_EXPOSURE getExposure() const { return State.rpc_exposureMode; }
  RASPICAM_AWB getAWB() const { return State.rpc_awbMode; }

  float getAWBG_red() { return State.awbg_red; }

  float getAWBG_blue() { return State.awbg_blue; }

  RASPICAM_IMAGE_EFFECT getImageEffect() const { return State.rpc_imageEffect; }
  RASPICAM_METERING getMetering() const { return State.rpc_exposureMeterMode; }
  bool isHorizontallyFlipped() const { return State.hflip; }
  bool isVerticallyFlipped() const { return State.vflip; }
  ...
  /**Returns the size of the required buffer for the different image types in
   * retrieve
   */
