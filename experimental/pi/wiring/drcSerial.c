...
static void myPullUpDnControl(struct wiringPiNodeStruct *node, int pin,
                              int mode) {
...
static void myDigitalWrite(struct wiringPiNodeStruct *node, int pin,
                           int value) {
...
int drcSetupSerial(const int pinBase, const int numPins, const char *device,
  ...
  if ((fd = serialOpen(device, baud)) < 0)
    return wiringPiFailure(WPI_ALMOST, "Unable to open DRC device (%s): %s",
  ...
  while (serialDataAvail(fd)) (void)serialGetchar(fd);
  ...
  if (!ok) {
    ...
    return wiringPiFailure(WPI_FATAL,
                           "Unable to communicate with DRC serial device");
  }
