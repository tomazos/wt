...
To resize(const From& m);

mat4 scale(float32 sx, float32 sy, float32 sz);

mat4 translate(vec3 delta);

mat4 roll(float32 gamma);

mat4 yaw(float32 alpha);

mat4 pitch(float32 beta);
...
template <typename To, typename From>
To resize(const From& from) {
  To to = To::Zero();

  constexpr size_t rows =
      std::min(size_t(To::RowsAtCompileTime), size_t(From::RowsAtCompileTime));
  constexpr size_t cols =
      std::min(size_t(To::ColsAtCompileTime), size_t(From::ColsAtCompileTime));

  to.template topLeftCorner<rows, cols>() =
      from.template topLeftCorner<rows, cols>();
