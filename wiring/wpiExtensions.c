...
#include "wiring/mcp23008.h"
#include "wiring/mcp23016.h"
#include "wiring/mcp23017.h"
#include "wiring/mcp23s08.h"
#include "wiring/mcp23s17.h"
#include "wiring/sr595.h"
#include "wiring/pcf8574.h"
#include "wiring/pcf8591.h"
#include "wiring/mcp3002.h"
#include "wiring/mcp3004.h"
#include "wiring/mcp4802.h"
#include "wiring/mcp3422.h"
#include "wiring/max31855.h"
#include "wiring/max5322.h"
#include "wiring/sn3218.h"
#include "wiring/drcSerial.h"

#include "wiring/wpiExtensions.h"
...
static int doExtensionMcp23008(char *progName, int pinBase, char *params) {
  ...
  if ((params = extractInt(progName, params, &i2c)) == NULL) return FALSE;
...
static int doExtensionMcp23s08(char *progName, int pinBase, char *params) {
  ...
  if ((params = extractInt(progName, params, &spi)) == NULL) return FALSE;
  ...
  if ((params = extractInt(progName, params, &port)) == NULL) return FALSE;
...
static int doExtensionSr595(char *progName, int pinBase, char *params) {
  ...
  if ((params = extractInt(progName, params, &pins)) == NULL) return FALSE;

  if ((pins < 8) || (pins > 32)) {
    verbError("%s: pin count (%d) out of range - 8-32 expected.", progName,
              pins);
    return FALSE;
  }

  if ((params = extractInt(progName, params, &data)) == NULL) return FALSE;

  if ((params = extractInt(progName, params, &clock)) == NULL) return FALSE;

  if ((params = extractInt(progName, params, &latch)) == NULL) return FALSE;
...
static int doExtensionMcp3422(char *progName, int pinBase, char *params) {
  ...
  if ((params = extractInt(progName, params, &gain)) == NULL) return FALSE;
...
static int doExtensionDrcS(char *progName, int pinBase, char *params) {
  ...
  if ((params = extractStr(progName, params, &port)) == NULL) return FALSE;
  ...
  if ((params = extractInt(progName, params, &baud)) == NULL) return FALSE;
 ...
 *	number. Other parameters after that are decoded by the module in
 *question.
 *********************************************************************************
 */
...
int loadWPiExtension(char *progName, char *extensionData, int printErrors) {
  ...
  *p++ = 0;
  ...
  for (extensionFn = extensionFunctions; extensionFn->name != NULL;
       ++extensionFn) {
