...
#include "cm/private/code.h"
#include "cm/private/debug.h"
#include "cm/private/do.h"
#include "cm/private/func.h"
#include "cm/private/lex.h"
#include "cm/private/mem.h"
#include "cm/private/object.h"
#include "cm/private/opcodes.h"
#include "cm/private/parser.h"
...
static void oneblock(LexState *ls) {
  /* block -> statlist */
  FuncState *fs = ls->fs;
  BlockCnt bl;
  enterblock(fs, &bl, 0);
...
static void dowhilestat(LexState *ls, int line) {
  /* dowhilestat -> DO stat WHILE '(' cond ')' */
  luaX_next(ls); /* skip DO */

  FuncState *fs = ls->fs;
  int do_init = luaK_getlabel(fs);
  BlockCnt bl1;
  enterblock(fs, &bl1, 1); /* loop block */
  oneblock(ls);

  checknext(ls, TK_WHILE);
  checknext(ls, '(');
  int condexit = cond(ls); /* read condition */
  checknext(ls, ')');
  checknext(ls, ';');

  luaK_jumpto(fs, do_init);
  leaveblock(fs);                 /* finish loop */
  luaK_patchtohere(fs, condexit); /* close the loop */
}
...
static void ifstat(LexState *ls, int line) {
  ...
  if (testnext(ls, TK_ELSE)) oneblock(ls); /* 'else' part */
