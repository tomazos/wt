...
class future {
 public:
  ...
  future(const T &result, const event &event)
      : m_result(result), m_event(event) {}

  future(const future<T> &other)
      : m_result(other.m_result), m_event(other.m_event) {}
  ...
  bool valid() const { return m_event != 0; }

  /// Blocks until the computation is complete.
  void wait() const { const_cast<event &>(m_event).wait(); }

  /// Returns the underlying event object.
  event get_event() const { return m_event; }
...
class future<void> {
 public:
  ...
  explicit future(const event &event) : m_event(event) {}
