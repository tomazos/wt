...
#include "compute/functional/get.h"
...
#define BOOST_COMPUTE_PRINT_ELEM(z, n, unused) \
  BOOST_PP_EXPR_IF(n, << ", ") << kernel.make_lit(boost::get<n>(x))
...
#define BOOST_COMPUTE_INJECT_TYPE(z, n, unused) kernel.inject_type<T##n>();
...
#define BOOST_COMPUTE_PRINT_TYPE(z, n, unused) +type_name<T##n>() + "_"

#define BOOST_COMPUTE_PRINT_TYPE_NAME(z, n, unused)                  \
  template <BOOST_PP_ENUM_PARAMS(n, class T)>                        \
  struct type_name_trait<boost::tuple<BOOST_PP_ENUM_PARAMS(n, T)>> { \
    static const char* value() {                                     \
      static std::string name = std::string("boost_tuple_")          \
          BOOST_PP_REPEAT(n, BOOST_COMPUTE_PRINT_TYPE, ~) "t";       \
      return name.c_str();                                           \
    }                                                                \
  };

BOOST_PP_REPEAT_FROM_TO(1, BOOST_COMPUTE_MAX_ARITY,
                        BOOST_COMPUTE_PRINT_TYPE_NAME, ~)
...
struct write_tuple_type_names<1, T, Rest...> {
  void operator()(std::ostream& os) { os << type_name<T>(); }
};
...
struct type_name_trait<boost::tuple<T...>> {
  ...
  static std::string make_type_name() {
    ...
    std::stringstream s;
    s << "boost_tuple_";
    write_tuple_type_names<boost::tuples::length<tuple_type>::value, T...>()(s);
    s << "_t";
    return s.str();
  }
};
...
struct type_name_trait<std::tuple<T...>> {
  ...
  static std::string make_type_name() {
    ...
    std::stringstream s;
    s << "std_tuple_";
    write_tuple_type_names<std::tuple_size<tuple_type>::value, T...>()(s);
    s << "_t";
    return s.str();
  }
};
...
#define BOOST_COMPUTE_GET_N(z, n, unused)                                  \
  ...
  inline meta_kernel& operator<<(                                          \
      meta_kernel& kernel,                                                 \
      const invoked_get<N, Arg, boost::tuple<BOOST_PP_ENUM_PARAMS(n, T)>>& \
          expr) {                                                          \
    typedef typename boost::tuple<BOOST_PP_ENUM_PARAMS(n, T)> T;           \
