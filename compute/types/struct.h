...
#include "compute/type_traits/type_definition.h"
#include "compute/type_traits/type_name.h"
#include "compute/detail/meta_kernel.h"
#include "compute/detail/variadic_macros.h"
...
inline std::string adapt_struct_insert_member(T (Struct::*)[N],
                                              const char *name) {
  std::stringstream s;
...
#define BOOST_COMPUTE_DETAIL_STRUCT_MEMBER_SIZE_SUM(struct_, members_)         \
  BOOST_PP_SEQ_FOLD_LEFT(                                                      \
      BOOST_COMPUTE_DETAIL_STRUCT_MEMBER_SIZE_ADD, 0,                          \
      BOOST_PP_SEQ_TRANSFORM(BOOST_COMPUTE_DETAIL_STRUCT_MEMBER_SIZE, struct_, \
                             members_))
...
#define BOOST_COMPUTE_DETAIL_STRUCT_IS_PACKED(struct_, members_) \
  (sizeof(struct_) ==                                            \
   BOOST_COMPUTE_DETAIL_STRUCT_MEMBER_SIZE_SUM(struct_, members_))
...
#define BOOST_COMPUTE_ADAPT_STRUCT(type, name, members)                        \
  BOOST_STATIC_ASSERT_MSG(BOOST_COMPUTE_DETAIL_STRUCT_IS_PACKED(               \
                              type, BOOST_COMPUTE_PP_TUPLE_TO_SEQ(members)),   \
                          "BOOST_COMPUTE_ADAPT_STRUCT() does not support "     \
  ...
  inline std::string type_definition<type>() {                                 \
    ...
    declaration                                                                \
        << "typedef struct __attribute__((packed)) {\n" BOOST_PP_SEQ_FOR_EACH( \
               BOOST_COMPUTE_DETAIL_ADAPT_STRUCT_INSERT_MEMBER, type,          \
  ...
  inline meta_kernel &operator<<(meta_kernel &k, type s) {                     \
    return k << "(" << #name                                                   \
             << "){" BOOST_PP_SEQ_FOR_EACH_I(                                  \
                    BOOST_COMPUTE_DETAIL_ADAPT_STRUCT_STREAM_MEMBER, s,        \
                    BOOST_COMPUTE_PP_TUPLE_TO_SEQ(members)) << "}";            \
  }                                                                            \
  }                                                                            \
  }                                                                            \
