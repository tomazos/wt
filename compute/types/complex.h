...
inline meta_kernel &operator<<(
    meta_kernel &kernel, const invoked_get<N, Arg, std::complex<T> > &expr) {
...
struct real {
  ...
  template <class Arg>
  detail::invoked_get<0, Arg, std::complex<T> > operator()(const Arg &x) const {
...
struct imag {
  ...
  template <class Arg>
  detail::invoked_get<1, Arg, std::complex<T> > operator()(const Arg &x) const {
...
struct invoked_complex_multiplies {
  ...
  invoked_complex_multiplies(const Arg1 &x, const Arg2 &y) : m_x(x), m_y(y) {}
...
inline meta_kernel &operator<<(
  ...
  kernel << "(" << type_name<value_type>() << ")"
         << "(" << expr.m_x << ".x*" << expr.m_y << ".x-" << expr.m_x << ".y*"
         << expr.m_y << ".y," << expr.m_x << ".y*" << expr.m_y << ".x+"
...
struct invoked_complex_conj {
  ...
  invoked_complex_conj(const Arg &arg) : m_arg(arg) {}
...
class multiplies<std::complex<T> >
 ...
 public:
  multiplies()
      : function<std::complex<T>(std::complex<T>, std::complex<T>)>(
            "complex_multiplies") {}

  template <class Arg1, class Arg2>
  detail::invoked_complex_multiplies<Arg1, Arg2, T> operator()(
...
struct conj {
  ...
  template <class Arg>
  detail::invoked_complex_conj<Arg, T> operator()(const Arg &x) const {
