...
class vector_type {
 public:
  ...
  explicit vector_type(const Scalar scalar) {
    for (int i = 0; i < N; i++) m_value[i] = scalar;
  }
  ...
  vector_type<Scalar, N> &operator=(const vector_type<Scalar, N> &other) {
    std::memcpy(m_value, other.m_value, sizeof(m_value));
    return *this;
  }
  ...
  Scalar &operator[](size_t i) { return m_value[i]; }

  Scalar operator[](size_t i) const { return m_value[i]; }
  ...
  class class_name : public vector_type<cl_scalar, size> {                    \
   public:                                                                    \
    ...
    explicit class_name(scalar_type arg) {                                    \
      BOOST_PP_REPEAT(size, BOOST_COMPUTE_VECTOR_TYPE_ASSIGN_CTOR_SINGLE_ARG, \
                      _)                                                      \
    }                                                                         \
    class_name(BOOST_PP_REPEAT(size,                                          \
                               BOOST_COMPUTE_VECTOR_TYPE_CTOR_ARG_FUNCTION,   \
                               _)) {                                          \
