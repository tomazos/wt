...
#include "compute/iterator/detail/get_base_iterator_buffer.h"
#include "compute/type_traits/is_device_iterator.h"
#include "compute/type_traits/result_of.h"
...
class strided_iterator_base {
 public:
  typedef ::boost::iterator_adaptor<
      ::boost::compute::strided_iterator<Iterator>, Iterator> type;
};
...
struct stride_expr {
  stride_expr(const IndexExpr &expr, Stride stride)
      : m_index_expr(expr), m_stride(stride) {}
...
struct strided_iterator_index_expr {
  ...
  strided_iterator_index_expr(const Iterator &input_iter, const Stride &stride,
                              const IndexExpr &index_expr)
      : m_input_iter(input_iter), m_stride(stride), m_index_expr(index_expr) {}
...
inline meta_kernel &operator<<(
    meta_kernel &kernel,
    const strided_iterator_index_expr<Iterator, Stride, IndexExpr> &expr) {
  return kernel << expr.m_input_iter[make_stride_expr(expr.m_index_expr,
                                                      expr.m_stride)];
}
...
class strided_iterator : public detail::strided_iterator_base<Iterator>::type {
 public:
  ...
  strided_iterator(Iterator iterator, difference_type stride)
      : super_type(iterator), m_stride(static_cast<difference_type>(stride)) {
    // stride must be greater than zero
    BOOST_ASSERT_MSG(stride > 0, "Stride must be greater than zero");
  }

  strided_iterator(const strided_iterator<Iterator> &other)
      : super_type(other.base()), m_stride(other.m_stride) {}

  strided_iterator<Iterator> &operator=(
      const strided_iterator<Iterator> &other) {
    if (this != &other) {
  ...
  size_t get_index() const { return super_type::base().get_index(); }
  ...
  detail::strided_iterator_index_expr<Iterator, difference_type,
                                      IndexExpression>
  operator[](const IndexExpression &expr) const {
    return detail::strided_iterator_index_expr<Iterator, difference_type,
                                               IndexExpression>(
 ...
 private:
  ...
  void increment() { std::advance(super_type::base_reference(), m_stride); }

  void decrement() { std::advance(super_type::base_reference(), -m_stride); }
  ...
  difference_type distance_to(const strided_iterator<Iterator> &other) const {
    return std::distance(this->base_reference(), other.base_reference()) /
           m_stride;
  }
...
/// \endcode
template <class Iterator>
inline strided_iterator<Iterator> make_strided_iterator(
    Iterator iterator,
...
/// auto strided_iterator_end = make_strided_iterator_end(vec.begin(),
/// vec.end(), 3);
///
/// // copy every 3rd element to result
/// boost::compute::copy(
///         strided_iterator_begin,
///         strided_iterator_end,ided_iterator referring to element that would
///         follow
///         the last element accessible through strided_iterator for \p first
///         iterator with \p stride.
///         result.begin(),
...
/// \endcode
template <class Iterator>
strided_iterator<Iterator> make_strided_iterator_end(
