...
class counting_iterator_base {
 public:
  typedef ::boost::iterator_facade< ::boost::compute::counting_iterator<T>, T,
...
class counting_iterator : public detail::counting_iterator_base<T>::type {
 public:
  ...
  counting_iterator(const T &init) : m_init(init) {}

  counting_iterator(const counting_iterator<T> &other) : m_init(other.m_init) {}
  ...
  template <class Expr>
  detail::counting_iterator_index_expr<T, Expr> operator[](
 ...
 private:
  ...
  reference dereference() const { return m_init; }

  bool equal(const counting_iterator<T> &other) const {
    return m_init == other.m_init;
  }

  void increment() { m_init++; }

  void decrement() { m_init--; }

  void advance(difference_type n) { m_init += static_cast<T>(n); }
