...
#include "compute/detail/mpl_vector_to_tuple.h"
#include "compute/types/tuple.h"
#include "compute/type_traits/is_device_iterator.h"
...
struct make_zip_iterator_value_type {
  typedef typename detail::mpl_vector_to_tuple<typename mpl::transform<
      IteratorTuple, make_iterator_value_type<mpl::_1>,
      mpl::back_inserter<mpl::vector<> > >::type>::type type;
};
...
class zip_iterator_base {
 public:
  typedef ::boost::iterator_facade<
      ...
      typename make_zip_iterator_value_type<IteratorTuple>::type> type;
};

template <class IteratorTuple, class IndexExpr>
struct zip_iterator_index_expr {
  typedef
      typename make_zip_iterator_value_type<IteratorTuple>::type result_type;

  zip_iterator_index_expr(const IteratorTuple &iterators,
                          const IndexExpr &index_expr)
      : m_iterators(iterators), m_index_expr(index_expr) {}
...
#define BOOST_COMPUTE_PRINT_ZIP_IDX(z, n, unused)                              \
  ...
  inline meta_kernel &operator<<(                                              \
      ...
      const zip_iterator_index_expr<                                           \
          boost::tuple<BOOST_PP_ENUM_PARAMS(n, Iterator)>, IndexExpr> &expr) { \
    typedef                                                                    \
        typename boost::tuple<BOOST_PP_ENUM_PARAMS(n, Iterator)> tuple_type;   \
    typedef                                                                    \
        typename make_zip_iterator_value_type<tuple_type>::type value_type;    \
    kernel.inject_type<value_type>();                                          \
    return kernel << "(" << type_name<value_type>() << ")"                     \
...
struct iterator_advancer {
  iterator_advancer(size_t n) : m_distance(n) {}
...
class zip_iterator : public detail::zip_iterator_base<IteratorTuple>::type {
 public:
  typedef typename detail::zip_iterator_base<IteratorTuple>::type super_type;
  typedef typename super_type::value_type value_type;
  typedef typename super_type::reference reference;
  typedef typename super_type::difference_type difference_type;
  typedef IteratorTuple iterator_tuple;

  zip_iterator(IteratorTuple iterators) : m_iterators(iterators) {}

  zip_iterator(const zip_iterator<IteratorTuple> &other)
      : m_iterators(other.m_iterators) {}

  zip_iterator<IteratorTuple> &operator=(
      const zip_iterator<IteratorTuple> &other) {
    if (this != &other) {
  ...
  const IteratorTuple &get_iterator_tuple() const { return m_iterators; }

  template <class IndexExpression>
  detail::zip_iterator_index_expr<IteratorTuple, IndexExpression> operator[](
      const IndexExpression &expr) const {
    return detail::zip_iterator_index_expr<IteratorTuple, IndexExpression>(
        m_iterators, expr);
  }
 ...
 private:
  ...
  reference dereference() const { return reference(); }
...
/// \endcode
template <class IteratorTuple>
inline zip_iterator<IteratorTuple> make_zip_iterator(IteratorTuple iterators) {
...
#define BOOST_COMPUTE_ZIP_GET_N(z, n, unused)                                 \
  ...
  inline meta_kernel &operator<<(                                             \
      meta_kernel &kernel,                                                    \
      const invoked_get<N, zip_iterator_index_expr<IteratorTuple, IndexExpr>, \
                        boost::tuple<BOOST_PP_ENUM_PARAMS(n, T)> > &expr) {   \
    ...
    return kernel << boost::get<N>(                                           \
               expr.m_arg.m_iterators)[expr.m_arg.m_index_expr];              \
  }
