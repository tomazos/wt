...
struct discard_iterator_base {
  typedef ::boost::iterator_facade< ::boost::compute::discard_iterator, void,
...
struct discard_iterator_index_expr {
  ...
  discard_iterator_index_expr(const IndexExpr &expr) : m_expr(expr) {}
...
inline meta_kernel &operator<<(
    meta_kernel &kernel, const discard_iterator_index_expr<IndexExpr> &expr) {
  (void)expr;
...
class discard_iterator : public detail::discard_iterator_base::type {
 public:
  ...
  discard_iterator(size_t index = 0) : m_index(index) {}

  discard_iterator(const discard_iterator &other) : m_index(other.m_index) {}
  ...
  /// \internal_
  template <class Expr>
  detail::discard_iterator_index_expr<Expr> operator[](const Expr &expr) const {
 ...
 private:
  ...
  /// \internal_
  reference dereference() const { return 0; }
