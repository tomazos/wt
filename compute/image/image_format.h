...
class image_format {
 public:
  ...
  image_format(const image_format &other) : m_format(other.m_format) {}
  ...
  const cl_image_format *get_format_ptr() const { return &m_format; }

  /// Returns \c true if \c *this is the same as \p other.
  bool operator==(const image_format &other) const {
    return m_format.image_channel_order == other.m_format.image_channel_order &&
  ...
  bool operator!=(const image_format &other) const { return !(*this == other); }
