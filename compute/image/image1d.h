...
class image1d : public image_object {
 public:
  ...
  image1d() : image_object() {}

  /// Creates a new image1d object.
  ///
  /// \see_opencl_ref{clCreateImage}
  image1d(const context &context, size_t image_width,
  ...
  image1d(const image1d &other) : image_object(other) {}
  ...
  image1d(image1d &&other) BOOST_NOEXCEPT : image_object(std::move(other)) {}
  ...
  extents<1> origin() const { return extents<1>(); }
  ...
  /// \overload
  template <int Enum>
  typename detail::get_object_info_type<image1d, Enum>::type get_info() const;
...
#ifdef CL_VERSION_1_2
    return image_object::get_supported_formats(context, CL_MEM_OBJECT_IMAGE1D,
                                               flags);
#else
