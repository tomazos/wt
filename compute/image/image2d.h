...
class image2d : public image_object {
 public:
  ...
  image2d() : image_object() {}

  /// Creates a new image2d object.
  ///
  /// \see_opencl_ref{clCreateImage}
  image2d(const context &context, size_t image_width, size_t image_height,
...
#else
    m_mem =
        clCreateImage2D(context, flags, format.get_format_ptr(), image_width,
                        image_height, image_row_pitch, host_ptr, &error);
#endif
  ...
  image2d(const context &context, cl_mem_flags flags,
          const image_format &format, size_t image_width, size_t image_height,
          size_t image_row_pitch = 0, void *host_ptr = 0) {
    cl_int error = 0;
  ...
  image2d(const image2d &other) : image_object(other) {}
  ...
  image2d(image2d &&other) BOOST_NOEXCEPT : image_object(std::move(other)) {}
  ...
  extents<2> origin() const { return extents<2>(); }
  ...
  /// \overload
  template <int Enum>
  typename detail::get_object_info_type<image2d, Enum>::type get_info() const;
  ...
  static std::vector<image_format> get_supported_formats(
    ...
    return image_object::get_supported_formats(context, CL_MEM_OBJECT_IMAGE2D,
                                               flags);
  }
