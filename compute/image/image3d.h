...
#include "compute/exception/opencl_error.h"
#include "compute/image/image_format.h"
...
class image3d : public image_object {
 public:
  ...
  image3d() : image_object() {}

  /// Creates a new image3d object.
  ///
  /// \see_opencl_ref{clCreateImage}
  image3d(const context &context, size_t image_width, size_t image_height,
    ...
    m_mem = clCreateImage(context, flags, format.get_format_ptr(), &desc,
                          host_ptr, &error);
#else
    m_mem = clCreateImage3D(
        context, flags, format.get_format_ptr(), image_width, image_height,
        image_depth, image_row_pitch, image_slice_pitch, host_ptr, &error);
#endif
  ...
  image3d(const context &context, cl_mem_flags flags,
          const image_format &format, size_t image_width, size_t image_height,
          size_t image_depth, size_t image_row_pitch,
          size_t image_slice_pitch = 0, void *host_ptr = 0) {
    cl_int error = 0;
  ...
  image3d(const image3d &other) : image_object(other) {}
  ...
  image3d(image3d &&other) BOOST_NOEXCEPT : image_object(std::move(other)) {}
  ...
  extents<3> origin() const { return extents<3>(); }
  ...
  /// \overload
  template <int Enum>
  typename detail::get_object_info_type<image3d, Enum>::type get_info() const;

  /// Returns the supported 3D image formats for the context.
  ///
  /// \see_opencl_ref{clGetSupportedImageFormats}
  static std::vector<image_format> get_supported_formats(
      const context &context, cl_mem_flags flags = read_write) {
    return image_object::get_supported_formats(context, CL_MEM_OBJECT_IMAGE3D,
                                               flags);
  }
  ...
  static bool is_supported_format(const image_format &format,
    ...
    return image_object::is_supported_format(format, context,
                                             CL_MEM_OBJECT_IMAGE3D, flags);
  }
...
BOOST_COMPUTE_DETAIL_DEFINE_GET_INFO_SPECIALIZATIONS(
    image3d,
    ((cl_image_format, CL_IMAGE_FORMAT))((size_t, CL_IMAGE_ELEMENT_SIZE))(
        (size_t, CL_IMAGE_ROW_PITCH))((size_t, CL_IMAGE_SLICE_PITCH))(
        (size_t, CL_IMAGE_WIDTH))((size_t, CL_IMAGE_HEIGHT))((size_t,
                                                              CL_IMAGE_DEPTH)))
