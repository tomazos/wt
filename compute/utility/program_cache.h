...
#include "compute/detail/lru_cache.h"
...
class program_cache : boost::noncopyable {
 public:
  ...
  /// program objects.
  program_cache(size_t capacity) : m_cache(capacity) {}
  ...
  size_t capacity() const { return m_cache.capacity(); }
  ...
  void insert(const std::string &key, const std::string &options,
  ...
  /// return *p;
  /// \endcode
  program get_or_build(const std::string &key, const std::string &options,
  ...
  static boost::shared_ptr<program_cache> get_global_cache(
      const context &context) {
    typedef detail::lru_cache<cl_context, boost::shared_ptr<program_cache> >
        cache_map;

    BOOST_COMPUTE_DETAIL_GLOBAL_STATIC(cache_map, caches, (8));

    boost::optional<boost::shared_ptr<program_cache> > cache =
        caches.get(context.get());
    if (!cache) {
