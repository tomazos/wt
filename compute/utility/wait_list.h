...
class wait_list {
 public:
  ...
  wait_list(const event &event) { insert(event); }

  /// Creates a new wait-list as a copy of \p other.
  wait_list(const wait_list &other) : m_events(other.m_events) {}
  ...
  wait_list(wait_list &&other) : m_events(std::move(other.m_events)) {}
  ...
  uint_ size() const { return m_events.size(); }
  ...
  void insert(const event &event) { m_events.push_back(event); }
  ...
  void wait() {
    if (!empty()) {
      BOOST_COMPUTE_ASSERT_CL_SUCCESS(clWaitForEvents(size(), get_event_ptr()));
    }
  }
