...
#define BOOST_COMPUTE_DETAIL_DEFINE_INVOKE(z, n, unused)                     \
  ...
  invoke(const Function& function, command_queue& queue,                     \
    ...
    typedef typename result_of<Function(BOOST_PP_ENUM_PARAMS(n, T))>::type   \
        result_type;                                                         \
    detail::meta_kernel k("invoke");                                         \
    detail::scalar<result_type> result(queue.get_context());                 \
    const size_t result_arg =                                                \
        k.add_arg<result_type*>(memory_object::global_memory, "result");     \
    BOOST_PP_REPEAT(n, BOOST_COMPUTE_DETAIL_INVOKE_ADD_ARG, ~)               \
    k << "*result = " << function(BOOST_PP_REPEAT(                           \
                             n, BOOST_COMPUTE_DETAIL_INVOKE_ARG, ~)) << ";"; \
...
BOOST_PP_REPEAT_FROM_TO(1, BOOST_COMPUTE_MAX_ARITY,
                        BOOST_COMPUTE_DETAIL_DEFINE_INVOKE, ~)
...
inline typename result_of<Function(Args...)>::type invoke(
    const Function& function, command_queue& queue, const Args&... args);
