...
class platform {
 public:
  ...
  explicit platform(cl_platform_id id) : m_platform(id) {}

  /// Creates a new platform as a copy of \p other.
  platform(const platform &other) : m_platform(other.m_platform) {}
  ...
  cl_platform_id id() const { return m_platform; }
  ...
  std::vector<device> devices(cl_device_type type = CL_DEVICE_TYPE_ALL) const {
    ...
    cl_int ret = clGetDeviceIDs(m_platform, type, static_cast<cl_uint>(count),
                                &device_ids[0], 0);
    if (ret != CL_SUCCESS) {
  ...
  /// \overload
  template <int Enum>
  typename detail::get_object_info_type<platform, Enum>::type get_info() const;
  ...
  void *get_extension_function_address(const char *function_name) const {
    ...
    return clGetExtensionFunctionAddressForPlatform(m_platform, function_name);
#else
...
BOOST_COMPUTE_DETAIL_DEFINE_GET_INFO_SPECIALIZATIONS(
    platform,
    ((std::string, CL_PLATFORM_PROFILE))((std::string, CL_PLATFORM_VERSION))(
        (std::string, CL_PLATFORM_NAME))((std::string, CL_PLATFORM_VENDOR))(
