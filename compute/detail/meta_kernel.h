...
#include "compute/closure.h"
#include "compute/function.h"
#include "compute/functional.h"
#include "compute/type_traits.h"
#include "compute/command_queue.h"
#include "compute/image/image2d.h"
#include "compute/image/image_sampler.h"
#include "compute/memory_object.h"
#include "compute/detail/device_ptr.h"
#include "compute/detail/sha1.h"
#include "compute/utility/program_cache.h"
...
class meta_kernel_variable {
 public:
  ...
  meta_kernel_variable(const std::string &name) : m_name(name) {}
...
class meta_kernel_literal {
 public:
  ...
  meta_kernel_literal(const T &value) : m_value(value) {}
...
struct meta_kernel_stored_arg {
  meta_kernel_stored_arg() : m_size(0), m_value(0) {}
...
struct meta_kernel_buffer_info {
  meta_kernel_buffer_info(const buffer &buffer, const std::string &id,
                          memory_object::address_space addr_space, size_t i)
      : m_mem(buffer.get()),
        identifier(id),
...
#define BOOST_COMPUTE_META_KERNEL_DECLARE_TYPE_STREAM_OPERATORS(type) \
  BOOST_COMPUTE_META_KERNEL_DECLARE_SCALAR_TYPE_STREAM_OPERATOR(      \
      BOOST_PP_CAT(type, _))                                          \
  BOOST_COMPUTE_META_KERNEL_DECLARE_VECTOR_TYPE_STREAM_OPERATOR(      \
      BOOST_PP_CAT(BOOST_PP_CAT(type, 2), _))                         \
  BOOST_COMPUTE_META_KERNEL_DECLARE_VECTOR_TYPE_STREAM_OPERATOR(      \
      BOOST_PP_CAT(BOOST_PP_CAT(type, 4), _))                         \
  BOOST_COMPUTE_META_KERNEL_DECLARE_VECTOR_TYPE_STREAM_OPERATOR(      \
      BOOST_PP_CAT(BOOST_PP_CAT(type, 8), _))                         \
  BOOST_COMPUTE_META_KERNEL_DECLARE_VECTOR_TYPE_STREAM_OPERATOR(      \
      BOOST_PP_CAT(BOOST_PP_CAT(type, 16), _))
...
class meta_kernel {
 public:
  ...
  explicit meta_kernel(const std::string &name) : m_name(name) {}

  meta_kernel(const meta_kernel &other) { m_source.str(other.m_source.str()); }
  ...
  std::string source() const {
    ...
    stream << "#define boost_make_pair(t1, x, t2, y) (boost_pair_type(t1, t2)) "
              "{ x, y }\n";
    ...
    // add kernel source
    stream << "__kernel void " << m_name << "(" << boost::join(m_args, ", ")
           << ")\n"
           << "{\n" << m_source.str() << "\n}\n";
  ...
  kernel compile(const context &context,
    ...
    std::string cache_key =
        "__boost_meta_kernel_" + static_cast<std::string>(detail::sha1(source));
  ...
  size_t add_arg(memory_object::address_space address_space,
    ...
    return add_arg_with_qualifiers<T>(address_space_prefix(address_space),
                                      name);
  }
  ...
  void add_extension_pragma(const std::string &extension,
    ...
    return const_cast<meta_kernel *>(this)
        ->add_extension_pragma(extension, value);
  }
  ...
  event exec_1d(command_queue &queue, size_t global_work_offset,
                size_t global_work_size) {
    const context &context = queue.get_context();

    ::boost::compute::kernel kernel = compile(context);

    return queue.enqueue_1d_range_kernel(kernel, global_work_offset,
                                         global_work_size, 0);
  }
  ...
  template <class T>
  std::string get_buffer_identifier(
      const buffer &buffer, const memory_object::address_space address_space =
                                memory_object::global_memory) {
    // check if we've already seen buffer
    for (size_t i = 0; i < m_stored_buffers.size(); i++) {
      const detail::meta_kernel_buffer_info &bi = m_stored_buffers[i];

      if (bi.m_mem == buffer.get() && bi.address_space == address_space) {
    ...
    m_stored_buffers.push_back(detail::meta_kernel_buffer_info(
        buffer, identifier, address_space, index));

    return identifier;
  }

  std::string get_image_identifier(const char *qualifiers,
                                   const image2d &image) {
  ...
  detail::invoked_function<bool, boost::tuple<Predicate> > if_(
  ...
  template <class Predicate>
  detail::invoked_function<bool, boost::tuple<Predicate> > else_if_(
      Predicate pred) const {
    return detail::invoked_function<bool, boost::tuple<Predicate> >(
...
#define BOOST_COMPUTE_META_KERNEL_INSERT_FUNCTION_ARGS(z, n, unused) \
  ...
  void insert_function_call_args(                                    \
    ...
    *this BOOST_PP_REPEAT_FROM_TO(                                   \
        1, n, BOOST_COMPUTE_META_KERNEL_STREAM_FUNCTION_ARG, ~)      \
        << boost::get<BOOST_PP_DEC(n)>(args);                        \
  }

  BOOST_PP_REPEAT_FROM_TO(1, BOOST_COMPUTE_MAX_ARITY,
                          BOOST_COMPUTE_META_KERNEL_INSERT_FUNCTION_ARGS, ~)
...
#undef BOOST_COMPUTE_META_KERNEL_INSERT_FUNCTION_ARGS
  ...
  static const char *address_space_prefix(
      const memory_object::address_space value) {
    switch (value) {
      case memory_object::global_memory:
        return "__global";
      case memory_object::local_memory:
        return "__local";
      case memory_object::private_memory:
        return "__private";
      case memory_object::constant_memory:
        return "__constant";
    };
 ...
 private:
  template <class T>
  size_t add_arg_with_qualifiers(const char *qualifiers,
                                 const std::string &name) {
    size_t index = add_arg<T>(name);
...
inline meta_kernel &operator<<(
    meta_kernel &kernel, const invoked_function<ResultType, ArgTuple> &expr) {
...
inline meta_kernel &operator<<(
    meta_kernel &kernel,
    const invoked_binary_operator<Arg1, Arg2, Result> &expr) {
  return kernel << "((" << expr.arg1() << ")" << expr.op() << "(" << expr.arg2()
                << "))";
}
...
inline meta_kernel &operator<<(
    meta_kernel &kernel,
    const invoked_unary_negate_function<Predicate, Arg> &expr) {
...
inline meta_kernel &operator<<(
    meta_kernel &kernel,
    const invoked_binary_negate_function<Predicate, Arg1, Arg2> &expr) {
...
template <class T, class Arg>
inline meta_kernel &operator<<(meta_kernel &k, const invoked_as<T, Arg> &expr) {
...
struct inject_type_impl<vector_type<Scalar, N> > {
  void operator()(meta_kernel &kernel) { kernel.inject_type<Scalar>(); }
};
