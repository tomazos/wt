...
#include "compute/detail/global_static.h"
...
class parameter_cache : boost::noncopyable {
 public:
  parameter_cache(const device &device)
      : m_dirty(false), m_device_name(device.name()) {
#ifdef BOOST_COMPUTE_USE_OFFLINE_CACHE
    // get offline cache file name (e.g.
    // /home/user/.boost_compute/tune/device.json)
  ...
  void set(const std::string &object, const std::string &parameter,
           uint_ value) {
  ...
  uint_ get(const std::string &object, const std::string &parameter,
            uint_ default_value) {
    std::map<std::pair<std::string, std::string>, uint_>::iterator iter =
        m_cache.find(std::make_pair(object, parameter));
    if (iter != m_cache.end()) {
      return iter->second;
    } else {
  ...
  static boost::shared_ptr<parameter_cache> get_global_cache(
      const device &device) {
    // device name -> parameter cache
    typedef std::map<std::string, boost::shared_ptr<parameter_cache> >
        cache_map;

    BOOST_COMPUTE_DETAIL_GLOBAL_STATIC(cache_map, caches,
                                       ((std::less<std::string>())));
 ...
 private:
  ...
  static std::string version_string() {
    ...
    std::snprintf(buf, sizeof(buf), "%d.%d.%d", BOOST_COMPUTE_VERSION_MAJOR,
                  BOOST_COMPUTE_VERSION_MINOR, BOOST_COMPUTE_VERSION_PATCH);
    return buf;
  }

  // returns the file path for the cached parameters
  std::string make_file_name() const {
    return detail::parameter_cache_path(true) +
           cannonical_device_name(m_device_name) + ".json";
  }
  ...
  void write_to_disk() {
    ...
    if (m_dirty) {
      ...
      for (map_type::const_iterator iter = m_cache.begin();
  ...
  void read_from_disk() {
    ...
    if (stored_device == m_device_name && stored_version == version_string()) {
      ...
      for (pt_iter iter = pt.begin(); iter != pt.end(); ++iter) {
        ...
        for (pt_iter child_iter = child_pt.begin();
             child_iter != child_pt.end(); ++child_iter) {
          set(iter->first, child_iter->first,
              boost::lexical_cast<uint_>(child_iter->second.data()));
        }
      }
    }
