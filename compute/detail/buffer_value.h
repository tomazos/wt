...
class buffer_value {
 public:
  ...
  buffer_value(const value_type &value) : m_value(value) {}
  ...
  buffer_value<T> operator-() const { return -T(*this); }
  ...
  buffer_value<T> &operator=(const T &value) {
    if (m_buffer.get()) {
      ...
      detail::write_single_value<T>(value, m_buffer, m_index / sizeof(T),
                                    queue);
