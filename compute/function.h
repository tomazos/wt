...
class invoked_function {
 public:
  ...
  invoked_function(const std::string &name, const std::string &source,
      ...
      : m_name(name), m_source(source), m_definitions(definitions) {}

  invoked_function(const std::string &name, const std::string &source,
                   const ArgTuple &args)
      : m_name(name), m_source(source), m_args(args) {}
...
class function {
 public:
  ...
  /// \internal_
  detail::invoked_function<result_type, boost::tuple<> > operator()() const {
    BOOST_STATIC_ASSERT_MSG(arity == 0,
  ...
  /// \internal_
  template <class Arg1>
  detail::invoked_function<result_type, boost::tuple<Arg1> > operator()(
      const Arg1 &arg1) const {
    BOOST_STATIC_ASSERT_MSG(arity == 1,
  ...
  detail::invoked_function<result_type, boost::tuple<Arg1, Arg2> > operator()(
      const Arg1 &arg1, const Arg2 &arg2) const {
    BOOST_STATIC_ASSERT_MSG(arity == 2,
  ...
  operator()(const Arg1 &arg1, const Arg2 &arg2, const Arg3 &arg3) const {
    BOOST_STATIC_ASSERT_MSG(arity == 3,
                            "Non-ternary function invoked with two arguments");

    return detail::invoked_function<result_type,
...
template <class Signature>
inline function<Signature> make_function_from_source(
...
inline std::vector<std::string> parse_argument_names(const char *arguments) {
  ...
  for (size_t i = 1; i < std::strlen(arguments) - 2; i++) {
    ...
    if (c == ' ') {
      ...
      std::string name(arguments + last_space + 1, i - last_space - 1);
      args.push_back(name);
    } else if (c == '<') {
      skip_comma++;
    } else if (c == '>') {
  ...
  std::string last_argument(arguments + last_space + 1,
                            std::strlen(arguments) - last_space - 2);
...
struct signature_argument_inserter {
  signature_argument_inserter(std::stringstream &s_, const char *arguments,
                              size_t last)
      : s(s_) {
    n = 0;
    m_last = last;
    ...
    BOOST_ASSERT_MSG(m_argument_names.size() == last,
...
template <class Signature>
inline std::string make_function_declaration(const char *name,
...
template <class Signature>
inline function<Signature> make_function_impl(const char *name,
                                              const char *arguments,
                                              const char *source) {
  std::stringstream s;
