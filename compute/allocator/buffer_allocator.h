...
class buffer_allocator {
 public:
  ...
  explicit buffer_allocator(const context& context)
      : m_context(context), m_mem_flags(buffer::read_write) {}

  buffer_allocator(const buffer_allocator<T>& other)
      : m_context(other.m_context), m_mem_flags(other.m_mem_flags) {}
 ...
 protected:
  void set_mem_flags(cl_mem_flags flags) { m_mem_flags = flags; }
