...
#include "compute/lambda/result_of.h"
#include "compute/lambda/functional.h"
...
#define BOOST_COMPUTE_LAMBDA_CONTEXT_FUNCTION(z, n, unused)               \
  ...
  void operator()(                                                        \
      proto::tag::function, const F &function,                            \
  ...
  BOOST_PP_REPEAT_FROM_TO(1, BOOST_COMPUTE_MAX_ARITY,
                          BOOST_COMPUTE_LAMBDA_CONTEXT_FUNCTION, ~)
...
#undef BOOST_COMPUTE_LAMBDA_CONTEXT_FUNCTION
  ...
  void operator()(proto::tag::if_else_, const Pred &p, const Arg1 &x,
                  const Arg2 &y) {
    proto::eval(p, *this);
    stream << '?';
    proto::eval(x, *this);
...
boost::compute::detail::meta_kernel &operator<<(
    boost::compute::detail::meta_kernel &kernel,
...
struct invoked_binary_expression {
  typedef
      typename ::boost::compute::result_of<Expr(Arg1, Arg2)>::type result_type;

  invoked_binary_expression(const Expr &expr, const Arg1 &arg1,
                            const Arg2 &arg2)
      : m_expr(expr), m_arg1(arg1), m_arg2(arg2) {}
...
boost::compute::detail::meta_kernel &operator<<(
  ...
  context<boost::tuple<Arg1, Arg2> > ctx(
      kernel, boost::make_tuple(expr.m_arg1, expr.m_arg2));
  proto::eval(expr.m_expr, ctx);
...
struct expression : proto::extends<Expr, expression<Expr>, domain> {
  ...
  expression(const Expr &expr = Expr()) : base_type(expr) {}
  ...
  template <class This>
  struct result<This()> {
    typedef typename ::boost::compute::lambda::result_of<Expr>::type type;
  };

  template <class This, class Arg>
  struct result<This(Arg)> {
    typedef typename ::boost::compute::lambda::result_of<
        Expr, typename boost::tuple<Arg> >::type type;
  };

  template <class This, class Arg1, class Arg2>
  struct result<This(Arg1, Arg2)> {
    typedef typename ::boost::compute::lambda::result_of<
        Expr, typename boost::tuple<Arg1, Arg2> >::type type;
  };

  template <class Arg>
  detail::invoked_unary_expression<expression<Expr>, Arg> operator()(
      const Arg &x) const {
    return detail::invoked_unary_expression<expression<Expr>, Arg>(*this, x);
  }

  template <class Arg1, class Arg2>
  detail::invoked_binary_expression<expression<Expr>, Arg1, Arg2> operator()(
      const Arg1 &x, const Arg2 &y) const {
    return detail::invoked_binary_expression<expression<Expr>, Arg1, Arg2>(
        *this, x, y);
  }
