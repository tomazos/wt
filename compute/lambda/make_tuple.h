...
#define BOOST_COMPUTE_MAKE_TUPLE_RESULT_GET_ARG(z, n, unused)             \
  typedef typename proto::result_of::child_c<Expr, BOOST_PP_INC(n)>::type \
      BOOST_PP_CAT(Arg, n);

#define BOOST_COMPUTE_MAKE_TUPLE_RESULT_GET_ARG_TYPE(z, n, unused)     \
  typedef typename lambda::result_of<BOOST_PP_CAT(Arg, n), Args>::type \
      BOOST_PP_CAT(T, n);
...
#define BOOST_COMPUTE_MAKE_TUPLE_RESULT_TYPE(z, n, unused)              \
  ...
  BOOST_PP_REPEAT_FROM_TO(1, BOOST_COMPUTE_MAX_ARITY,
                          BOOST_COMPUTE_MAKE_TUPLE_RESULT_TYPE, ~)
...
#undef BOOST_COMPUTE_MAKE_TUPLE_RESULT_TYPE
  ...
  struct lambda_result {
    typedef typename make_tuple_result_type<
        Expr, Args, proto::arity_of<Expr>::value - 1>::type type;
  };

#define BOOST_COMPUTE_MAKE_TUPLE_GET_ARG_TYPE(z, n, unused)              \
  typedef typename lambda::result_of<BOOST_PP_CAT(Arg, n),               \
                                     typename Context::args_tuple>::type \
      BOOST_PP_CAT(T, n);
...
#define BOOST_COMPUTE_LAMBDA_MAKE_TUPLE_APPLY_EVAL_ARG(z, n, unused) \
  BOOST_PP_EXPR_IF(n, ctx.stream << ", ";)                           \
  proto::eval(BOOST_PP_CAT(arg, n), ctx);

#define BOOST_COMPUTE_MAKE_TUPLE_APPLY(z, n, unused)                      \
  template <class Context, BOOST_PP_ENUM_PARAMS(n, class Arg)>            \
  static void apply(                                                      \
      Context &ctx,                                                       \
      BOOST_PP_REPEAT(n, BOOST_COMPUTE_LAMBDA_MAKE_TUPLE_APPLY_ARG, ~)) { \
  ...
  BOOST_PP_REPEAT_FROM_TO(1, BOOST_COMPUTE_MAX_ARITY,
                          BOOST_COMPUTE_MAKE_TUPLE_APPLY, ~)
...
#define BOOST_COMPUTE_LAMBDA_MAKE_TUPLE(z, n, unused)                          \
  ...
  inline typename proto::result_of::make_expr<                                 \
      proto::tag::function, detail::make_tuple_func,                           \
      BOOST_PP_REPEAT(n, BOOST_COMPUTE_LAMBDA_MAKE_TUPLE_ARG_TYPE, ~)>::type   \
...
BOOST_PP_REPEAT_FROM_TO(1, BOOST_COMPUTE_MAX_ARITY,
                        BOOST_COMPUTE_LAMBDA_MAKE_TUPLE, ~)
