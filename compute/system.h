...
#include "compute/exception/no_device_found.h"
...
class system {
 public:
  ...
  /// \endcode
  static void finish() { default_queue().finish(); }
  ...
  static device find_default_device() {
    ...
    if (name || type || platform || vendor) {
      for (size_t i = 0; i < devices_.size(); i++) {
        ...
        if (name && !matches(device.name(), name)) continue;

        if (type && matches(std::string("GPU"), type))
          if (!(device.type() & device::gpu)) continue;

        if (type && matches(std::string("CPU"), type))
          if (!(device.type() & device::cpu)) continue;

        if (platform && !matches(device.platform().name(), platform)) continue;

        if (vendor && !matches(device.vendor(), vendor)) continue;
