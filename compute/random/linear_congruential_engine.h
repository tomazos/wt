...
class linear_congruential_engine {
 public:
  ...
  linear_congruential_engine<T> &operator=(
      const linear_congruential_engine<T> &other) {
    if (this != &other) {
      m_context = other.m_context;
    }
  ...
  /// \overload
  void seed(command_queue &queue) { seed(default_seed); }
  ...
  /// \internal_
  void generate(discard_iterator first, discard_iterator last,
                command_queue &queue) {
    (void)queue;
    ...
    m_seed *= detail::read_single_value<T>(m_multiplicands, size - 1, queue);
  }
 ...
 private:
  ...
  void generate_multiplicands(command_queue &queue) {
    kernel multiplicand_kernel = m_program.create_kernel("multiplicand");
  ...
  void update_seed(command_queue &queue) {
    m_seed *= detail::read_single_value<T>(m_multiplicands, threads - 1, queue);
  }
