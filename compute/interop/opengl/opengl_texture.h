...
#include "compute/interop/opengl/gl.h"
...
class opengl_texture : public image_object {
 public:
  ...
  opengl_texture(const context &context, GLenum texture_target, GLint miplevel,
    ...
    m_mem = clCreateFromGLTexture(context, flags, texture_target, miplevel,
                                  texture, &error);
#else
    m_mem = clCreateFromGLTexture2D(context, flags, texture_target, miplevel,
                                    texture, &error);
#endif
  ...
  opengl_texture(const opengl_texture &other) : image_object(other) {}
