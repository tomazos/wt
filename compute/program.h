...
#include "compute/detail/getenv.h"
#include "compute/detail/path.h"
...
///     boost::compute::program::create_with_source_file("/path/to/bar.cl",
///     context);
/// \endcode
///
...
class program {
 public:
  ...
  program(const program &other) : m_program(other.m_program) {
    if (m_program) {
      clRetainProgram(m_program);
    }
  }
  ...
  program(program &&other) BOOST_NOEXCEPT : m_program(other.m_program) {
    other.m_program = 0;
  }
  ...
  ~program() {
    if (m_program) {
      BOOST_COMPUTE_ASSERT_CL_SUCCESS(clReleaseProgram(m_program));
    }
  }

  /// Returns the underlying OpenCL program.
  cl_program &get() const { return const_cast<cl_program &>(m_program); }
  ...
  std::vector<unsigned char> binary() const {
    ...
    cl_int error = clGetProgramInfo(m_program, CL_PROGRAM_BINARIES,
                                    sizeof(unsigned char **), &binary_ptr, 0);
    if (error != CL_SUCCESS) {
  ...
  /// \overload
  template <int Enum>
  typename detail::get_object_info_type<program, Enum>::type get_info() const;
  ...
  T get_build_info(cl_program_build_info info, const device &device) const {
    return detail::get_object_info<T>(clGetProgramBuildInfo, m_program, info,
                                      device.id());
  }
  ...
  void compile(const std::string &options = std::string()) {
    ...
    cl_int ret =
        clCompileProgram(m_program, 0, 0, options_string, 0, 0, 0, 0, 0);
  ...
  static program link(const std::vector<program> &programs,
    ...
    cl_program program_ = clLinkProgram(
        context.get(), 0, 0, options_string,
        static_cast<uint_>(programs.size()),
        reinterpret_cast<const cl_program *>(&programs[0]), 0, 0, &ret);
  ...
  std::string build_log() const {
    return get_build_info<std::string>(CL_PROGRAM_BUILD_LOG,
                                       get_devices().front());
  }
  ...
  static program create_with_source(const std::string &source,
    ...
    cl_int error = 0;
    cl_program program_ =
        clCreateProgramWithSource(context, uint_(1), &source_string, 0, &error);
    if (!program_) {
      BOOST_THROW_EXCEPTION(opencl_error(error));
    }
  ...
  static program create_with_binary(const unsigned char *binary,
    ...
    cl_program program_ =
        clCreateProgramWithBinary(context, uint_(1), &device, &binary_size,
                                  &binary, &binary_status, &error);
    if (!program_) {
      BOOST_THROW_EXCEPTION(opencl_error(error));
    }
  ...
  static program create_with_binary_file(const std::string &file,
    ...
    // read binary
    std::vector<unsigned char> binary((std::istreambuf_iterator<char>(stream)),
                                      std::istreambuf_iterator<char>());
  ...
  static program create_with_builtin_kernels(const context &context,
    ...
    cl_program program_ = clCreateProgramWithBuiltInKernels(
        context.get(), static_cast<uint_>(devices.size()),
        reinterpret_cast<const cl_device_id *>(&devices[0]),
        kernel_names.c_str(), &error);
  ...
  /// Create a new program with \p source in \p context and builds it with \p
  /// options.
  /**
   ...
   * on Windows.
   */
  static program build_with_source(const std::string &source,
                                   const context &context,
                                   const std::string &options = std::string()) {
#ifdef BOOST_COMPUTE_USE_OFFLINE_CACHE
...
// Cache is apparently not available. Just compile the sources.
 ...
 private:
  ...
  static void save_program_binary(const std::string &hash,
                                  const program &prog) {
  ...
  static boost::optional<program> load_program_binary(const std::string &hash,
                                                      const context &ctx) {
    ...
    return boost::optional<program>(
        program::create_with_binary(binary.data(), binary_size, ctx));
  }
...
BOOST_COMPUTE_DETAIL_DEFINE_GET_INFO_SPECIALIZATIONS(
    program,
    ((cl_uint, CL_PROGRAM_REFERENCE_COUNT))((cl_context, CL_PROGRAM_CONTEXT))(
        (cl_uint, CL_PROGRAM_NUM_DEVICES))((std::vector<cl_device_id>,
                                            CL_PROGRAM_DEVICES))(
        (std::string, CL_PROGRAM_SOURCE))((std::vector<size_t>,
...
BOOST_COMPUTE_DETAIL_DEFINE_GET_INFO_SPECIALIZATIONS(
    program,
    ((size_t, CL_PROGRAM_NUM_KERNELS))((std::string, CL_PROGRAM_KERNEL_NAMES)))
#endif  // CL_VERSION_1_2
