...
class memory_object {
 public:
  ...
  cl_mem &get() const { return const_cast<cl_mem &>(m_mem); }
  ...
  void set_destructor_callback(
      void(BOOST_COMPUTE_CL_CALLBACK *callback)(cl_mem memobj, void *user_data),
      void *user_data = 0) {
  ...
  void set_destructor_callback(Function callback) {
    set_destructor_callback(destructor_callback_invoker,
 ...
 protected:
  /// \internal_
  memory_object() : m_mem(0) {}

  /// \internal_
  explicit memory_object(cl_mem mem, bool retain = true) : m_mem(mem) {
    if (m_mem && retain) {
  ...
  /// \internal_
  memory_object(const memory_object &other) : m_mem(other.m_mem) {
    if (m_mem) {
      clRetainMemObject(m_mem);
    }
  }
  ...
  /// \internal_
  memory_object(memory_object &&other) BOOST_NOEXCEPT : m_mem(other.m_mem) {
    other.m_mem = 0;
  }
  ...
  ~memory_object() {
    if (m_mem) {
      BOOST_COMPUTE_ASSERT_CL_SUCCESS(clReleaseMemObject(m_mem));
    }
  }
