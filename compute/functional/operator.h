...
struct invoked_binary_operator {
  ...
  invoked_binary_operator(const std::string &op, const Expr1 &arg1,
                          const Expr2 &arg2)
      : m_op(op), m_expr1(arg1), m_expr2(arg2) {}

  std::string op() const { return m_op; }

  Expr1 arg1() const { return m_expr1; }

  Expr2 arg2() const { return m_expr2; }
...
#define BOOST_COMPUTE_DECLARE_BINARY_OPERATOR(name, op, return_type, arg_type) \
  ...
  class name : public function<return_type(arg_type, arg_type)> {              \
   public:                                                                     \
    name()                                                                     \
        : function<return_type(arg_type, arg_type)>(                           \
              BOOST_PP_STRINGIZE(name)) {}                                     \
                                                                               \
    template <class Arg1, class Arg2>                                          \
    detail::invoked_binary_operator<Arg1, Arg2, T> operator()(                 \
