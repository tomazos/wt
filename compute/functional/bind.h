...
struct invoked_bound_function {
  invoked_bound_function(Function f, BoundArgs bound_args, Args args)
      : m_function(f), m_bound_args(bound_args), m_args(args) {}
  ...
  typename get_nth_arg_type<N>::type get_nth_arg(
      typename boost::enable_if_c<is_placeholder_arg<N>::value>::type * =
          0) const {
    typedef typename boost::tuples::element<N, BoundArgs>::type nth_bound_arg;
  ...
  template <int N>
  typename get_nth_arg_type<N>::type get_nth_arg(
      typename boost::disable_if_c<is_placeholder_arg<N>::value>::type * =
          0) const {
...
inline meta_kernel &apply_invoked_bound_function(
    ...
    typename boost::enable_if_c<boost::tuples::length<BoundArgs>::value ==
                                1>::type * = 0) {
...
struct bound_function {
  ...
  bound_function(Function f, BoundArgs args) : m_function(f), m_args(args) {}

  template <class Arg1>
  detail::invoked_bound_function<Function, BoundArgs, boost::tuple<Arg1> >
  operator()(const Arg1 &arg1) const {
    return detail::invoked_bound_function<Function, BoundArgs,
                                          boost::tuple<Arg1> >(
        m_function, m_args, boost::make_tuple(arg1));
  }

  template <class Arg1, class Arg2>
  detail::invoked_bound_function<Function, BoundArgs, boost::tuple<Arg1, Arg2> >
...
#if !defined(BOOST_COMPUTE_NO_VARIADIC_TEMPLATES) || \
...
inline detail::bound_function<F, boost::tuple<Args...> > bind(F f,
                                                              Args... args) {
...
inline detail::bound_function<F, boost::tuple<A1> > bind(F f, A1 a1) {
...
inline detail::bound_function<F, boost::tuple<A1, A2> > bind(F f, A1 a1,
                                                             A2 a2) {
...
inline detail::bound_function<F, boost::tuple<A1, A2, A3> > bind(F f, A1 a1,
                                                                 A2 a2, A3 a3) {
