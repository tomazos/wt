...
#include "compute/event.h"
#include "compute/buffer.h"
#include "compute/device.h"
#include "compute/kernel.h"
#include "compute/context.h"
#include "compute/exception.h"
#include "compute/image/image1d.h"
#include "compute/image/image2d.h"
#include "compute/image/image3d.h"
#include "compute/image/image_object.h"
#include "compute/utility/wait_list.h"
#include "compute/detail/get_object_info.h"
#include "compute/detail/assert_cl_success.h"
#include "compute/utility/extents.h"
...
class command_queue {
 public:
...
#endif
    {
      m_queue = clCreateCommandQueue(context, device.id(), properties, &error);
    }
  ...
  command_queue(const command_queue &other) : m_queue(other.m_queue) {
    if (m_queue) {
      clRetainCommandQueue(m_queue);
    }
  }
  ...
  command_queue(command_queue &&other) BOOST_NOEXCEPT : m_queue(other.m_queue) {
    other.m_queue = 0;
  }
  ...
  ~command_queue() {
    if (m_queue) {
      BOOST_COMPUTE_ASSERT_CL_SUCCESS(clReleaseCommandQueue(m_queue));
    }
  }
  ...
  /// \overload
  template <int Enum>
  typename detail::get_object_info_type<command_queue, Enum>::type get_info()
      const;
  ...
  ///
  /// \see copy()
  event enqueue_read_buffer(const buffer &buffer, size_t offset, size_t size,
                            void *host_ptr,
    ...
    cl_int ret = clEnqueueReadBuffer(m_queue, buffer.get(), CL_TRUE, offset,
                                     size, host_ptr, events.size(),
  ...
  event enqueue_read_buffer_async(const buffer &buffer, size_t offset,
    ...
    cl_int ret = clEnqueueReadBuffer(m_queue, buffer.get(), CL_FALSE, offset,
                                     size, host_ptr, events.size(),
                                     events.get_event_ptr(), &event_.get());
  ...
  event enqueue_read_buffer_rect(
      const buffer &buffer, const size_t buffer_origin[3],
    ...
    cl_int ret = clEnqueueReadBufferRect(
        m_queue, buffer.get(), CL_TRUE, buffer_origin, host_origin, region,
        buffer_row_pitch, buffer_slice_pitch, host_row_pitch, host_slice_pitch,
        host_ptr, events.size(), events.get_event_ptr(), &event_.get());
  ...
  ///
  /// \see copy()
  event enqueue_write_buffer(const buffer &buffer, size_t offset, size_t size,
                             const void *host_ptr,
                             const wait_list &events = wait_list()) {
    BOOST_ASSERT(m_queue != 0);
    BOOST_ASSERT(size <= buffer.size());
    ...
    cl_int ret = clEnqueueWriteBuffer(m_queue, buffer.get(), CL_TRUE, offset,
                                      size, host_ptr, events.size(),
                                      events.get_event_ptr(), &event_.get());
  ...
  event enqueue_write_buffer_async(const buffer &buffer, size_t offset,
    ...
    cl_int ret = clEnqueueWriteBuffer(m_queue, buffer.get(), CL_FALSE, offset,
                                      size, host_ptr, events.size(),
                                      events.get_event_ptr(), &event_.get());
  ...
  ///
  /// \see copy()
  event enqueue_copy_buffer(const buffer &src_buffer, const buffer &dst_buffer,
    ...
    cl_int ret = clEnqueueCopyBuffer(
        m_queue, src_buffer.get(), dst_buffer.get(), src_offset, dst_offset,
        size, events.size(), events.get_event_ptr(), &event_.get());
  ...
  event enqueue_copy_buffer_rect(
    ...
    cl_int ret = clEnqueueCopyBufferRect(
        m_queue, src_buffer.get(), dst_buffer.get(), src_origin, dst_origin,
        region, buffer_row_pitch, buffer_slice_pitch, host_row_pitch,
        host_slice_pitch, events.size(), events.get_event_ptr(), &event_.get());
  ...
  event enqueue_fill_buffer(const buffer &buffer, const void *pattern,
    ...
    cl_int ret = clEnqueueFillBuffer(m_queue, buffer.get(), pattern,
                                     pattern_size, offset, size, events.size(),
                                     events.get_event_ptr(), &event_.get());
  ...
  void *enqueue_map_buffer(const buffer &buffer, cl_map_flags flags,
    ...
    cl_int ret = 0;
    void *pointer =
        clEnqueueMapBuffer(m_queue, buffer.get(), CL_TRUE, flags, offset, size,
                           events.size(), events.get_event_ptr(), 0, &ret);
  ...
  event enqueue_unmap_buffer(const buffer &buffer, void *mapped_ptr,
                             const wait_list &events = wait_list()) {
    BOOST_ASSERT(buffer.get_context() == this->get_context());
  ...
  ///
  /// \see_opencl_ref{clEnqueueUnmapMemObject}
  event enqueue_unmap_mem_object(cl_mem mem, void *mapped_ptr,
                                 const wait_list &events = wait_list()) {
    BOOST_ASSERT(m_queue != 0);
    ...
    cl_int ret =
        clEnqueueUnmapMemObject(m_queue, mem, mapped_ptr, events.size(),
                                events.get_event_ptr(), &event_.get());
  ...
  event enqueue_read_image(const image_object &image, const size_t *origin,
    ...
    cl_int ret = clEnqueueReadImage(
        m_queue, image.get(), CL_TRUE, origin, region, row_pitch, slice_pitch,
        host_ptr, events.size(), events.get_event_ptr(), &event_.get());
  ...
  /// \overload
  template <size_t N>
  event enqueue_read_image(const image_object &image, const extents<N> origin,
    ...
    return enqueue_read_image(image, origin3, region3, row_pitch, slice_pitch,
                              host_ptr, events);
  }
  ...
  event enqueue_write_image(image_object &image, const size_t *origin,
    ...
    cl_int ret = clEnqueueWriteImage(m_queue, image.get(), CL_TRUE, origin,
                                     region, input_row_pitch, input_slice_pitch,
                                     host_ptr, events.size(),
                                     events.get_event_ptr(), &event_.get());
  ...
  event enqueue_write_image(image_object &image, const extents<N> origin,
    ...
    return enqueue_write_image(image, origin3, region3, host_ptr,
                               input_row_pitch, input_slice_pitch, events);
  }
  ...
  event enqueue_copy_image(const image_object &src_image,
    ...
    cl_int ret = clEnqueueCopyImage(
        m_queue, src_image.get(), dst_image.get(), src_origin, dst_origin,
        region, events.size(), events.get_event_ptr(), &event_.get());
  ...
  template <size_t N>
  event enqueue_copy_image(const image_object &src_image,
                           image_object &dst_image, const extents<N> src_origin,
                           const extents<N> dst_origin, const extents<N> region,
                           const wait_list &events = wait_list()) {
    ...
    return enqueue_copy_image(src_image, dst_image, src_origin3, dst_origin3,
                              region3, events);
  }
  ...
  event enqueue_copy_image_to_buffer(const image_object &src_image,
    ...
    cl_int ret = clEnqueueCopyImageToBuffer(
        m_queue, src_image.get(), dst_buffer.get(), src_origin, region,
        dst_offset, events.size(), events.get_event_ptr(), &event_.get());
  ...
  event enqueue_copy_buffer_to_image(const memory_object &src_buffer,
                                     image_object &dst_image, size_t src_offset,
    ...
    cl_int ret = clEnqueueCopyBufferToImage(
        m_queue, src_buffer.get(), dst_image.get(), src_offset, dst_origin,
        region, events.size(), events.get_event_ptr(), &event_.get());
  ...
  event enqueue_fill_image(image_object &image, const void *fill_color,
    ...
    cl_int ret = clEnqueueFillImage(m_queue, image.get(), fill_color, origin,
                                    region, events.size(),
                                    events.get_event_ptr(), &event_.get());
  ...
  template <size_t N>
  event enqueue_fill_image(image_object &image, const void *fill_color,
                           const extents<N> origin, const extents<N> region,
                           const wait_list &events = wait_list()) {
    BOOST_ASSERT(image.get_context() == this->get_context());
    ...
    return enqueue_fill_image(image, fill_color, origin3, region3, events);
  }
  ...
  event enqueue_migrate_memory_objects(uint_ num_mem_objects,
    ...
    cl_int ret = clEnqueueMigrateMemObjects(
        m_queue, num_mem_objects, mem_objects, flags, events.size(),
        events.get_event_ptr(), &event_.get());
  ...
  event enqueue_nd_range_kernel(const kernel &kernel, size_t work_dim,
    ...
    cl_int ret = clEnqueueNDRangeKernel(
        m_queue, kernel, static_cast<cl_uint>(work_dim), global_work_offset,
        global_work_size, local_work_size, events.size(),
        events.get_event_ptr(), &event_.get());
  ...
  event enqueue_nd_range_kernel(const kernel &kernel,
                                ...
                                const wait_list &events = wait_list()) {
    return enqueue_nd_range_kernel(kernel, N, global_work_offset.data(),
                                   global_work_size.data(),
                                   local_work_size.data(), events);
  }

  /// Convenience method which calls enqueue_nd_range_kernel() with a
  /// one-dimensional range.
  event enqueue_1d_range_kernel(const kernel &kernel, size_t global_work_offset,
                                size_t global_work_size, size_t local_work_size,
                                const wait_list &events = wait_list()) {
    return enqueue_nd_range_kernel(
        kernel, 1, &global_work_offset, &global_work_size,
        local_work_size ? &local_work_size : 0, events);
  }
...
// just forward to the equivalent clEnqueueNDRangeKernel() call.
    ...
    size_t one = 1;
    cl_int ret =
        clEnqueueNDRangeKernel(m_queue, kernel, 1, 0, &one, &one, events.size(),
                               events.get_event_ptr(), &event_.get());
#else
    cl_int ret = clEnqueueTask(m_queue, kernel, events.size(),
                               events.get_event_ptr(), &event_.get());
#endif
  ...
  event enqueue_native_kernel(
      void(BOOST_COMPUTE_CL_CALLBACK *user_func)(void *), void *args,
      size_t cb_args, uint_ num_mem_objects, const cl_mem *mem_list,
      const void **args_mem_loc, const wait_list &events = wait_list()) {
    BOOST_ASSERT(m_queue != 0);

    event event_;
    cl_int ret = clEnqueueNativeKernel(
        m_queue, user_func, args, cb_args, num_mem_objects, mem_list,
        args_mem_loc, events.size(), events.get_event_ptr(), &event_.get());
    if (ret != CL_SUCCESS) {
      BOOST_THROW_EXCEPTION(opencl_error(ret));
    }
  ...
  event enqueue_native_kernel(void(BOOST_COMPUTE_CL_CALLBACK *user_func)(void),
                              const wait_list &events = wait_list()) {
    return enqueue_native_kernel(detail::nullary_native_kernel_trampoline,
                                 reinterpret_cast<void *>(&user_func),
                                 sizeof(user_func), 0, 0, 0, events);
  }
  ...
  event enqueue_barrier(const wait_list &events) {
    ...
    ret = clEnqueueBarrierWithWaitList(m_queue, events.size(),
                                       events.get_event_ptr(), &event_.get());
  ...
  event enqueue_svm_memcpy(void *dst_ptr, const void *src_ptr, size_t size,
                           const wait_list &events = wait_list()) {
    event event_;

    cl_int ret = clEnqueueSVMMemcpy(m_queue, CL_TRUE, dst_ptr, src_ptr, size,
  ...
  ///
  /// \see_opencl2_ref{clEnqueueSVMMemcpy}
  event enqueue_svm_memcpy_async(void *dst_ptr, const void *src_ptr,
                                 size_t size,
                                 const wait_list &events = wait_list()) {
    event event_;

    cl_int ret = clEnqueueSVMMemcpy(m_queue, CL_FALSE, dst_ptr, src_ptr, size,
                                    events.size(), events.get_event_ptr(),
                                    &event_.get());
  ...
  event enqueue_svm_fill(void *svm_ptr, const void *pattern,
...

  {
    ...
    cl_int ret = clEnqueueSVMMemFill(m_queue, svm_ptr, pattern, pattern_size,
                                     size, events.size(),
                                     events.get_event_ptr(), &event_.get());
  ...
  event enqueue_svm_free(void *svm_ptr, const wait_list &events = wait_list()) {
    ...
    cl_int ret = clEnqueueSVMFree(m_queue, 1, &svm_ptr, 0, 0, events.size(),
                                  events.get_event_ptr(), &event_.get());
  ...
  event enqueue_svm_map(void *svm_ptr, size_t size, cl_map_flags flags,
                        const wait_list &events = wait_list()) {
    event event_;

    cl_int ret =
        clEnqueueSVMMap(m_queue, CL_TRUE, flags, svm_ptr, size, events.size(),
                        events.get_event_ptr(), &event_.get());
  ...
  event enqueue_svm_unmap(void *svm_ptr,
    ...
    cl_int ret = clEnqueueSVMUnmap(m_queue, svm_ptr, events.size(),
                                   events.get_event_ptr(), &event_.get());
...
inline image1d image1d::clone(command_queue &queue) const {
  image1d copy(get_context(), width(), format(), get_memory_flags());
...
inline image2d image2d::clone(command_queue &queue) const {
  image2d copy(get_context(), width(), height(), format(), get_memory_flags());
...
inline image3d image3d::clone(command_queue &queue) const {
  image3d copy(get_context(), width(), height(), depth(), format(),
               get_memory_flags());
...
BOOST_COMPUTE_DETAIL_DEFINE_GET_INFO_SPECIALIZATIONS(
    command_queue,
    ((cl_context, CL_QUEUE_CONTEXT))((cl_device_id, CL_QUEUE_DEVICE))(
        (uint_, CL_QUEUE_REFERENCE_COUNT))((cl_command_queue_properties,
                                            CL_QUEUE_PROPERTIES)))
