...
class buffer : public memory_object {
 public:
  ...
  buffer(const context &context, size_t size, cl_mem_flags flags = read_write,
         void *host_ptr = 0) {
    cl_int error = 0;
    m_mem = clCreateBuffer(context, flags, (std::max)(size, size_t(1)),
                           host_ptr, &error);
    if (!m_mem) {
  ...
  buffer(const buffer &other) : memory_object(other) {}
  ...
  buffer(buffer &&other) BOOST_NOEXCEPT : memory_object(std::move(other)) {}
  ...
  size_t size() const { return get_memory_size(); }
  ...
  /// \overload
  template <int Enum>
  typename detail::get_object_info_type<buffer, Enum>::type get_info() const;
  ...
  buffer create_subbuffer(cl_mem_flags flags, size_t origin, size_t size) {
    BOOST_ASSERT(origin + size <= this->size());
    BOOST_ASSERT(origin % (get_context()
                               .get_device()
                               .get_info<CL_DEVICE_MEM_BASE_ADDR_ALIGN>() /
                           8) ==
                 0);
    cl_int error = 0;
    ...
    cl_mem mem = clCreateSubBuffer(m_mem, flags, CL_BUFFER_CREATE_TYPE_REGION,
                                   &region, &error);
...
BOOST_COMPUTE_DETAIL_DEFINE_GET_INFO_SPECIALIZATIONS(
    buffer,
    ((cl_mem_object_type, CL_MEM_TYPE))((cl_mem_flags, CL_MEM_FLAGS))(
        (size_t, CL_MEM_SIZE))((void *, CL_MEM_HOST_PTR))((cl_uint,
                                                           CL_MEM_MAP_COUNT))(
        (cl_uint, CL_MEM_REFERENCE_COUNT))((cl_context, CL_MEM_CONTEXT)))

#ifdef CL_VERSION_1_1
BOOST_COMPUTE_DETAIL_DEFINE_GET_INFO_SPECIALIZATIONS(
    buffer, ((cl_mem, CL_MEM_ASSOCIATED_MEMOBJECT))((size_t, CL_MEM_OFFSET)))
#endif  // CL_VERSION_1_1
