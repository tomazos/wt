...
#include "compute/algorithm/any_of.h"
#include "compute/algorithm/fill.h"
#include "compute/algorithm/transform_reduce.h"
#include "compute/container/vector.h"
#include "compute/functional/integer.h"
...
class dynamic_bitset {
 public:
  ...
  dynamic_bitset(size_type size, command_queue &queue)
      : m_bits(size / sizeof(block_type), queue.get_context()), m_size(size) {
  ...
  dynamic_bitset(const dynamic_bitset &other)
      : m_bits(other.m_bits), m_size(other.m_size) {}
  ...
  size_type max_size() const { return m_bits.max_size() * bits_per_block; }
  ...
  size_type count(command_queue &queue) const {
    ulong_ count = 0;
    transform_reduce(m_bits.begin(), m_bits.end(), &count,
                     popcount<block_type>(), plus<ulong_>(), queue);
  ...
  void resize(size_type num_bits, command_queue &queue) {
    ...
    if (new_block_count > current_block_count) {
      fill_n(m_bits.begin() + current_block_count,
             new_block_count - current_block_count, block_type(0), queue);
    }
  ...
  void set(size_type n, command_queue &queue) { set(n, true, queue); }
  ...
  bool any(command_queue &queue) const {
    return any_of(m_bits.begin(), m_bits.end(), lambda::_1 != block_type(0),
                  queue);
  }

  /// Returns \c true if all of the bits in the bitset are set to zero.
  bool none(command_queue &queue) const { return !any(queue); }
  ...
  void reset(size_type n, command_queue &queue) { set(n, false, queue); }

  /// Empties the bitset (e.g. \c resize(0)).
  void clear() { m_bits.clear(); }

  /// Returns the allocator used to allocate storage for the bitset.
  allocator_type get_allocator() const { return m_bits.get_allocator(); }
