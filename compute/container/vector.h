...
#include "compute/allocator/buffer_allocator.h"
...
class vector {
 public:
  ...
  explicit vector(size_type count,
      ...
      : m_size(count), m_allocator(context) {
  ...
  vector(size_type count, const T &value,
      ...
      : m_size(count), m_allocator(queue.get_context()) {
    m_data = m_allocator.allocate((std::max)(count, _minimum_capacity()));
  ...
  vector(const std::vector<T, OtherAlloc> &vector,
         command_queue &queue = system::default_queue())
      : m_size(vector.size()), m_allocator(queue.get_context()) {
    m_data = m_allocator.allocate((std::max)(m_size, _minimum_capacity()));
  ...
  vector(std::initializer_list<T> list,
         command_queue &queue = system::default_queue())
      : m_size(list.size()), m_allocator(queue.get_context()) {
    m_data = m_allocator.allocate((std::max)(m_size, _minimum_capacity()));
  ...
  iterator end() {
    return ::boost::compute::make_buffer_iterator<T>(m_data.get_buffer(),
                                                     m_size);
  }
  ...
  void resize(size_type size, command_queue &queue) {
    if (size < capacity()) {
      ...
      // allocate new buffer
      pointer new_data = m_allocator.allocate(
          static_cast<size_type>(static_cast<float>(size) * _growth_factor()));
  ...
  size_type capacity() const { return m_data.get_buffer().size() / sizeof(T); }
  ...
  void shrink_to_fit(command_queue &queue) { (void)queue; }
  ...
  void pop_back(command_queue &queue) { resize(size() - 1, queue); }
  ...
  void insert(iterator position, size_type count, const T &value,
    ...
    ::boost::compute::copy(tmp.begin(), tmp.end(),
  ...
  const buffer &get_buffer() const { return m_data.get_buffer(); }
...
struct set_kernel_arg<vector<T, Alloc> > {
  void operator()(kernel &kernel_, size_t index,
