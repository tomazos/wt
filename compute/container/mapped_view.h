...
class mapped_view {
 public:
  ...
  mapped_view(const mapped_view<T> &other) : m_buffer(other.m_buffer) {
    m_mapped_ptr = 0;
  }
  ...
  size_type size() const { return m_buffer.size() / sizeof(T); }
  ...
  void map(cl_map_flags flags, command_queue &queue) {
    ...
    m_mapped_ptr =
        queue.enqueue_map_buffer(m_buffer, flags, 0, m_buffer.size());
  }
  ...
  /// \endcode
  void map(command_queue &queue) { map(CL_MAP_READ | CL_MAP_WRITE, queue); }
 ...
 private:
  /// \internal_
  static buffer _make_mapped_buffer(T *host_ptr, size_t n,
                                    const context &context) {
    return buffer(context, n * sizeof(T),
                  buffer::read_write | buffer::use_host_ptr, host_ptr);
  }

  /// \internal_
  static buffer _make_mapped_buffer(const T *host_ptr, size_t n,
                                    const context &context) {
    return buffer(context, n * sizeof(T),
