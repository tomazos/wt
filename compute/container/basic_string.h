...
#include "compute/algorithm/search.h"
...
class basic_string {
 public:
  ...
  typedef
      typename ::boost::compute::vector<CharT>::const_reference const_reference;
  typedef typename ::boost::compute::vector<CharT>::iterator iterator;
  typedef
      typename ::boost::compute::vector<CharT>::const_iterator const_iterator;
  typedef typename ::boost::compute::vector<CharT>::reverse_iterator
      reverse_iterator;
  ...
  basic_string(size_type count, CharT ch) : m_data(count) {
  ...
  basic_string(const char* s, size_type count) : m_data(s, s + count) {}

  basic_string(const char* s) : m_data(s, s + std::strlen(s)) {}
  ...
  size_type find(basic_string& str, size_type pos = 0) const {
    const_iterator iter =
        ::boost::compute::search(begin() + pos, end(), str.begin(), str.end());
    if (iter == end()) {
      return npos;
    } else {
      return static_cast<size_type>(std::distance(begin(), iter));
...
std::ostream& operator<<(
  ...
  boost::compute::copy(outStr.begin(), outStr.end(),
                       std::ostream_iterator<CharT>(stream), queue);
  return stream;
}
