...
#include "compute/algorithm/max_element.h"
#include "compute/algorithm/min_element.h"
...
class valarray {
 public:
  ...
  valarray(const T* values, size_t size,
           const context& context = system::default_context())
      : m_buffer(context, size * sizeof(T)) {
  ...
  valarray<T> operator-() const {
    BOOST_STATIC_ASSERT_MSG(
        ...
        " and vector types");
    valarray<T> result(size());
    BOOST_COMPUTE_FUNCTION(T, unary_minus, (T x), { return -x; });
  ...
  valarray<T> operator~() const {
    ...
    valarray<T> result(size());
    BOOST_COMPUTE_FUNCTION(T, bitwise_not, (T x), { return ~x; });
  ...
  valarray<char> operator!() const {
    ...
    BOOST_COMPUTE_FUNCTION(char, logical_not, (T x), { return !x; });
  ...
  size_t size() const { return m_buffer.size() / sizeof(T); }
  ...
  T(min)() const { return *(boost::compute::min_element(begin(), end())); }

  T(max)() const { return *(boost::compute::max_element(begin(), end())); }

  T sum() const { return boost::compute::accumulate(begin(), end(), T(0)); }
 ...
 private:
  buffer_iterator<T> begin() const { return buffer_iterator<T>(m_buffer, 0); }
...
#define BOOST_COMPUTE_DEFINE_VALARRAY_COMPOUND_ASSIGNMENT(op, op_name, assert) \
  template <class T>                                                           \
  inline valarray<T>& valarray<T>::operator op##=(const T& val) {              \
    assert transform(                                                          \
        begin(), end(), begin(),                                               \
    ...
    return *this;                                                              \
                                                                               ...
                                                                               \
  template <class T>                                                           \
  inline valarray<T>& valarray<T>::operator op##=(const valarray<T>& rhs) {    \
    assert transform(begin(), end(), rhs.begin(), begin(), op_name<T>());      \
    return *this;                                                              \
  }

/// \internal_
#define BOOST_COMPUTE_DEFINE_VALARRAY_COMPOUND_ASSIGNMENT_ANY(op, op_name) \
  BOOST_COMPUTE_DEFINE_VALARRAY_COMPOUND_ASSIGNMENT(                       \
      op, op_name,                                                         \
      BOOST_STATIC_ASSERT_MSG(                                             \
          is_fundamental<T>::value,                                        \
          "This operator can be used with all OpenCL built-in scalar"      \
  ...
  BOOST_COMPUTE_DEFINE_VALARRAY_COMPOUND_ASSIGNMENT(                         \
      ...
      BOOST_STATIC_ASSERT_MSG(                                               \
          is_fundamental<T>::value &&                                        \
              !is_floating_point<typename scalar_type<T>::type>::value,      \
          "This operator can be used with all OpenCL built-in scalar"        \
          " and vector types except the built-in scalar and vector float "   \
          "types");)
...
#define BOOST_COMPUTE_DEFINE_VALARRAY_BINARY_OPERATOR(op, op_name, assert)   \
  ...
  valarray<T> operator op(const valarray<T>& lhs, const valarray<T>& rhs) {  \
    assert valarray<T> result(lhs.size());                                   \
    transform(&lhs[0], &lhs[lhs.size()], &rhs[0], &result[0], op_name<T>()); \
    return result;                                                           \
  }                                                                          \
                                                                             \
  template <class T>                                                         \
  valarray<T> operator op(const T& val, const valarray<T>& rhs) {            \
    assert valarray<T> result(rhs.size());                                   \
