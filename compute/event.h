...
#include "compute/detail/duration.h"
...
/// \code
/// std::cout << "time = " << e.duration<std::chrono::milliseconds>().count() <<
/// "ms\n";
/// \endcode
///
...
class event {
 public:
  ...
  explicit event(cl_event event, bool retain = true) : m_event(event) {
    if (m_event && retain) {
      clRetainEvent(event);
    }
  }
  ...
  event(const event &other) : m_event(other.m_event) {
    if (m_event) {
  ...
  event(event &&other) BOOST_NOEXCEPT : m_event(other.m_event) {
    other.m_event = 0;
  }
  ...
  ~event() {
    if (m_event) {
      BOOST_COMPUTE_ASSERT_CL_SUCCESS(clReleaseEvent(m_event));
    }
  }

  /// Returns a reference to the underlying OpenCL event object.
  cl_event &get() const { return const_cast<cl_event &>(m_event); }
  ...
  /// \overload
  template <int Enum>
  typename detail::get_object_info_type<event, Enum>::type get_info() const;
  ...
  T get_profiling_info(cl_profiling_info info) const {
    return detail::get_object_info<T>(clGetEventProfilingInfo, m_event, info);
  }
  ...
  void set_callback(void(BOOST_COMPUTE_CL_CALLBACK *callback)(cl_event event,
                                                              cl_int status,
                                                              void *user_data),
  ...
  void set_callback(Function callback, cl_int status = CL_COMPLETE) {
    set_callback(event_callback_invoker, status,
  ...
  /// execute:
  /// \code
  /// std::cout << event.duration<std::chrono::milliseconds>().count() << " ms"
  /// << std::endl;
  /// \endcode
  ///
  ...
  bool operator==(const event &other) const { return m_event == other.m_event; }
  ...
  cl_int get_status() const { return status(); }
...
BOOST_COMPUTE_DETAIL_DEFINE_GET_INFO_SPECIALIZATIONS(
    event,
    ((cl_command_queue, CL_EVENT_COMMAND_QUEUE))((cl_command_type,
                                                  CL_EVENT_COMMAND_TYPE))((
        cl_int, CL_EVENT_COMMAND_EXECUTION_STATUS))((cl_uint,
