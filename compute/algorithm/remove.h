...
#include "compute/algorithm/remove_if.h"
...
inline Iterator remove(Iterator first, Iterator last, const T &value,
  ...
  if (vector_size<value_type>::value == 1) {
    return ::boost::compute::remove_if(first, last, _1 == value, queue);
  } else {
    return ::boost::compute::remove_if(first, last, all(_1 == value), queue);
  }
}
