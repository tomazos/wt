...
struct reverse_kernel : public meta_kernel {
  reverse_kernel(Iterator first, Iterator last) : meta_kernel("reverse") {
    ...
    *this << decl<cl_uint>("i") << " = get_global_id(0);\n"
  ...
  void exec(command_queue &queue) { exec_1d(queue, 0, m_size / 2); }
