...
inline InputIterator np_ceiling(InputIterator first, InputIterator last,
                                ValueType value, command_queue &queue) {
  typedef typename std::iterator_traits<InputIterator>::value_type value_type;
  ...
  size_t index_arg = k.add_arg<int *>(memory_object::global_memory, "index");
  size_t value_arg =
      k.add_arg<value_type>(memory_object::private_memory, "value");
  atomic_max<int_> atomic_max_int;
...
///
template <class InputIterator>
inline bool next_permutation(InputIterator first, InputIterator last,
