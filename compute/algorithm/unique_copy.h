...
#include "compute/algorithm/copy_if.h"
#include "compute/algorithm/transform.h"
#include "compute/algorithm/gather.h"
...
inline OutputIterator serial_unique_copy(InputIterator first,
  ...
  size_t unique_count_arg =
      k.add_arg<uint_ *>(memory_object::global_memory, "unique_count");

  k << k.decl<uint_>("index") << " = 0;\n" << k.decl<value_type>("current")
    << " = " << first[k.var<uint_>("0")] << ";\n" << result[k.var<uint_>("0")]
    << " = current;\n"
...
inline OutputIterator unique_copy(InputIterator first, InputIterator last,
                                  OutputIterator result, BinaryPredicate op,
                                  command_queue &queue) {
  if (first == last) {
    return result;
  }
  ...
  transform(first, last - 1, first + 1, flags.begin() + 1, not2(op), queue);
...
inline OutputIterator unique_copy(
    ...
    BinaryPredicate op, command_queue &queue = system::default_queue()) {
  size_t count = detail::iterator_range_size(first, last);
  if (count < 32) {
