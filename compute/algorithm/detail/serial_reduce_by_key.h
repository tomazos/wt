...
inline size_t serial_reduce_by_key(
  ...
  typedef typename ::boost::compute::result_of<BinaryFunction(
      value_type, value_type)>::type result_type;
  ...
  size_t result_size_arg =
      k.add_arg<uint_ *>(memory_object::global_memory, "result_size");

  convert<result_type> to_result_type;

  k << k.decl<result_type>("result") << " = " << to_result_type(values_first[0])
    ...
    << "    if ("
    << predicate(k.var<key_type>("previous_key"), k.var<key_type>("key"))
    << ") {\n"
    <<
    ...
    << "    }\n "
    << "    else { \n" << keys_result[k.var<uint_>("size - 1")]
    << " = previous_key;\n" << values_result[k.var<uint_>("size - 1")]
    << " = result;\n"
    << "        result = value;\n"
    << "        size++;\n"
    << "    } \n"
