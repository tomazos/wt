...
#include "compute/functional/atomic.h"
...
inline InputIterator find_extrema_with_atomics(InputIterator first,
  ...
  k << "const uint gid = get_global_id(0);\n"
    ...
    << "while("
    << compare(first[k.var<uint_>("gid")], first[k.var<uint_>("old_index")])
    << "){\n"
    << "#else\n"
    << "while("
    << compare(first[k.var<uint_>("old_index")], first[k.var<uint_>("gid")])
    << "){\n"
    << "#endif\n"
    <<

      "  if("
    << atomic_cmpxchg_uint(k.var<uint_ *>("index"), k.var<uint_>("old_index"),
  ...
  size_t index_arg_index =
      k.add_arg<uint_ *>(memory_object::global_memory, "index");
