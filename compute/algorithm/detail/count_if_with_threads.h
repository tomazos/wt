...
class count_if_with_threads_kernel : meta_kernel {
 public:
  ...
  count_if_with_threads_kernel() : meta_kernel("count_if_with_threads") {}

  void set_args(InputIterator first, InputIterator last, Predicate predicate)
  ...
  size_t exec(command_queue &queue) {
    ...
    if (m_size / threads < minimum_block_size) {
      threads = static_cast<size_t>(
          (std::max)(std::ceil(float(m_size) / minimum_block_size), 1.0f));
    }
    ...
    ::boost::compute::copy(counts.begin(), counts.end(), host_counts.begin(),
                           queue);
...
template <class InputIterator, class Predicate>
inline size_t count_if_with_threads(InputIterator first, InputIterator last,
