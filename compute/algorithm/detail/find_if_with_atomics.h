...
template <class InputIterator, class UnaryPredicate>
inline InputIterator find_if_with_atomics_multiple_vpt(
    InputIterator first, InputIterator last, UnaryPredicate predicate,
    const size_t count, const uint_ vpt, command_queue &queue) {
  typedef typename std::iterator_traits<InputIterator>::value_type value_type;
  typedef typename std::iterator_traits<InputIterator>::difference_type
      difference_type;
  ...
  if (device.type() & device::gpu) {
    k << k.decl<const uint_>("lsize") << " = get_local_size(0);\n"
      << k.decl<uint_>("id")
      << " = get_local_id(0) + get_group_id(0) * lsize * vpt;\n"
      ...
      << first[k.var<const uint_>("id")] << ";\n"
                                            "    if("
      << predicate(k.var<const value_type>("value")) << "){\n"
      << "        "
    ...
    k << k.decl<uint_>("id") << " = get_global_id(0) * " << k.var<uint_>("vpt")
      << ";\n" << k.decl<const uint_>("end") << " = min("
      << "id + " << k.var<uint_>("vpt") << ","
      << "count"
      << ");\n"
  ...
  const size_t global_wg_size =
      static_cast<size_t>(std::ceil(float(count) / vpt));
...
inline InputIterator find_if_with_atomics(InputIterator first,
  ...
  std::string cache_key =
      std::string("__boost_find_if_with_atomics_") + type_name<value_type>();
  ...
  if (device.type() & device::gpu) {
    ...
    if (count <= one_vpt_threshold) {
      return find_if_with_atomics_one_vpt(first, last, predicate, count, queue);
    }
  }
  ...
  if (device.type() & device::gpu) {
    ...
    vpt = static_cast<size_t>(std::ceil(float(count) / max_compute_units));
  }

  return find_if_with_atomics_multiple_vpt(first, last, predicate, count, vpt,
                                           queue);
}
