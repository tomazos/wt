...
class copy_kernel : public meta_kernel {
 public:
  copy_kernel(const device &device) : meta_kernel("copy") {
    m_count = 0;
  ...
  void set_range(InputIterator first, InputIterator last,
                 OutputIterator result) {
    m_count_arg = add_arg<uint_>("count");

    *this << "uint index = get_local_id(0) + "
...
template <class InputIterator, class OutputIterator>
inline OutputIterator copy_on_device(InputIterator first, InputIterator last,
                                     OutputIterator result,
                                     command_queue &queue) {
...
inline discard_iterator copy_on_device(InputIterator first, InputIterator last,
...
template <class T>
inline svm_ptr<T> copy_on_device(svm_ptr<T> first, svm_ptr<T> last,
                                 svm_ptr<T> result, command_queue &queue) {
  size_t count = iterator_range_size(first, last);
  if (count == 0) {
    return result;
  }

  queue.enqueue_svm_memcpy(result.get(), first.get(), count * sizeof(T));
...
inline future<svm_ptr<T> > copy_on_device_async(svm_ptr<T> first,
  ...
  event event_ = queue.enqueue_svm_memcpy_async(result.get(), first.get(),
                                                count * sizeof(T));
