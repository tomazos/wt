...
inline InputIterator serial_find_extrema(InputIterator first,
  ...
  k << k.decl<value_type>("value") << " = " << first[k.expr<uint_>("0")]
    ...
    << "  if("
    << compare(k.var<value_type>("candidate"), k.var<value_type>("value"))
    << "){\n"
    << "#else\n"
    << "  if("
    << compare(k.var<value_type>("value"), k.var<value_type>("candidate"))
    << "){\n"
    << "#endif\n"
    <<
