...
#include "compute/detail/iterator_plus_distance.h"
...
inline HostIterator copy_to_host(DeviceIterator first, DeviceIterator last,
                                 HostIterator result, command_queue &queue) {
  ...
  queue.enqueue_read_buffer(buffer, offset * sizeof(value_type),
...
inline std::vector<bool>::iterator copy_to_host(
    DeviceIterator first, DeviceIterator last,
    std::vector<bool>::iterator result, command_queue &queue) {
...
inline future<HostIterator> copy_to_host_async(DeviceIterator first,
  ...
  event event_ = queue.enqueue_read_buffer_async(
...
inline HostIterator copy_to_host(svm_ptr<T> first, svm_ptr<T> last,
                                 HostIterator result, command_queue &queue) {
  size_t count = iterator_range_size(first, last);
  if (count == 0) {
    return result;
  ...
  queue.enqueue_svm_memcpy(::boost::addressof(*result), first.get(),
                           count * sizeof(T));
...
inline future<HostIterator> copy_to_host_async(svm_ptr<T> first,
  ...
  event event_ = queue.enqueue_svm_memcpy_async(::boost::addressof(*result),
                                                first.get(), count * sizeof(T));
