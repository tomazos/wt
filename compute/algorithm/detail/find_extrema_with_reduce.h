...
#include "compute/detail/parameter_cache.h"
#include "compute/memory/local_buffer.h"
#include "compute/type_traits/type_name.h"
#include "compute/utility/program_cache.h"
#include "compute/algorithm/detail/serial_find_extrema.h"
...
inline size_t find_extrema_with_reduce(InputIterator first, size_t count,
                                       ResultIterator result,
                                       vector<uint_>::iterator result_idx,
                                       size_t work_groups_no,
                                       size_t work_group_size, Compare compare,
  ...
  size_t block_arg =
      k.add_arg<input_type *>(memory_object::local_memory, "block");
  size_t block_idx_arg =
      k.add_arg<uint_ *>(memory_object::local_memory, "block_idx");
  ...
  k <<
    ...
    << "compare_result = "
    << compare(k.var<input_type>("acc"), k.var<input_type>("next")) << ";\n"
    << "#else\n"
    << "compare_result = "
    << compare(k.var<input_type>("next"), k.var<input_type>("acc")) << ";\n"
    << "#endif\n"
  ...
  k << "#pragma unroll\n"
       "for(" << k.decl<uint_>("offset") << " = " << uint_(work_group_size)
    << " / 2; offset > 0; "
    << "offset = offset / 2) {\n"
    ...
    << "#ifndef BOOST_COMPUTE_FIND_MAXIMUM\n"
    << "compare_result = "
    << compare(k.var<input_type>("mine"), k.var<input_type>("other")) << ";\n"
    << "#else\n"
    << "compare_result = "
    << compare(k.var<input_type>("other"), k.var<input_type>("mine")) << ";\n"
    << "#endif\n"
...
uint_ find_extrema_final(InputIterator candidates,
                         ...
                         const size_t count, Compare compare,
                         const bool find_minimum, const size_t work_group_size,
                         command_queue &queue) {
  typedef typename std::iterator_traits<InputIterator>::value_type input_type;
  ...
  find_extrema_with_reduce(candidates, count, result.begin(),
                           result_idx.begin(), 1, work_group_size, compare,
                           find_minimum, queue);
...
template <class InputIterator>
uint_ find_extrema_final(
    InputIterator candidates, vector<uint_>::iterator candidates_idx,
    const size_t count,
    ::boost::compute::less<
        typename std::iterator_traits<InputIterator>::value_type> compare,
    const bool find_minimum, const size_t work_group_size,
    command_queue &queue) {
  ...
  vector<uint_>::iterator candidates_idx_last = candidates_idx + count;
  ...
  ::boost::compute::copy(candidates, candidates_last, host_candidates.begin(),
                         queue);
...
template <class InputIterator, class Compare>
InputIterator find_extrema_with_reduce(InputIterator first, InputIterator last,
                                       Compare compare, const bool find_minimum,
                                       command_queue &queue) {
  typedef typename std::iterator_traits<InputIterator>::difference_type
      difference_type;
  typedef typename std::iterator_traits<InputIterator>::value_type input_type;
  ...
  find_extrema_with_reduce(first, count, candidates.begin(),
                           candidates_idx.begin(), work_groups_no,
                           work_group_size, compare, find_minimum, queue);
