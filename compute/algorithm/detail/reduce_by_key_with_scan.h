...
#include "compute/algorithm/inclusive_scan.h"
#include "compute/container/vector.h"
#include "compute/container/detail/scalar.h"
...
/// of original keys \p keys_first. New keys can be distinguish by simple
/// equality
/// predicate.
///
...
inline void generate_uint_keys(InputKeyIterator keys_first,
                               size_t number_of_keys, BinaryPredicate predicate,
  ...
  size_t work_groups_no =
      static_cast<size_t>(std::ceil(float(number_of_keys) / work_group_size));

  queue.enqueue_1d_range_kernel(kernel, 0, work_groups_no * work_group_size,
                                work_group_size);
...
template <class InputValueIterator, class OutputValueIterator,
          class BinaryFunction>
inline void carry_outs(vector<uint_>::iterator keys_first,
                       InputValueIterator values_first, size_t count,
                       vector<uint_>::iterator carry_out_keys_first,
                       OutputValueIterator carry_out_values_first,
                       BinaryFunction function, size_t work_group_size,
                       command_queue &queue) {
  typedef typename std::iterator_traits<OutputValueIterator>::value_type
      value_out_type;

  detail::meta_kernel k("reduce_by_key_with_scan_carry_outs");
  k.add_set_arg<const uint_>("count", uint_(count));
  size_t local_keys_arg =
      k.add_arg<uint_ *>(memory_object::local_memory, "lkeys");
  size_t local_vals_arg =
      k.add_arg<value_out_type *>(memory_object::local_memory, "lvals");
  ...
  k << k.decl<const uint_>("gid") << " = get_global_id(0);\n"
    ...
    << "lvals[lid] = value;\n" <<
      ...
      // Calculate carry out for each work group by performing Hillis/Steele
      // scan
...
inline void carry_ins(vector<uint_>::iterator carry_out_keys_first,
                      ...
                      size_t carry_out_size, BinaryFunction function,
                      size_t work_group_size, command_queue &queue) {
  typedef typename std::iterator_traits<OutputValueIterator>::value_type
      value_out_type;

  uint_ values_pre_work_item =
      static_cast<uint_>(std::ceil(float(carry_out_size) / work_group_size));
  ...
  k << k.decl<const uint_>("id")
    << " = get_global_id(0) * values_per_work_item;\n" << k.decl<uint_>("idx")
    << " = id;\n" << k.decl<const uint_>("wg_size") << " = get_local_size(0);\n"
    << k.decl<const uint_>("lid") << " = get_local_id(0);\n"
    << k.decl<const uint_>("group_id") << " = get_group_id(0);\n"
    <<
    ...
    << carry_out_keys_first[k.var<const uint_>("idx")] << ";\n"
    << k.decl<value_out_type>("result") << " = "
    << carry_out_values_first[k.var<const uint_>("idx")] << ";\n"
    << carry_in_values_first[k.var<const uint_>("idx")] << " = result;\n" <<

      k.decl<const uint_>("end")
    << " = (id + values_per_work_item) <= carry_out_size"
    ...
    << "        result = value;\n"
       "    }\n"
    << "    " << carry_in_values_first[k.var<const uint_>("idx")]
    << " = result;\n"
    << "    previous_key = key;\n"
       "}\n" <<
  ...
  queue.enqueue_1d_range_kernel(kernel, 0, work_group_size, work_group_size);
}
...
inline void final_reduction(InputKeyIterator keys_first,
                            ...
                            OutputValueIterator values_result, size_t count,
                            BinaryFunction function,
                            vector<uint_>::iterator new_keys_first,
                            vector<uint_>::iterator carry_in_keys_first,
                            OutputValueIterator carry_in_values_first,
                            size_t carry_in_size, size_t work_group_size,
                            command_queue &queue) {
  typedef typename std::iterator_traits<OutputValueIterator>::value_type
      value_out_type;
  ...
  k << k.decl<const uint_>("gid") << " = get_global_id(0);\n"
    ...
    << "if(group_id > 0 && save) {\n"
    << "    carry_in_key = "
    << carry_in_keys_first[k.var<const uint_>("group_id - 1")] << ";\n"
    << "    if(key == carry_in_key){\n"
    << "        other_value = "
    << carry_in_values_first[k.var<const uint_>("group_id - 1")] << ";\n"
    << "        result = " << function(k.var<value_out_type>("result"),
                                       k.var<value_out_type>("other_value"))
    << ";\n"
    << "    }\n"
...
/// \internal_
///
/// 1. For each work group carry-out value is calculated (it's done by
/// key-oriented
...
inline size_t reduce_by_key_with_scan(
    InputKeyIterator keys_first, InputKeyIterator keys_last,
    InputValueIterator values_first, OutputKeyIterator keys_result,
    OutputValueIterator values_result, BinaryFunction function,
    BinaryPredicate predicate, command_queue &queue) {
  typedef
      typename std::iterator_traits<InputValueIterator>::value_type value_type;
  typedef typename std::iterator_traits<InputKeyIterator>::value_type key_type;
  typedef typename std::iterator_traits<OutputValueIterator>::value_type
      value_out_type;
  ...
  // predicate. New key is also an index for keys_result and values_result
  // vectors,
  ...
  final_reduction(keys_first, values_first, keys_result, values_result, count,
                  function, new_keys_first, carry_out_keys.begin(),
                  carry_in_values.begin(), carry_out_size, work_group_size,
                  queue);

  const size_t result =
      read_single_value<uint_>(new_keys.get_buffer(), count - 1, queue);
  return result + 1;
}
