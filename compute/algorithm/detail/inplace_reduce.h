...
inline void inplace_reduce(Iterator first, Iterator last,
  ...
  k << "const uint gid = get_global_id(0);\n"
    ...
    << "if(index < input_size){\n" << k.decl<value_type>("sum")
    << " = input[index];\n"
    << "for(uint i = 1;\n"
    << "i < values_per_thread && (index + i) < input_size;\n"
    << "i++){\n"
    << "    sum = " << function(k.var<value_type>("sum"),
  ...
  if (input_buffer != &first.get_buffer()) {
    ::boost::compute::copy(output.begin(), output.begin() + 1, first, queue);
  }
}
