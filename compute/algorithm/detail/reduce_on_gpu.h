...
#include "compute/utility/source.h"
#include "compute/program.h"
#include "compute/command_queue.h"
#include "compute/detail/vendor.h"
#include "compute/detail/parameter_cache.h"
#include "compute/detail/work_size.h"
...
inline void initial_reduce(InputIterator first, InputIterator last,
                           buffer result, const Function &function,
                           kernel &reduce_kernel, const uint_ vpt,
                           const uint_ tpb, command_queue &queue) {
  (void)function;
  (void)reduce_kernel;
  ...
  k << k.decl<const uint_>("offset") << " = get_group_id(0) * VPT * TPB;\n"
...
inline void initial_reduce(const buffer_iterator<T> &first,
                           const buffer_iterator<T> &last, const buffer &result,
                           const plus<T> &function, kernel &reduce_kernel,
                           const uint_ vpt, const uint_ tpb,
                           command_queue &queue) {
  (void)function;
...
inline void reduce_on_gpu(InputIterator first, InputIterator last,
                          buffer_iterator<T> result, Function function,
                          command_queue &queue) {
  ...
  k << k.decl<const uint_>("block_offset")
    << " = get_group_id(0) * VPT * TPB;\n"
    << "__global const " << type_name<T>()
    << " *block = input + offset + block_offset;\n"
    << k.decl<const uint_>("lid") << " = get_local_id(0);\n"
    <<
  ...
  std::string cache_key =
      std::string("__boost_reduce_on_gpu_") + type_name<T>();
