...
#include "compute/functional/detail/nvidia_ballot.h"
#include "compute/functional/detail/nvidia_popcount.h"
...
inline size_t count_if_with_ballot(InputIterator first, InputIterator last,
  ...
  uint_ result;
  ::boost::compute::reduce(counts.begin(), counts.end(), &result, queue);
  return result;
}
