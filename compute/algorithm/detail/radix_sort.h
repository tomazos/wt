...
#include "compute/type_traits/is_vector_type.h"
...
const char radix_sort_source[] =
    ...
    "    if(lid < K2_BITS){\n"
    "        global_counts[K2_BITS*get_group_id(0) + lid] = "
    "local_counts[lid];\n"
...
inline void radix_sort_impl(const buffer_iterator<T> first,
  ...
  for (uint_ i = 0; i < sizeof(sort_type) * CHAR_BIT / k; i++) {
    ...
    queue.enqueue_1d_range_kernel(count_kernel, 0, block_count * block_size,
                                  block_size);
    ...
    if (k == 1) {
      ...
      ::boost::compute::exclusive_scan(
          ...
          make_buffer_iterator<counter_type>(counts.get_buffer(),
                                             counts.size() / 2),
          make_buffer_iterator<counter_type>(counts.get_buffer()), queue);
    } else if (k == 2) {
    ...
    queue.enqueue_1d_range_kernel(scatter_kernel, 0, block_count * block_size,
                                  block_size);
...
inline void radix_sort(Iterator first, Iterator last, command_queue &queue) {
  radix_sort_impl(first, last, buffer_iterator<int>(), queue);
}

template <class KeyIterator, class ValueIterator>
inline void radix_sort_by_key(KeyIterator keys_first, KeyIterator keys_last,
