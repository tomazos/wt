...
#include "compute/algorithm/detail/serial_reduce_by_key.h"
#include "compute/algorithm/detail/reduce_by_key_with_scan.h"
...
size_t reduce_by_key_on_gpu(InputKeyIterator keys_first,
                            ...
                            BinaryFunction function, BinaryPredicate predicate,
                            command_queue &queue) {
...
bool reduce_by_key_on_gpu_requirements_met(InputKeyIterator keys_first,
  ...
  return (count > 256) && !(device.type() & device::cpu) &&
...
template <class InputKeyIterator, class InputValueIterator,
          class OutputKeyIterator, class OutputValueIterator,
          class BinaryFunction, class BinaryPredicate>
inline std::pair<OutputKeyIterator, OutputValueIterator> dispatch_reduce_by_key(
  ...
  typedef typename std::iterator_traits<OutputKeyIterator>::difference_type
      key_difference_type;
  typedef typename std::iterator_traits<OutputValueIterator>::difference_type
  ...
  if (count < 2) {
    ...
    return std::make_pair<OutputKeyIterator, OutputValueIterator>(
        keys_result + static_cast<key_difference_type>(count),
        values_result + static_cast<value_difference_type>(count));
  }

  size_t result_size = 0;
  if (reduce_by_key_on_gpu_requirements_met(
          keys_first, values_first, keys_result, values_result, count, queue)) {
    result_size = detail::reduce_by_key_on_gpu(
        keys_first, keys_last, values_first, keys_result, values_result,
        function, predicate, queue);
  } else {
    result_size = detail::serial_reduce_by_key(
        keys_first, keys_last, values_first, keys_result, values_result,
        function, predicate, queue);
  }
