...
#include "compute/algorithm/reduce.h"
#include "compute/algorithm/detail/serial_accumulate.h"
...
inline T generic_accumulate(InputIterator first, InputIterator last, T init,
  ...
  detail::serial_accumulate(first, last, device_result.begin(), init, function,
                            queue);
...
BOOST_PP_SEQ_FOR_EACH(
    BOOST_COMPUTE_DETAIL_DECLARE_CAN_ACCUMULATE_WITH_REDUCE, _,
...
template <class InputIterator, class T, class BinaryFunction>
inline T dispatch_accumulate(InputIterator first, InputIterator last, T init,
                             BinaryFunction function, command_queue &queue) {
  size_t size = iterator_range_size(first, last);
  if (size == 0) {
    return init;
