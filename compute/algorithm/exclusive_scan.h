...
#include "compute/algorithm/detail/scan.h"
...
inline OutputIterator exclusive_scan(
    InputIterator first, InputIterator last, OutputIterator result, T init,
    BinaryOperator binary_op, command_queue &queue = system::default_queue()) {
...
inline OutputIterator exclusive_scan(
  ...
  return detail::scan(first, last, result, true, init,
                      boost::compute::plus<output_type>(), queue);
}
...
inline OutputIterator exclusive_scan(
  ...
  return detail::scan(first, last, result, true, output_type(0),
                      boost::compute::plus<output_type>(), queue);
}
