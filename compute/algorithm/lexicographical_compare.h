...
inline bool dispatch_lexicographical_compare(InputIterator1 first1,
  ...
  std::string cache_key = std::string("__boost_lexicographical_compare") +
  ...
  queue.enqueue_1d_range_kernel(lexicographical_compare_kernel, 0, max_size, 0);

  return boost::compute::any_of(result_vector.begin(), result_vector.end(),
                                _1 == 0, queue);
}
...
inline bool lexicographical_compare(
    ...
    InputIterator2 last2, command_queue &queue = system::default_queue()) {
  return detail::dispatch_lexicographical_compare(first1, last1, first2, last2,
                                                  queue);
}
