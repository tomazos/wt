...
#include "compute/algorithm/detail/reduce_by_key.h"
...
///
/// Returns a pair of iterators at the end of the ranges [\p keys_result,
/// keys_result_last)
...
/// \param predicate binary predicate which returns true only if two keys are
/// equal
...
inline std::pair<OutputKeyIterator, OutputValueIterator> reduce_by_key(
    ...
    BinaryPredicate predicate, command_queue &queue = system::default_queue()) {
...
inline std::pair<OutputKeyIterator, OutputValueIterator> reduce_by_key(
  ...
  return reduce_by_key(keys_first, keys_last, values_first, keys_result,
                       values_result, function, equal_to<key_type>(), queue);
}
...
inline std::pair<OutputKeyIterator, OutputValueIterator> reduce_by_key(
  ...
  return reduce_by_key(keys_first, keys_last, values_first, keys_result,
                       values_result, plus<value_type>(), equal_to<key_type>(),
                       queue);
}
