...
#define LUA_STRFTIMEOPTIONS                                          \
  {                                                                  \
    "aAbBcCdDeFgGhHIjmMnprRStTuUVwWxXyYzZ%", "", "E", "cCxXyY", "O", \
        "deHImMSuUVwWy"                                              \
  }
#endif
...
static int getfield(lua_State *L, const char *key, int d) {
  ...
  if (!isnum) {
    if (d < 0) return luaL_error(L, "field '%s' missing in date table", key);
    res = d;
  }
  lua_pop(L, 1);
  return res;
...
static const luaL_Reg syslib[] = {{"clock", os_clock},
                                  {"date", os_date},
