...
enum EValueCategory { VC_LVALUE, VC_XVALUE, VC_PRVALUE };
...
inline const char* EValueCategoryToString(EValueCategory e) {
  switch (e) {
    ...
    case VC_PRVALUE:
      return "VC_PRVALUE";
    default:
      throw logic_error("internal error: unknown value category");
  }
}
...
struct Expression : Pooled {
  ...
  Expression(EValueCategory value_category, Type* type)
      : value_category(value_category), type(type) {}
  ...
  virtual void set_type(Type* new_type) { type = new_type; }

  virtual Expression* to_fundamental_type(EFundamentalType source,
                                          EFundamentalType destination);
  ...
  virtual vector<byte> get_constant_data() {
    throw logic_error("get_constant_data on non-constant expression");
  }

  virtual vector<byte> get_reference_data() {
    throw logic_error("get_reference_data on invalid expression");
  }
...
struct FundamentalTypeConversion : Expression {
  ...
  FundamentalTypeConversion(Type* type, Expression* operand)
      : Expression(VC_PRVALUE, type), operand(operand) {}
...
struct StringLiteral : Expression {
  ...
  StringLiteral(Type* type, const vector<byte>& data)
      : Expression(VC_LVALUE, type), data(data) {}
...
vector<byte> ConvertFundamentalValue(EFundamentalType source,
                                     EFundamentalType destination,
                                     const vector<byte>& data_in);

struct Immediate : Expression {
  vector<byte> data;

  Immediate(Type* type, const vector<byte>& data)
      : Expression(VC_PRVALUE, type), data(data) {}

  virtual bool is_constant_expression() { return true; }

  virtual bool is_null_pointer_constant() {
    if (type->is_nullptr_t()) return true;

    if (!type->is_integral_type()) return false;
...
struct StringLiteralAddress : Expression {
  ...
  StringLiteralAddress(StringLiteral* string_literal, Type* type)
      : Expression(VC_PRVALUE, type), string_literal(string_literal) {}
...
struct PointerToBoolConversion : Expression {
  ...
  Expression* relocate() { return operand->relocate()->pointer_to_bool(); }
...
struct LValueToRValueConversion : Expression {
  ...
  LValueToRValueConversion(Type* type, Variable* variable)
      : Expression(VC_PRVALUE, type), variable(variable) {}
...
struct ArrayVariablePointer : Expression {
  ...
  ArrayVariablePointer(Type* type, Variable* array)
      : Expression(VC_PRVALUE, type), array(array) {}
...
struct VariableExpression : Expression {
  ...
  VariableExpression(Type* type, Variable* variable)
      : Expression(VC_LVALUE, type), variable(variable) {}
...
struct FunctionAddress : Expression {
  ...
  FunctionAddress(Type* type, Function* function)
      : Expression(VC_PRVALUE, type), function(function) {}
...
struct FunctionExpression : Expression {
  ...
  FunctionExpression(Type* type, Function* function)
      : Expression(VC_LVALUE, type), function(function) {}
