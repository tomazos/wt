...
{
	static unordered_set<string> S = { "u", "U", "L" };
...
{
	static unordered_set<string> S = { "u8", "u", "U", "L" };
...
{
	static unordered_set<string> S = { "u8R", "uR", "UR", "LR", "R" };
...
{
	static unordered_set<int> simple_escape_chars =
	{ '\'', '"', '?', '\\', 'a', 'b', 'f', 'n', 'r', 't', 'v' };
...
struct TrigraphDecoder
{
	vector<int> decode(int input)
	{
		static unordered_map<int, int> trigraphs =
		{
			{ '=',  '#' },
			{ '/',  '\\'},
			{ '\'', '^' },
...
struct PPTokenizer
{
	...
	void emit_user_defined_character_literal(const string& data)
	{
		...
		output.emit_user_defined_character_literal(SourceInfo{data, filename, linenum});
	}
	...
	void emit_user_defined_string_literal(const string& data)
	{
		...
		output.emit_user_defined_string_literal(SourceInfo{data, filename, linenum});
	}
	...
	action_t next_state()
	{
		...
		case pp_number:
			...
			else if (IsAllowedIdentifierBodyCharacter(lookahead) || lookahead == '.')
				return keep_wait(pp_number);
			else
			{
		...
		case identifier:
			if (IsAllowedIdentifierBodyCharacter(lookahead))
				return keep_wait(identifier);
			else if (lookahead == '\'' && IsCharacterLiteralEncodingPrefix(accumulator_utf8()))
				return keep_wait(character_literal);
			else if (lookahead == '"' && IsStringLiteralEncodingPrefix(accumulator_utf8()))
				return keep_wait(string_literal);
			else if (lookahead == '"' && IsRawStringLiteralPrefix(accumulator_utf8()))
			{
				raw_mode = true;
		...
		case character_literal_backslash:
			if (IsSimpleEscapeChar(lookahead) || (lookahead >= '0' && lookahead <= '7'))
				return keep_wait(character_literal);
			else if (lookahead == 'x')
		...
		case done:
			throw logic_error("already done");
