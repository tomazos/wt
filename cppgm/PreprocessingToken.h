...
inline string EPreprocessingTokenTypeToString(EPreprocessingTokenType t) {
  switch (t) {
    ...
    case PP_CHARACTER_LITERAL:
      return "PP_CHARACTER_LITERAL";
    case PP_USER_DEFINED_CHARACTER_LITERAL:
      return "PP_USER_DEFINED_CHARACTER_LITERAL";
    case PP_STRING_LITERAL:
      return "PP_STRING_LITERAL";
    case PP_USER_DEFINED_STRING_LITERAL:
      return "PP_USER_DEFINED_STRING_LITERAL";
    case PP_OP_OR_PUNC:
      return "PP_OP_OR_PUNC";
    case PP_NONWHITESPACE_CHAR:
      return "PP_NONWHITESPACE_CHAR";
    case PP_PLACEMARKER:
      return "PP_PLACEMARKER";
    case PP_EOF:
      return "PP_EOF";
    default:
      throw logic_error("unknown EPreprocessingTokenType");
  }
}

struct PreprocessingToken {
  PreprocessingToken() : pp_type(PP_INVALID) {}

  PreprocessingToken(SourceInfo src, EPreprocessingTokenType pp_type)
      : src(src), pp_type(pp_type) {}
  ...
  bool operator==(const PreprocessingToken& that) const {
    return pp_type == that.pp_type && src.spelling == that.src.spelling;
  }
  bool operator!=(const PreprocessingToken& that) const {
    return pp_type == that.pp_type && src.spelling != that.src.spelling;
  }

  bool is_identifier(const string& id) const {
    return pp_type == PP_IDENTIFIER && src.spelling == id;
  }
  bool is_varargs() const {
    return pp_type == PP_IDENTIFIER && src.spelling == "__VA_ARGS__";
  }
  bool is_hash() const {
    return pp_type == PP_OP_OR_PUNC &&
           (src.spelling == "#" || src.spelling == "%:");
  }
  bool is_hash2_blessed() const {
    return blessed && pp_type == PP_OP_OR_PUNC &&
           (src.spelling == "##" || src.spelling == "%:%:");
  }
  bool is_hash2_unblessed() const {
    return !blessed && pp_type == PP_OP_OR_PUNC &&
           (src.spelling == "##" || src.spelling == "%:%:");
  }
  bool is_lparen() const {
    return pp_type == PP_OP_OR_PUNC && src.spelling == "(";
  }
  ...
  string to_string() const {
    ...
    oss << EPreprocessingTokenTypeToString(pp_type) << "(" << src.spelling
        << ")" << (blessed ? "*" : "");
...
struct PreprocesingTokenPPTokenStream : IPPTokenStream {
  ...
  void emit_user_defined_character_literal(SourceInfo src) {
    tokens.push_back(
        PreprocessingToken(src, PP_USER_DEFINED_CHARACTER_LITERAL));
  }
...
void WritePPTokens(const vector<PreprocessingToken>& tokens,
