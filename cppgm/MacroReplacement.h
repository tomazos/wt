...
struct MacroDefinition {
  ...
  enum EMacroParamUsage { eStringized, eUnexpanded, eExpanded };

  set<pair<string, EMacroParamUsage>> param_usage;

  MacroDefinition(const string& macro_name,
                  const vector<PreprocessingToken>& replacement_list);
  MacroDefinition(const string& macro_name, bool var_args,
  ...
  void bless() {
    for (auto& x : replacement_list) x.blessed = true;
  }
  ...
  bool operator==(const MacroDefinition& that) const {
    return function_like == that.function_like &&
           ...
           var_args == that.var_args && parameters == that.parameters;
  }
  ...
  size_t get_param(const string& id) const {
    ...
    if (it == parameters.end()) throw logic_error("parameter not found: " + id);
...
struct MacroReplacer {
  ...
  MacroReplacer(const unordered_map<string, MacroDefinition>& macros,
                vector<PreprocessingToken> raw_input,
                SourcePositionTracker* tracker);
