...
{
	return (code_point >= 0 && code_point < 0xD800) || (code_point >= 0xE000 && code_point < 0x110000);
}
...
vector<int> Utf8Decoder::decode(int input)
{
	if (y == 0)
	{
		if (input == -1)
...
u16string EncodeUtf16(const vector<int>& input)
{
	...
	for (int x : input)
	{
		if (x < 0 || (x >= 0xD800 && x < 0xE000) || x >= 0x110000)
			throw logic_error("utf16 encoding error: " + x);
	...
	for (char16_t c : input)
	{
		...
		case 0:
			...
			else if (x >= 0xD800 && x < 0xDC00)
			{
				...
				throw logic_error("invalid UTF-16 code unit: " + to_string(x));
		...
		case 1:
			if (x >= 0xDC00 && x < 0xE000)
			{
				...
				throw logic_error("invalud UTF-16 code units: " + to_string(W1) + " " + to_string(x));

			break;

		default:
			throw logic_error("DecodeUtf16 unexpected state");
		}
	}

	if (state != 0)
		throw logic_error("unterminated UTF-16 sequence");

	return output;
}

// See C++ Standard Annex E: Universal character names for indentifier characters
bool IsCharacterAllowed_E1(int c)
{
	static const vector<pair<int, int>> ranges =
	{
		{0xA8,0xA8},
		{0xAA,0xAA},
		{0xAD,0xAD},
	...
	auto it = upper_bound(ranges.begin(), ranges.end(), c, [](int a, const pair<int,int>& b) { return a < b.first; });
	if (it == ranges.begin())
		return false;
	else
