...
struct CY86Literal
{
	CY86Literal(ECY86LiteralType type,
	...
	void write(ostream& o) const
	{
		switch (type)
		{
		case YLT_FLOATING: o << "floating"; break;
		case YLT_SIGNEDINTEGER: o << "signed"; break;
		case YLT_UNSIGNEDINTEGER: o << "unsigned"; break;
		case YLT_OTHER: o << "other"; break;
		default: throw logic_error("internal error ECY86LiteralType");
		}
	...
	signed long int to_delta64()
	{
		...
		{
			if (out.size() > 8)
				throw logic_error("unexpected oversized literal");
...
inline X86Register CY86RegisterToX86(ECY86Register reg)
{
	switch (reg)
	{
	case YR_SP:  return XR_RSP;
	case YR_BP:  return XR_RBP;
	case YR_X8:  return XR_R12L;
	case YR_X16: return XR_R12W;
	case YR_X32: return XR_R12D;
	case YR_X64: return XR_R12;
	case YR_Y8:  return XR_R13L;
	case YR_Y16: return XR_R13W;
	case YR_Y32: return XR_R13D;
	case YR_Y64: return XR_R13;
	case YR_Z8:  return XR_R14L;
	case YR_Z16: return XR_R14W;
	case YR_Z32: return XR_R14D;
	case YR_Z64: return XR_R14;
	case YR_T8:  return XR_R15L;
	case YR_T16: return XR_R15W;
	case YR_T32: return XR_R15D;
	case YR_T64: return XR_R15;
	default:
...
inline X86Register XR_SCRATCHW(size_t width)
{
	switch (width)
	{
	case 1: return XR_SCRATCH8;
	case 2: return XR_SCRATCH16;
	case 4: return XR_SCRATCH32;
	case 8: return XR_SCRATCH64;
	default: throw logic_error("XR_SCRATCH Unknown size");
	}
}
...
inline X86Register XR_SCROTCHW(size_t width)
{
	switch (width)
	{
	case 1: return XR_SCROTCH8;
	case 2: return XR_SCROTCH16;
	case 4: return XR_SCROTCH32;
	case 8: return XR_SCROTCH64;
	default: throw logic_error("XR_SCRATCH Unknown size");
	}
}
...
{
	CY86Operand(CY86OperandTypes type)
		: type(type)
	{}
	...
	virtual void assemble_raw(X86Program&) { throw logic_error("internal error: immediate expected"); }
	...
	virtual void assemble_fild(X86Program& program, bool sgned)
	{
		...
		else if (!sgned && width == 8)
		{
			...
			program.add_instruction(XI_MOV(XR_SCRATCH64, to_byte_vector((double)pow((double) 2, 64))));
	...
	virtual void assemble_fistp(X86Program& program, bool sgned)
	{
		if (!sgned && width == 8)
		{
			program.add_instruction(XI_MOV(XR_SCRATCH64, to_byte_vector(0x8000000000000000UL)));
			program.add_instruction(XI_MOV(mem(XR_RSP,-16), XR_SCRATCH64));
			program.add_instruction(XI_FILD(mem(XR_RSP,-16,8)));
			program.add_instruction(XI_FADDP());
...
{
	CY86Register(ECY86Register reg)
		: CY86Operand(YOT_REGISTER)
		, reg(reg)
	{
	...
	virtual void assemble_fpush(X86Program& program)
	{
		...
		default: throw logic_error("invalid operand size for fpush");
		}
	}
...
{
	CY86AbsoluteImmediate(const CY86Literal& literal)
		: literal(literal)
	{
		width = literal.width();
	}
...
{
	CY86Memory() : CY86Operand(YOT_MEMORY)
	{
		width = 0;
	}
...
{
	CY86AbsoluteMemory(unsigned long int address)
		: address(address)
	{}
	...
	virtual void assemble_fipop(X86Program& program, bool sgned)
	{
		...
		program.add_instruction(XI_MOV(XR_SCRATCH64, to_byte_vector(address)));
		program.add_instruction(XI_MOV(mem(XR_SCRATCH64, 0, width), XR_SCROTCHW(width)));
	}
};
...
struct CY86RegisterMemory : CY86Memory
{
	...
	virtual void write(ostream& o) const
	{
		o << "RegisterMemory(" << CY86RegisterToString(reg)
			<< " " << delta << " " << width << ")";
	}
...
{
	CY86LiteralStatement(const CY86Literal& literal)
		: literal(literal)
	{}
	...
	virtual void write(ostream& o) const
	{ o << "LiteralStatement("; literal.write(o); o << ")"; }
...
struct CY86Instruction : CY86Object
{
	...
	size_t assemble(X86Program& program)
	{
		...
		case YO_MOVE32:
		case YO_MOVE64:
			operands[1]->assemble_load(program, X86RegisterTruncate(XR_RAX, operands[1]->width));
			operands[0]->assemble_store(program, X86RegisterTruncate(XR_RAX, operands[1]->width));
			break;
		...
		case YO_JUMPIF:
			{
				...
				program[jne_pos.index].immediate = to_byte_vector<signed char>(end-start);
			}
			break;
		...
		case YO_SDIV64:
		{
			...
			else
			{
				...
				program.add_instruction(XI_MOV(XR_CL, to_byte_vector(byte(8*w-1))));
		...
		case YO_SYSCALL6: operands[7]->assemble_load(program, XR_R9);
		case YO_SYSCALL5: operands[6]->assemble_load(program, XR_R8);
		case YO_SYSCALL4: operands[5]->assemble_load(program, XR_R10);
		case YO_SYSCALL3: operands[4]->assemble_load(program, XR_RDX);
		case YO_SYSCALL2: operands[3]->assemble_load(program, XR_RSI);
		case YO_SYSCALL1:
		case YO_SYSCALL0:
			...
			if (opcode != YO_SYSCALL0)
				operands[2]->assemble_load(program, XR_RDI); // load last because rdi is scratch
...
struct CY86Program
{
	...
	void add_instruction(string opcode_str, const vector<CY86Operand*>& operands)
	{
