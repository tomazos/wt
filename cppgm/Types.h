...
struct CvQualifiers {
  ...
  bool operator==(const CvQualifiers& cv2) const {
    return const_qualified == cv2.const_qualified &&
           volatile_qualified == cv2.volatile_qualified;
  }
  ...
  bool operator<(const CvQualifiers& that) const {
    if (!const_qualified && !volatile_qualified &&
        (that.const_qualified || that.volatile_qualified))
      return true;

    if (that.const_qualified && that.volatile_qualified &&
...
struct Type : Pooled {
  ...
  Type(ETypeCategory category, bool const_qualified, bool volatile_qualified)
      : category(category), cv(const_qualified, volatile_qualified) {}
  ...
  virtual Type* array_to_pointer() {
    throw logic_error("invalid type for array to pointer");
  }
...
struct FundamentalType : Type {
  ...
  FundamentalType(EFundamentalType type, bool const_qualified,
                  bool volatile_qualified)
      : Type(TC_FUNDAMENTAL, const_qualified, volatile_qualified), type(type) {}

  FundamentalType(ETokenType simple_type_keyword, bool const_qualified,
                  bool volatile_qualified)
  ...
  virtual Type* unqualify() { return new FundamentalType(type, false, false); }
  ...
  virtual bool is_nullptr_t() const { return type == FT_NULLPTR_T; }

  virtual bool is_literal_type() const { return is_scalar_type(); }
  ...
  virtual bool allowed_function_parameter_type() const {
    return type != FT_VOID;
  }
...
struct PointerType : Type {
  ...
  PointerType(Type* T, bool const_qualified, bool volatile_qualified)
      : Type(TC_POINTER, const_qualified, volatile_qualified), T(T) {
  ...
  virtual Type* unqualify() { return new PointerType(T, false, false); }

  virtual Type* to_void_pointer() {
    return new PointerType(new FundamentalType(FT_VOID, T->cv.const_qualified,
                                               T->cv.volatile_qualified),
                           cv.const_qualified, cv.volatile_qualified);
  }
  ...
  virtual bool allowed_function_parameter_type() const {
    return !T->is_array_unknown_bound();
  }
  virtual bool allowed_function_return_type() const { return true; }
  ...
  static vector<pair<CvQualifiers, CvQualifiers>>
  get_cv_qualification_signature(PointerType* source, PointerType* destination);
...
struct ReferenceType : Type {
  ...
  ReferenceType(ETypeCategory category, Type* T)
      : Type(category, false, false), T(T) {
  ...
  virtual Type* const_qualify() { return this; }

  virtual Type* volatile_qualify() { return this; }

  virtual Type* unqualify() { return this; }
...
struct ArrayType : Type {
  ...
  ArrayType(Type* element_type, size_t bound)
      : Type(TC_ARRAY, false, false), element_type(element_type), bound(bound) {
  ...
  virtual size_t size() {
    if (!bound) throw logic_error("sizeof(unknown bound)");

    return element_type->size() * bound;
  }

  virtual size_t alignment() { return element_type->alignment(); }
  ...
  virtual bool is_complete() const {
    return bound > 0 && element_type->is_complete();
  }

  virtual bool is_default_initializable() const {
    return element_type->is_default_initializable();
  }
...
struct FunctionType : Type {
  ...
  FunctionType(Type* return_type, vector<Type*> parameter_type_list,
               bool varargs)
  ...
  virtual Type* to_function_parameter_type() {
    return new PointerType(this, false, false);
  }
  ...
  virtual bool equal(Type* that_in) const {
    if (auto that = dynamic_cast<FunctionType*>(that_in)) {
      if (!(this->return_type->equal(that->return_type))) return false;

      if (this->parameter_type_list.size() != that->parameter_type_list.size())
        return false;

      for (size_t i = 0; i < parameter_type_list.size(); i++)
        if (!(this->parameter_type_list[i]->equal(
                that->parameter_type_list[i])))
          return false;

      if (this->varargs != that->varargs) return false;
  ...
  virtual size_t size() { throw logic_error("sizeof(function)"); }

  virtual size_t alignment() { throw logic_error("alignof(function)"); }

  virtual bool is_default_initializable() const {
    throw logic_error("internal error: functions are not initializable");
  }
