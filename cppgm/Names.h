...
struct QualifiedName : Pooled {
  ...
  QualifiedName(bool global) : global(global) {}
  ...
  string get_unqualified_id() const { return identifiers.back(); }
...
struct LinkerName {
  ...
  LinkerName(const string& name) : name(name) {}
...
struct LinkerSymbol : Pooled {
  ...
  static const LinkerSymbol* global_linker_symbol() { return new LinkerSymbol; }
  ...
  const LinkerSymbol* with_function_name(const string& name,
                                         FunctionType* extra) const {
    vector<LinkerName> new_names(names);
  ...
  bool operator<(const LinkerSymbol& that) const { return names < that.names; }

 private:
  LinkerSymbol() = default;

  LinkerSymbol(vector<LinkerName>&& names_in) : names(move(names_in)) {}
};

struct LinkerSymbolLess {
  bool operator()(const LinkerSymbol* const& lhs,
                  const LinkerSymbol* const& rhs) const {
    return *lhs < *rhs;
  }
};
