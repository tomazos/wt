...
constexpr SpecifiersBitField SP_SIMPLE_TYPE_SPECIFIERS =
	SP_CHAR | SP_CHAR16_T | SP_CHAR32_T | SP_WCHAR_T | SP_BOOL | SP_SHORT | SP_LONG_1 |
	SP_LONG_2 | SP_INT | SP_SIGNED | SP_UNSIGNED | SP_FLOAT | SP_DOUBLE | SP_VOID;
...
inline string SpecifiersToString(SpecifiersBitField specifiers)
{
	...
	#define CASE_OUTPUT_SPECIFIER(x) case x: output_specifier(x, #x); break;
	for (int i = 0; i <= MAX_SPECIFIERS_BIT_FIELD; i++)
	{
		...
		default: throw logic_error("internal error: unknown specifier");
		}
	}
...
struct ArrayDeclaratorSuffix : DeclaratorSuffix
{
	...
	ArrayDeclaratorSuffix(RawExpression* expression)
...
struct FunctionDeclaratorSuffix : DeclaratorSuffix
{
	...
	FunctionDeclaratorSuffix(ParameterDeclarationClause* parameter_declarartion_clause)
...
struct Declarator : Pooled
{
	...
	void push_ptr_operator(PtrOperator* ptr_operator) { ptr_operators.push_back(ptr_operator); }
	void push_inner_declarator(Declarator* declarator) { this->declarator = declarator; }
	void push_declarator_id(QualifiedName* name) { this->name = name; }
	void push_entities(set<Entity*> entities) { this->entities = entities; }
	void push_array(RawExpression* expression) { suffixes.push_back(new ArrayDeclaratorSuffix(expression)); }
	void push_parameter_declaration_clause(ParameterDeclarationClause* parameter_declaration_clause)
		{ suffixes.push_back(new FunctionDeclaratorSuffix(parameter_declaration_clause)); }

	bool just_name()
	{
		return ptr_operators.empty() && suffixes.empty() && (name || declarator->just_name());
	}

	bool viable_function_declarator()
	{
		return (name || declarator->just_name()) && suffixes.size() == 1 && suffixes[0]->is_function_declarator();
	}
};
...
struct KeywordDeclSpecifier : DeclSpecifier
{
	...
	KeywordDeclSpecifier(ETokenType keyword)
		: keyword(keyword)
	{};
...
inline SpecifiersBitField KeywordToSpecifier(ETokenType specifier_keyword)
{
	...
	{
	case KW_LONG:		  throw logic_error("long needs special handling");
	case KW_REGISTER:     return SP_REGISTER;
	case KW_STATIC:       return SP_STATIC;
	case KW_THREAD_LOCAL: return SP_THREAD_LOCAL;
	case KW_EXTERN:		  return SP_EXTERN;
	case KW_INLINE:       return SP_INLINE;
	case KW_TYPEDEF:      return SP_TYPEDEF;
	case KW_CONSTEXPR:    return SP_CONSTEXPR;
	case KW_CHAR:         return SP_CHAR;
	case KW_CHAR16_T:     return SP_CHAR16_T;
	case KW_CHAR32_T:     return SP_CHAR32_T;
	case KW_WCHAR_T:	  return SP_WCHAR_T;
	case KW_BOOL:		  return SP_BOOL;
	case KW_SHORT:		  return SP_SHORT;
	case KW_INT:		  return SP_INT;
	case KW_SIGNED:		  return SP_SIGNED;
	case KW_UNSIGNED:     return SP_UNSIGNED;
	case KW_FLOAT:		  return SP_FLOAT;
	case KW_DOUBLE:		  return SP_DOUBLE;
	case KW_VOID:		  return SP_VOID;
	case KW_CONST:		  return SP_CONST;
	case KW_VOLATILE:	  return SP_VOLATILE;
	default: throw logic_error("unknown specifier keyword");
	}
}

inline EFundamentalType SimpleTypeSpecifiersToFundamentalType(SpecifiersBitField simple_type_specifiers)
{
	switch (simple_type_specifiers)
	{
	case SP_CHAR: return FT_CHAR;
	case SP_UNSIGNED | SP_CHAR: return FT_UNSIGNED_CHAR;
	case SP_SIGNED | SP_CHAR: return FT_SIGNED_CHAR;
	case SP_CHAR16_T: return FT_CHAR16_T;
	case SP_CHAR32_T: return FT_CHAR32_T;
	case SP_BOOL: return FT_BOOL;
	case SP_UNSIGNED: return FT_UNSIGNED_INT;
	case SP_UNSIGNED | SP_INT: return FT_UNSIGNED_INT;
	case SP_SIGNED: return FT_INT;
	case SP_SIGNED | SP_INT: return FT_INT;
	case SP_INT: return FT_INT;
	case SP_UNSIGNED | SP_SHORT | SP_INT: return FT_UNSIGNED_SHORT_INT;
	case SP_UNSIGNED | SP_SHORT: return FT_UNSIGNED_SHORT_INT;
	case SP_UNSIGNED | SP_LONG_1 | SP_INT: return FT_UNSIGNED_LONG_INT;
	case SP_UNSIGNED | SP_LONG_1: return FT_UNSIGNED_LONG_INT;
	case SP_UNSIGNED | SP_LONG_2 | SP_INT: return FT_UNSIGNED_LONG_LONG_INT;
	case SP_UNSIGNED | SP_LONG_2: return FT_UNSIGNED_LONG_LONG_INT;
	case SP_SIGNED | SP_LONG_1 | SP_INT: return FT_LONG_INT;
	case SP_SIGNED | SP_LONG_1: return FT_LONG_INT;
	case SP_SIGNED | SP_LONG_2 | SP_INT: return FT_LONG_LONG_INT;
	case SP_SIGNED | SP_LONG_2: return FT_LONG_LONG_INT;
	case SP_LONG_2 | SP_INT: return FT_LONG_LONG_INT;
	case SP_LONG_2: return FT_LONG_LONG_INT;
	case SP_LONG_1 | SP_INT: return FT_LONG_INT;
	case SP_LONG_1: return FT_LONG_INT;
	case SP_SIGNED | SP_SHORT | SP_INT: return FT_SHORT_INT;
	case SP_SIGNED | SP_SHORT: return FT_SHORT_INT;
	case SP_SHORT | SP_INT: return FT_SHORT_INT;
	case SP_SHORT: return FT_SHORT_INT;
	case SP_WCHAR_T: return FT_WCHAR_T;
	case SP_FLOAT: return FT_FLOAT;
	case SP_DOUBLE: return FT_DOUBLE;
	case SP_LONG_1 | SP_DOUBLE: return FT_LONG_DOUBLE;
	case SP_VOID: return FT_VOID;
	default: throw logic_error("invalid simple type specifier combination");
	}
}
...
struct DeclSpecifierSeq
{
	...
	{
		if (auto keyword_specifier = dynamic_cast<KeywordDeclSpecifier*>(specifier))
		{
			...
			else
			{
				...
				if (specifiers & specifier)
					throw logic_error("duplicate specifier");

				specifiers |= specifier;
			}
		}
		else if (auto typedecl_specifier_in = dynamic_cast<TypeDeclSpecifier*>(specifier))
		{
		...
		else if (auto decltype_specifier_in = dynamic_cast<DeclTypeSpecifier*>(specifier))
		{
	...
	bool empty() { return specifiers == 0 && !decltype_specifier && !typedecl_specifier; }
};
...
//
//	SimpleDeclaration(DeclSpecifierSeq* decl_specifier_seq, InitDeclaratorList* init_declarator_list)
...
struct FunctionDefinition : Declaration
{
	...
	FunctionDefinition(DeclSpecifierSeq* decl_specifier_seq, Declarator* declarator, Statement* statement)
		: decl_specifier_seq(decl_specifier_seq)
		, declarator(declarator)
		, statement(statement)
	{}
...
struct AliasDeclaration : Declaration
{
	...
	AliasDeclaration(const string& name, Type* type)
		: name(name)
		, type(type)
	{}
};
...
struct UsingDeclaration : Declaration
{
	...
	UsingDeclaration(QualifiedName* name)
		: name(name)
	{}
};
...
struct ConditionDeclaration : Condition
{
	...
	ConditionDeclaration(DeclSpecifierSeq* decl_specifier_seq, Declarator* declarator, RawExpression* expression)
		: decl_specifier_seq(decl_specifier_seq)
		, declarator(declarator)
		, expression(expression)
	{}
...
struct ExpressionCondition : Condition
{
	...
	ExpressionCondition(RawExpression* expression)
		: expression(expression)
	{}
};
...
{
	virtual Expression* create_initializer_expression(SymbolTable& /*symtab*/, Type* /*destination*/) { throw logic_error("not yet implemented"); }
};
...
struct DirectInitializer : Initializer
{
	...
	void write(ostream& out) const { out << "DirectInitializer (" << expression_list << ")"; }
};
...
struct CopyInitializer : Initializer
{
	...
	Expression* create_initializer_expression(SymbolTable& symtab, Type* destination);

	CopyInitializer(RawExpression* expression)
		: expression(expression)
	{}

	void write(ostream& out) const { out << "CopyInitializer (" << expression << ")"; }
};

struct DefaultInitializer : Initializer
{
	Expression* create_initializer_expression(SymbolTable&, Type*) { return nullptr; }
...
struct ParameterDeclaration
{
	...
	ParameterDeclaration(DeclSpecifierSeq* decl_specifier_seq, Declarator* declarator)
		: decl_specifier_seq(decl_specifier_seq)
		, declarator(declarator)
	{}

	ParameterDeclaration(DeclSpecifierSeq* decl_specifier_seq, Declarator* declarator, RawExpression* expression)
		: decl_specifier_seq(decl_specifier_seq)
		, declarator(declarator)
