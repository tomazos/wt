...
struct SymbolTable
{
	...
	Variable* create_temporary(EStorageDuration, Type*, Expression* initializer_expression);
	...
	void enter_namespace_definition(const string& namespace_name, bool inline_namespace);
	...
	set<Entity*> current_lookup_entity(QualifiedName* name, bool direct_only, EntityCategory entity_category);
	set<Entity*> current_lookup_entity(const string& name, EntityCategory entity_category);
	...
	DeclaratorInfo analyze_declarator(Type* base_type, Declarator*, bool last_ref_in = false);

	size_t evaluate_array_bound(RawExpression* expr);

	Entity* declare_name(SpecifiersBitField specifiers, QualifiedName* name, Type* type, Entity* entity);
...
struct ScopeChanger
{
	...
	void reset()
	{
		symtab.current_namespace = old_namespace;
	}
	...
	set<Entity*> enter_qualified_declarator_id(QualifiedName* name)
	{
		...
		if (entities.empty())
			throw logic_error("qualified name not found");
