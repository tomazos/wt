...
struct PostTokenInputStream : IPPTokenStream
{
	...
	PostTokenInputStream(IPostTokenOutputStream& output)
		: output(output)
	{}
	...
	virtual void emit_pp_number(SourceInfo src)
	{
		...
		try
		{
			...
			case TT_LITERAL:
			{
				...
				output.emit_literal(src, p->fundamental_type, p->data.data(), p->data.size());
			}
			break;
			...
			case TT_UDINTEGER:
			{
				...
				output.emit_user_defined_literal_integer(src, p->ud_suffix, p->source);
			}
			break;
			...
			case TT_UDFLOATING:
			{
				...
				output.emit_user_defined_literal_floating(src, p->ud_suffix, p->source);
			}
			break;
	...
	virtual void emit_user_defined_character_literal(SourceInfo src)
	{
		...
		try
		{
			...
			output.emit_user_defined_literal_character(src, p->ud_suffix, p->fundamental_type, p->data.data(), p->data.size());
		}
		catch (exception& e)
		{
			output.emit_invalid(src);
	...
	void terminate_string_literals()
	{
		...
		try
		{
			...
			if (token->token_type == TT_UDSTRING)
			{
				...
				output.emit_user_defined_literal_string_array(src, p->ud_suffix, p->num_elements, p->fundamental_type, p->data.data(), p->data.size());
			}
			else if (token->token_type == TT_LITERAL)
			{
				auto p = dynamic_cast<LiteralToken*>(token.get());
				output.emit_literal_array(src, p->num_elements, p->fundamental_type, p->data.data(), p->data.size());
			}
			else
				throw("unexpected token_type from TokenizeStringLiterals: " + SimpleTokenTypeToString(token->token_type));
		}
		catch (exception& e)
		{
			output.emit_invalid(src);
...
//			auto p = dynamic_cast<LiteralToken*>(token.get());
//			output.emit_literal_array(source, p->num_elements, p->fundamental_type, p->data.data(), p->data.size());
//		}
//		catch (exception& e)
//		{
...
//			auto p = dynamic_cast<UserDefinedStringLiteralToken*>(token.get());
//			output.emit_user_defined_literal_string_array(source, p->ud_suffix, p->num_elements, p->fundamental_type, p->data.data(), p->data.size());
//		}
//		catch (exception& e)
//		{
//			output.emit_invalid(source);
