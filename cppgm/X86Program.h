...
struct X86Program
{
	...
	{
		immediate_fixups.push_back(ImmediateFixup{instructions.size()-1, label, delta});
	}
	...
	void pad_to_alignment(size_t alignment)
	{
		...
		if (extra != 0)
			add_instruction(XI_RAW(vector<byte>(alignment-extra, 0)));
	}
	...
	vector<byte> output()
	{
		...
		for (auto instruction : instructions)
		{
			...
			machine_codes.insert(machine_codes.end(), machine_code.begin(), machine_code.end());
		}
