...
//	}
	...
	{
		if (is_error)
			throw logic_error("erroneous value evaluated eval_bool");
		if (is_signed)
		{
			if (s)
				return true;
	...
	{
		if (is_error)
			throw logic_error("erroneous value evaluated is_ok_shift");
		if (is_signed)
			return (s >= 0 && s < 64);
		else
			return (u < 64);
...
//
	...
	CtrlExprVal(const LiteralToken& token)
		: is_error(false)
	{
		...
		if (token.is_array)
			throw logic_error("array in controlling expression");

		switch (token.fundamental_type)
		{
		case FT_SIGNED_CHAR: set_signed(token.get<signed char>()); return;
		case FT_SHORT_INT: set_signed(token.get<short int>()); return;
		case FT_INT: set_signed(token.get<int>()); return;
		case FT_LONG_INT: set_signed(token.get<long int>()); return;
		case FT_LONG_LONG_INT: set_signed(token.get<long long int>()); return;
		case FT_UNSIGNED_CHAR: set_unsigned(token.get<unsigned char>()); return;
		case FT_UNSIGNED_SHORT_INT: set_unsigned(token.get<unsigned short int>()); return;
		case FT_UNSIGNED_INT: set_unsigned(token.get<unsigned int>()); return;
		case FT_UNSIGNED_LONG_INT: set_unsigned(token.get<unsigned long int>()); return;
		case FT_UNSIGNED_LONG_LONG_INT: set_unsigned(token.get<unsigned long long int>()); return;
		
		case FT_WCHAR_T: set_signed(token.get<wchar_t>()); return;
		case FT_CHAR: set_signed(token.get<char>()); return;
		case FT_CHAR16_T: set_unsigned(token.get<char16_t>()); return;
		case FT_CHAR32_T: set_unsigned(token.get<char32_t>()); return;
		
		case FT_BOOL: 
		case FT_VOID:
		case FT_NULLPTR_T:
			throw logic_error("internal error: unexpected literal type passed as LiteralToken");
	...
	CtrlExprVal operator +() const { return is_signed ? CtrlExprVal(+s, is_error) : CtrlExprVal(+u, is_error); }
	CtrlExprVal operator -() const { return is_signed ? CtrlExprVal(-s, is_error) : CtrlExprVal(-u, is_error); }
	CtrlExprVal operator !() const { return is_signed ? CtrlExprVal(intmax_t(!s), is_error) : CtrlExprVal(intmax_t(!u), is_error); }
	CtrlExprVal operator ~() const { return is_signed ? CtrlExprVal(~s, is_error) : CtrlExprVal(~u, is_error); }
...
#define DefineCtrlExprBinOpL(op)                                                \
	...
	{                                                                           \
		...
		/* if (!is_signed && !x.is_signed) */ return CtrlExprVal(intmax_t(u op x.u), e); \
	}
...
struct CtrlExprParser
{
	...
	CtrlExprParser(const vector<PToken>& tokens_in, function<bool(const string&)> is_defined_f_in)
		: tokens(tokens_in)
		, cur(tokens.begin())
		, is_defined_f(is_defined_f_in)
	{
	...
	{
		static PToken invalid_token = make_shared<Token>(SourceInfo{"", "", 0}, TT_INVALID);
	...
	CtrlExprVal parse_conditional_expression()
	{
		...
		if (peek_lookahead()->token_type == OP_QMARK)
		{
			...
			if (!on_true.is_signed)
				on_false.is_signed = false;

			if (!on_false.is_signed)
				on_true.is_signed = false;
	...
	CtrlExprVal parse_equality_expression()
	{
		...
		while (peek_lookahead()->token_type == OP_EQ || peek_lookahead()->token_type == OP_NE)
		{
	...
	CtrlExprVal parse_relational_expression()
	{
				...
				|| peek_lookahead()->token_type == OP_GE)
		{
			...
			{
			case OP_LT: val = (val < next); break;
			case OP_GT: val = (val > next); break;
			case OP_LE: val = (val <= next); break;
			case OP_GE: val = (val >= next); break;
	...
	CtrlExprVal parse_unary_expression()
	{
			...
			|| peek_lookahead()->token_type == OP_COMPL)
		{
			...
			switch (op->token_type)
			{
			case OP_PLUS: return +val;
			case OP_MINUS: return -val;
			case OP_LNOT: return !val;
			case OP_COMPL: return ~val;
			default: throw logic_error("internal error");
			}
		}
		else
			...
			else if (idname == "defined")
			{
				...
				if (target->token_type == OP_LPAREN)
				{
					...
					if (pop_lookahead()->token_type != OP_RPAREN)
						throw logic_error("no closing paren on defined(identifier) in controlling expression");
				}

				if (target->token_type != TT_IDENTIFIER)
					throw logic_error("non-identifier after defined in controlling expression");
