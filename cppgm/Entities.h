...
inline const char* ELinkageToString(ELinkage e)
{
	switch (e)
	{
	case LK_NONE: return "LK_NONE";
	case LK_EXTERNAL: return "LK_EXTERNAL";
	case LK_INTERNAL: return "LK_INTERNAL";
	default: throw logic_error("internal error: unknown linkage");
	}
}
...
constexpr EntityCategory EC_ANY = EC_FUNCTION | EC_OBJECT | EC_REFERENCE | EC_TYPEDEF | EC_NAMESPACE;
...
struct Entity : Pooled
{
	...
	virtual Entity* redeclare(SpecifiersBitField specifiers, Type* type, Namespace* ns) = 0;

	virtual void set_initializer(SymbolTable&, Initializer* initializer)
	{
		if (initializer)
			throw logic_error("invalid entity for initializer");
	}

	virtual bool has_category(EntityCategory entity_category) = 0;

	virtual bool is_defined() { throw logic_error("is_defined on invalid entity"); }
	virtual size_t image_size() { throw logic_error("image_size on invalid entity"); }
	virtual size_t image_alignment() { throw logic_error("image_size on invalid entity"); }

	virtual void relocate() { }

	virtual void set_function_definition() { throw logic_error("invalid entity for function definition"); }
...
struct Function : Entity
{
	...
	Function(const string& name, SpecifiersBitField specifiers, FunctionType* type, Namespace* ns);
	...
	{
		out << "function " << name << " " << SpecifiersToString(specifiers) << " " << type << " defined=" << defined;
	}
	...
	{
		if (defined)
			throw logic_error("function " + name + " already defined");
		defined = true;
	}
...
//	}
//
//	Entity* redeclare(SpecifiersBitField specifiers, Type* type_in, Namespace* ns);
//};
...
struct Variable : Entity
{
	...
	Variable(const string& name, SpecifiersBitField specifiers, Type* type, Namespace* ns);
	...
	Entity* redeclare(SpecifiersBitField specifiers_in, Type* type_in, Namespace* ns);
	...
	virtual bool is_defined()
	{
		return initializer || initializer_expression;
	}
...
struct TypeDef : Entity
{
	...
	TypeDef(SpecifiersBitField specifiers, const string& name, Type* type, Namespace* ns);
...
struct Namespace : Entity
{
	...
	void set_global()
	{
		linker_symbol = LinkerSymbol::global_linker_symbol();
	}
	...
	void add_namespace(Namespace* ns)
	{
		...
		if (ns->inline_namespace || ns->name.empty())
			add_using_directive(ns);
...
//		ns2->dump(cout, true);
	...
	void add_inline_namespace(Namespace* ns)
	{
		...
		if (inline_namespace)
			enclosing_namespace->add_inline_namespace(ns);
	}
	...
	set<Entity*> simple_lookup_entity(const string& entity_name, bool direct_only, EntityCategory entity_category)
	{
		set<Entity*> result;
		...
		for (auto it = range.first; it != range.second; ++it)
		{
			...
			if (entity->has_category(entity_category))
	...
	set<Entity*> direct_lookup_set(const string& name, bool direct_only, EntityCategory entity_category)
	{
		set<Entity*> result;

		for (Namespace* ns : inline_namespace_set)
		{
			auto entities = ns->simple_lookup_entity(name, direct_only, entity_category);
	...
	set<Entity*> namespace_qualified_lookup_set(const string& name, bool direct_only, EntityCategory entity_category, set<Namespace*>& done)
	{
		if (done.count(this))
			return {};
		else
		...
		if (!results.empty() || direct_only)
			return results;

		for (Namespace* ns : inline_namespace_set)
			for (Namespace* ns2 : ns->using_directives_direct)
			{
				auto entities = ns2->namespace_qualified_lookup_set(name, direct_only, entity_category, done);
	...
	set<Entity*> qualified_lookup_entity(const string& name, bool direct_only, EntityCategory entity_category = EC_ANY)
	{
		set<Namespace*> done;

		auto results = namespace_qualified_lookup_set(name, direct_only, entity_category, done);
	...
	Namespace* qualified_lookup_namespace(const string& name)
	{
		...
		if (results.size() > 1)
			throw logic_error(name + " ambiguous");
	...
	set<Entity*> unqualified_lookup_entity(const string& name, EntityCategory entity_category,
	                                             map<Namespace*, set<Namespace*>>& search_namespaces);

	Entity* declare_name(SpecifiersBitField specifiers, const string& name, Type* type);
	...
	void add_using_declaration(const string& name, set<Entity*> entities)
	{
		...
		{
			bool save = true;
			for (auto it = existing_entities.first; it != existing_entities.second; ++it)
			{
			...
			if (save)
				scope.insert(make_pair(name, entity));
		}
	}

	void dump_namespace_set(const string& name, ostream& out, const set<Namespace*>& s)
	{
		out << name << " = { ";
		for (Namespace* n : s)
			if (n->name.empty())
	...
	void dump(ostream& out, bool extra = false)
	{
		...
		if (extra)
		{
			...
			dump_namespace_set("using_directives_direct", out, using_directives_direct);
			dump_namespace_set("using_directives_closure", out, using_directives_closure);
			dump_namespace_set("reverse_using_directives_closure", out, reverse_using_directives_closure);
